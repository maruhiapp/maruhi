...
- 上限到達で個別行を落とす窓には、`auth.login_failed_suppressed` を**窓あたり 1 行**(ベストエフォート)残す(2026-08-24 deepsec M4 対応): 全体上限は全 actor 共有のため、匿名の洪水が枠を使い切ると後続の標的型失敗が黙って消える — 抑制が起きたこと自体を監査可能にする。actor は個別行と同じく user_id なしの type=user とし、payload は窓長と上限のみ(§1-2 の禁止情報 — 外部 provider ID・IP — をこの経路でも増やさない)。粗い発信元単位の別枠計数は「limiter 用の追加状態に発信元識別子を持つ」ことになるため採らない。ここでの「監査可能」は `auth.login_failed` 本体と同じく**運営者ビュー(D1 直接参照)での可視性**を指す — actor / target user_id を持たないため本人軸の読み取り(§7)には現れず、運営者向け API は L-4(SECURITY_REVIEW_2026-08-14)と同じく未実装・待ちである
