...
- 上限は **`auth_method` 単位のバケット**で数える(2026-08-24 deepsec R4 対応): 全 actor 共有の単一枠だと、片方の経路(例 device flow)への匿名の洪水がもう片方(Web OAuth)の標的型失敗まで黙って消せる。バケットを分けると、少なくとも**洪水の経路と別の経路は記録が生き残る**。同一経路内での被覆(同じ `auth_method` を狙う洪水)は残る限界であり、その可視化は下の抑制件数が担う。**発信元単位**(IP / その前置ハッシュ)の別枠計数は引き続き採らない — 「limiter 用の追加状態に発信元識別子を持つ」ことになり §1-2 の線引きを崩す
...
- 上限到達で個別行を落とす窓には `auth.login_failed_suppressed` を残す(2026-08-24 deepsec M4 / R4 対応)。抑制が起きたこと自体に加えて**抑制の量**も監査可能にするため、窓あたり 1 行ではなく、バケットの抑制件数が **1・10・100・…(10 の冪)** に達した時点で 1 行残す — 書き込みは抑制件数に対して対数的に有界(洪水下でも窓あたり数行)で、行の密度と最後の件数から抑制の規模が読める。payload は `auth_method`・窓長・上限・その時点の抑制件数のみ(§1-2 の禁止情報 — 外部 provider ID・IP — をこの経路でも増やさない)。actor は個別行と同じく user_id なしの type=user。ここでの「監査可能」は `auth.login_failed` 本体と同じく**運営者ビュー(D1 直接参照)での可視性**を指す — actor / target user_id を持たないため本人軸の読み取り(§7)には現れず、運営者向け API は L-4(SECURITY_REVIEW_2026-08-14)と同じく未実装・待ちである
