...
| Q4 | (variable × environment) × 期間 → 閲覧・変更した主体一覧(逆引き。インシデント対応用) | Q2 と同じ索引。**集約形の `var.read` は変数 ID を列に持たない**ため、環境単位の行(variable_id IS NULL・event = var.read)を Q2 の存在区間(var.created 〜 var.deleted)で seq 範囲に絞ってから payload の `variables` を検査する(値付き pull は環境の全アクティブ変数を返すため、存在区間内の集約行はほぼ全て当該変数を含み、検査は「ページ上限 × payload 長」で止まる — 存在区間の外では 1 行も検査しない)。§7 の `variable_id` フィルタも同じ形(旧形の列一致との和集合) |
