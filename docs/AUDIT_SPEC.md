...
Version: 1.9-draft
Status: 0.6 までは所有者承認済み(0.3 は 2026-08-02 の PR #18 マージで承認。§3.3 の `dek.registered` への署名者鍵 FP の追加は CRYPTO_SPEC §5.1 として PR #21 で承認済み。§3.3 の署名付きデータ操作への actor 鍵 FP 拡張と §3.4 の `chain.environment_created` は CRYPTO_SPEC 0.4-draft の波及として 2026-08-04 の PR #27 マージで承認。0.6 = §5.2 案 A の D1 実装と §3.1 の記録細則 — 2026-08-10 セッション 21、マージをもって承認)。0.7-draft = Phase 2 機能裁定の起草(2026-08-12 セッション 22): §3.2 invite.* / §3.5 ワークロードリースへの改訂 / §4.1 revoke_server 変種の入力更新 / §6 可視性クラス(未決 #1 の解消)/ §7 読み取り API — **本改訂 PR のマージをもって所有者承認とする**。0.8-draft = Wave 2 B2 の所有者裁定(2026-08-15 セッション 25): §3.3 `rotation.recommended` / `rotation.dismissed` の記録粒度・actor・取り下げ権限、`dek.deleted` の自動掃除変種(AUTH_SPEC §12-6)、`var.version_pushed` の再暗号化マーカー payload / §4.1 手順 5 の解消導出からのマーカー付き push の除外(義務ローテーションの再暗号化 push による全自動誤解消 — 仕様内部の矛盾 — の解消)/ §7 取り下げ操作エンドポイント — **本改訂を含む実装 PR のマージをもって所有者承認とする**(PR #70 マージ済み = 承認済み)。0.9-draft = Wave 2 C1 の所有者裁定(2026-08-16 セッション 26): §5.1 `row_id` 列(ワイヤ行識別子 = ランダム値)/ §7 読み取り API のカーソル・行識別子の不透明化と `seq` の admin 限定開示(§7「件数にも漏らさない」と §6「欠番 = 削除の痕跡」の両立 — pullfrog レビュー指摘の解消)— **本改訂を含む実装 PR のマージをもって所有者承認とする**(PR #71 マージ済み = 承認済み)。1.0-draft = Wave 3 D の起草(2026-08-18 セッション 27 — 設計探索は docs/notes/session-27.md): §3.4 `chain.checkpointed` / §5.1 監査ヘッド累積ハッシュ / §6 チェックポイントによる事後改竄検出とヘッド申告を監査イベント化しない判断 / §8 未決 #2 の解消(CRYPTO_SPEC §6.2 `checkpoint` に統合)— **本改訂 PR のマージをもって所有者承認とする**。1.1-draft = W3a(2026-08-30): §3.1 `auth.token_revoked` の actor の明確化(actor = 実行主体・対象 id は payload — AUTH_SPEC §6 指定失効の導入に伴う。新イベントは追加しない)— **本改訂を含む実装 PR のマージをもって所有者承認とする**。1.2-draft = S0(値なしスキーマ — CRYPTO_SPEC 0.8-draft / AUTH_SPEC 0.16-draft)の起草(2026-08-30 セッション 46): §3.3 `project.schema_policy_changed`(AUTH_SPEC §12-11)と `var.created` の declared 作成の注記 — **本改訂 PR のマージをもって所有者承認とする**。1.3-draft = スキーマ再発行の監査乖離の解消(2026-09-01 オーナー裁定 — PR #121 / #123 申し送り。候補比較は PR #125 の申し送り対応スレッド): §3.3 `var.schema_reissued` の新設(名前不変のメタステートメント再発行 — 従来は `var.renamed` として記録され、改名していない操作が「renamed」を名乗っていた)。棄却案 = `var.renamed` の意味拡張(イベント名が恒久に事実を偽る)・統一イベント `var.meta_reissued` への正規化(過去行との語彙断絶)・payload フラグのみ(名前の嘘が残る)・読み側再分類(監査行単体では改名の有無を判定できない — 判定材料が揃う唯一の位置は書き込み時のサーバー)— **本改訂を含む実装 PR のマージをもって所有者承認とする**。1.4-draft = H1(サインアップ制御 — AUTH_SPEC 0.18-draft §3)の起草(2026-09-01): §3.1 `auth.signup_denied` の新設(signupPolicy による新規作成拒否の記録 — H3 の「サインアップ拒否の計数」トリップワイヤの材料。`auth.login_failed` と同じ固定窓上限規律)と `auth.user_created` payload への `signupInviteId` の追記(招待コード消費由来の作成の突合材料)— **本改訂を含む実装 PR のマージをもって所有者承認とする**。1.5-draft = 監査ログの成長密度対策 ②(2026-09-02 オーナー決定 — 未決 4 の前倒し解消。H0〔docs/notes/hosted-design.md §3-3〕は集約を「ドッグフーディング実測後」と置いていたが、実測を待たず設計・実装する): §3.3 `var.read` の記録粒度を「変数ごとに 1 行」から「値付き一括 pull ごとに環境単位 1 行(payload に返した変数の列挙)」へ改訂(集約形)/ §4.2 Q3・Q4 の索引要件に payload 内列挙への依存を明記 / §5.3 の量の記述 / §8 未決 4 の解消。AUTH_SPEC 0.20-draft §12-7 / §12-8 と同時改訂 — **本改訂を含む実装 PR のマージをもって所有者承認とする**。1.6-draft = KL3(master 鍵ラップ台帳 — CRYPTO_SPEC 0.9-draft §8 / AUTH_SPEC 0.21-draft §13-6〜13-10)の起草(2026-09-12。設計録は docs/notes/integration-options.md 補足 19): §3.1 に `auth.key_wrap_*` / `auth.guardian_*` / `auth.key_handoff_*` の 9 事件を追加(既存の `auth.recovery_*` は不変)。設計の 13 項目は 2026-09-12 に所有者承認済み — **本改訂を含む実装 PR のマージをもって仕様文言の承認とする**。1.7-draft = IV(2026-09-13。設計録は docs/notes/integration-options.md 補足 21): §3.2 invite.accepted への注記(payload 不変・裏付け元の login を書かない)。事件の追加なし — **本改訂を含む実装 PR のマージをもって仕様文言の承認とする**。1.8-draft = ES + PF1(2026-09-14 — CRYPTO_SPEC 0.11-draft / AUTH_SPEC 0.23-draft。設計録は docs/notes/es-design.md、起草時のドラフトは docs/notes/es-spec-drafts.md): §3.3 `rotation.recommended` の trigger / §3.4 のミラー payload の scope・四眼 4 種と適用行 / §4.1 の環境別アクセス窓と change_role 変種 / §6 の可視性クラス不変。設計の 24 項目は 2026-09-14 に所有者承認済み — **本改訂 PR のマージをもって所有者承認とする**。**1.8-draft への K3 追記(2026-09-15 — サーバーの scope 執行、設計録 es-design.md §9)**: §4.2 Q1 の列挙に `chain.role_changed` を追加(§4.1 手順 2 が窓の復元に role_changed の payload を要求し §3.4 も ★ にしているのに Q1 行が旧列挙のままだった索引要件の記述訂正 — 合意規則ではない。索引は不変)— **本追記を含む実装 PR のマージをもって所有者承認とする**。1.9-draft = DK(デバイス鍵分離 — 2026-09-19。CRYPTO_SPEC 0.12-draft / AUTH_SPEC 0.24-draft。設計録は docs/notes/dk-design.md、起草時のドラフトは docs/notes/dk-spec-drafts.md): §2 の鍵 FP = 端末の注記 / §3.1 の 3 事件の説明文の予備鍵化(事件名は不変)/ §3.4 の `chain.device_added` / `chain.device_revoked` / §4.1 の `revoke_device` 変種(端末の窓による区間の絞り込み・(a) は user_id 照合 — 起草時の「actor 鍵 FP による照合」は `var.read` が FP を持たないため K1 レビューで訂正。承認項目 8 の一部訂正、設計録 dk-design.md §6 K1-12)/ §4.2 Q1 の列挙 / §6 のクラス不変。設計の 16 項目は 2026-09-20 に所有者承認済み(設計録 §4) — **本改訂 PR のマージをもって仕様文言の承認とする**
...
- **鍵 FP は端末を指す(2026-09-19 DK)**: 端末鍵(CRYPTO_SPEC §3)の導入後、`key_fingerprint` は「その時点でその user_id がその操作に使った端末」を同定する。actor に端末の別フィールドは持たない(識別子は user_id + 鍵 FP のまま — §1-2 不変)
...
| `auth.signup_denied` | auth_method, 理由種別(`policy-closed` / `invite-required` / `invite-invalid`) | signupPolicy(AUTH_SPEC §3 — 2026-09-01 H1)による新規作成の拒否。GitHub 認証は成功しているが、提示された外部 ID は**記録しない**(§1-2 — 拒否時点で内部 user_id は存在しない)。actor は `auth.login_failed` と同じ user_id なしの type=user |
...
| `auth.recovery_blob_fetched` | — | ラップ済み予備鍵(旧 master 秘密鍵 — 2026-09-19 DK。事件名は不変)の取得(CRYPTO_SPEC §8。セキュリティ上の要監視イベント) |
| `auth.recovery_code_reissued` | — | 再発行(旧ラップ削除を含む) |
| `auth.key_wrap_registered` | kind(`passkey-prf` / `guardian`)、wrapId / groupId、mode、recipientCount | 予備鍵ラップ台帳(旧 master 鍵ラップ台帳 — 2026-09-19 DK。事件名は不変)への登録(AUTH_SPEC §13-7 — 2026-09-12 KL3)。actor = ward |
...
| `auth.key_handoff_approved` | requestId、source(groupId — `device` は 2026-09-19 DK で削除。既存行の `device` は歴史値として有効)、shareIndex | 承認の受理(**要監視**)。actor = 承認者(user_id + 鍵 FP = 承認に使った端末)、target = ward |
...
- `auth.login_failed` の記録は**固定窓の上限つき**(1 時間 100 行、超過分は記録しない。ベストエフォート): 本イベントは唯一の未認証経路からの書き込みであり、無効リクエストの洪水による D1 書き込み増幅(可用性・コスト攻撃)を有界にする。洪水そのものは窓内の上限到達として観測でき、要ローテーション検出(§4)の入力ではないため SHOULD 記録で足りる(2026-08-10 セッション 21 セキュリティレビュー対応)
- 上限は **`auth_method + reason` 単位のバケット**で数える(2026-08-24 deepsec R4 / 2026-08-27 S5 対応): 全 actor 共有の単一枠や `auth_method` だけの枠だと、片方の経路・理由への匿名の洪水が別理由の失敗まで黙って消せる。現行の失敗は認証前で target user_id を持たず、個別行が運ぶ区別可能な分類は `auth_method` と `reason` だけなので、その直積を独立枠にして**監査が元から持つ攻撃形状を相互に失明させない**。同じ reason 内の各試行には区別可能な対象情報が元からなく、抑制後の量は下の reason 付きマーカーが担う。**発信元単位**(IP / その前置ハッシュ)の別枠計数は引き続き採らない — 「limiter 用の追加状態に発信元識別子を持つ」ことになり §1-2 の線引きを崩す
- **`auth.signup_denied` の記録規律(2026-09-01 H1)**: `auth.login_failed` と同じ固定窓上限規律に従う(未認証経路からの書き込みであり、拒否の洪水による D1 書き込み増幅を有界にする — SHOULD 記録)。バケットは `イベント名 + reason` 単位(拒否理由ごとに独立の枠 — login_failed の `auth_method + reason` と同じ「監査が元から持つ攻撃形状を相互に失明させない」論法)。上限到達の窓には `auth.signup_denied_suppressed` マーカーを login_failed と同じ 10 の冪規律で残す。actor / target user_id を持たないため本人軸の読み取り(§7)には現れず、可視性は運営者ビュー(D1 直接参照)— H3 のトリップワイヤ「サインアップ拒否の計数」はこの行を数える
- 計数は監査ログの走査ではなく**専用のカウンタ行**(バケットごと 1 行の可変状態。append-only の §1-4 は監査テーブルの規律であり、このカウンタは監査行ではない)で行う(2026-08-24 deepsec R5 対応): 追記のたびに窓内の `auth.login_failed` 行を数えると、append-only で伸び続けるテーブルを毎回走査することになり、有界にしたい洪水の経路そのものがコスト増幅器になる。窓のリセット・加算・上限判定は **1 文の条件付き UPSERT**(`RETURNING` で新しい計数を受け取る)で原子的に行う — recovery 取得計数(AUTH_SPEC §13-3)と同じ形
- 上限到達で個別行を落とす窓には `auth.login_failed_suppressed` を残す(2026-08-24 deepsec M4 / R4 対応)。抑制が起きたこと自体に加えて**抑制の量と理由**も監査可能にするため、窓あたり 1 行ではなく、バケットの抑制件数が **1・10・100・…(10 の冪)** に達した時点で 1 行残す — 書き込みは抑制件数に対して対数的に有界(洪水下でも窓あたり数行)で、行の密度と最後の件数から抑制の規模が読める。payload は `auth_method`・`reason`・窓長・上限・その時点の抑制件数のみ(§1-2 の禁止情報 — 外部 provider ID・IP — をこの経路でも増やさない)。actor は個別行と同じく user_id なしの type=user。ここでの「監査可能」は `auth.login_failed` 本体と同じく**運営者ビュー(D1 直接参照)での可視性**を指す — actor / target user_id を持たないため本人軸の読み取り(§7)には現れず、運営者向け API は L-4(SECURITY_REVIEW_2026-08-14)と同じく未実装・待ちである
- トークン**使用**はイベント化しない(高頻度・低情報。api_tokens.last_used_at と、データ系イベントの actor.api_token_id が代替)
- **`auth.token_revoked` の actor(2026-08-30 W3a — 指定失効の導入に伴う明確化)**: actor は**失効を実行した主体**であり、失効対象のトークン id は payload の `tokenId` が運ぶ。自己失効(CLI logout — 提示トークン自身)では actor のトークン = 対象なので `actor_api_token_id` と `payload.tokenId` が一致するが、指定失効(AUTH_SPEC §6 — セッション主体・`*` × admin の別トークン)では一致しない(セッション経由は §2 どおり auth_method を payload に写し `actor_api_token_id` は持たない)。拒否された指定失効(一様 404・403)は記録しない(実際に消えた行と 1:1)
- 同名トークンのローテーション(AUTH_SPEC §6 の置換)は `auth.token_created` 1 行で表し、置換された旧行の削除を独立の `auth.token_revoked` にしない(発行の意味論が「置換」であり、明示失効と区別する)(2026-08-10)。ただしその 1 行は**置換の対象**を運ぶ: 消えた行の maruhi 発行トークン id を `replacedTokenId` として payload に載せる(2026-08-24 deepsec R6 対応。新規発行ではキー自体が現れない = 置換なし)。device 交換の `tokenName` は呼び出し側が選べるため、GitHub identity を奪った第三者が被害者の既存トークン名を指定して**そのトークンを黙って無効化**できる — 発行行だけでは「なぜ動かなくなったか」がログから再構成できない。id は削除文より前に置いた監査追記の SQL 内から拾う(先行 SELECT を足すと、読みと batch の間の並行ローテーションで実際に消えた行と食い違う)
...
| `var.created` ★ | variable_id, environment_id, 変数名スナップショット, **actor_key_fingerprint** | 同上 + 同梱 version 1 の writer 署名(CRYPTO_SPEC §4.1)。**declared 作成(値なし — AUTH_SPEC §12-5。2026-08-30)も本イベント**(存在区間の開始 = metaVersion 1 の受理という意味論を保つ): 値署名が存在しないため actor_key_fingerprint はステートメント署名の author 鍵 FP のみを写し、activation の値 push は `var.version_pushed`(version 1)として記録される。値が 1 バージョンも存在しない区間は要ローテーション検出(§4)に対して空虚(読める値がない)であり検出を誤らせない |
...
| `var.renamed` | variable_id, environment_id, 新名スナップショット, **actor_key_fingerprint** | メタステートメントの author 鍵 FP を写す(2026-08-03)。**名前が実際に変わった再発行のみ**(2026-09-01 — 名前不変の再発行は `var.schema_reissued`。分岐は受理時の直前ステートメント名との byte 比較)。名前とスキーマ欄が同時に変わる再発行も本イベント 1 行(名前変更が主事象 — 名前は監査の主要用途〔名前履歴・§4 の存在区間〕の主キーであり、1 操作 1 行の記録規律を保つ) |
| `var.schema_reissued` | variable_id, environment_id, 変数名スナップショット(不変), **actor_key_fingerprint** | **名前不変のメタステートメント再発行**(2026-09-01 新設 — 実質はスキーマ欄の設定・変更: 再発行で name 以外に変えられる欄はスキーマ欄のみ — AUTH_SPEC §12-5)。**発火条件は名前の不変のみで、スキーマ欄が実際に変わったかを問わない**(無変更の再発行・スキーマ欄を持たない v1 → v1 の同名再発行も本イベント — 受理面は内容の変化を検査せず〔CAS・署名・ポリシーのみ〕、差分検査の追加は無変更再発行のための第 3 イベントか記録の欠落を要求する。受理拒否〔422〕にもしない: 有効署名の書き込みを内容で落とすのは帰属証跡を持つ操作の黙殺と同方向で、無害な no-op は記録して残す側が監査の性格に合う)。メタステートメントの author 鍵 FP を写す。スキーマ欄の**内容**(型・必須・description)は payload に写さない(ログ経由の注入面・肥大を作らない — 変更内容の正はステートメント履歴)。**過去行の注記**: 2026-09-01 の本改訂以前のサーバーはこの操作を `var.renamed` として記録していた(監査は append-only であり過去行は改訂しない — 当該期間の `var.renamed` は名前不変の再発行を含みうる) |
| `var.deleted` ★ | variable_id, environment_id, **actor_key_fingerprint** | 削除しても過去の閲覧可能性は消えない(§4)。削除ステートメントの author 鍵 FP を写す(2026-08-03) |
| `var.read` ★ | environment_id, payload = { variables: [{ variableId, epoch, version }, …] } | **暗号文の配布**に対して記録する(pull / Web での取得)。**値付き一括 pull ごとに環境単位 1 行**(2026-09-02 改訂 — 集約形。旧: 変数ごとに 1 行。variable_id / epoch / version 列は NULL で、返した変数の列挙を payload に持つ — 下記の記録細則)。**メタデータのみモード(AUTH_SPEC §12-7)は暗号文を配布しないため記録しない**(読んでいないものを読んだと記録しない — 2026-08-10) |
...
| `rotation.recommended` | target_user_id(remove / 降格 / **縮小** 変種)/ target_key_fingerprint(revoke_server 変種), variable_id, environment_id, payload = { basis, triggerChainSeq, **trigger** } | §4 の算出結果の永続化(UI / CLI 表示用)。**1 (variable × environment) 1 行**(2026-08-15 明確化 — §4.2 Q5 の索引要件から。集合を 1 行に畳まない)。**`trigger` = `remove_member` \| `change_role`(降格・縮小 — 2026-09-14 ES)\| `revoke_device`(2026-09-19 DK — §4.1 の変種)\| `revoke_server`**。四眼経由の適用は完成した `approve` エントリの seq を `triggerChainSeq` に持つ(PF1) |
| `rotation.dismissed` | variable_id, environment_id | 人間による明示的な取り下げ(append-only の打ち消しイベント)。1 対 1 行 |
| `project.schema_policy_changed` | payload = { 旧値, 新値 } | プロジェクト設定 `schemaPolicy` の変更(AUTH_SPEC §12-11。2026-08-30)。actor は変更した本人(type=user — 署名を伴わない設定操作のため FP は持たない)。設定値自体は pull 応答で全メンバーへ advisory 配布されるためクラス 1 |

- **`var.read` の集約形(2026-09-02 オーナー決定 — 未決 4 の解消。ドッグフーディング実測を待たず前倒し — docs/notes/hosted-design.md §3-3)**: CI からの定期 pull で最も高頻度になるイベントであり、旧形(1 変数 1 行)では行数が「pull 回数 × 変数数」で増えた。記録単位を**値付き一括 pull 1 回(環境単位)= 1 行**に改める:
  - **列**: environment_id(非 NULL)。variable_id / epoch / version は NULL(変数粒度の座標は payload が持つ)。actor は従来どおり読み取った主体(type=user — 署名を伴わないため FP は持たない)
  - **payload** = `{ variables: [ { variableId, epoch, version }, … ] }` — 応答に暗号文を含めて返した変数の**全列挙**。epoch / version は返した最新バージョンの座標(要ローテーション検出の変数粒度の照合と、インシデント対応〔どのバージョンを取得したか〕の材料)。列挙は `variableId` の**昇順(コード単位比較)・重複なし**(1 pull は各アクティブ変数を高々 1 回返す — 同一 pull 内で同じ変数が 2 回現れる形は存在しない)、要素のキー順は `variableId` → `epoch` → `version` に固定する(保存バイト列が row_digest の入力 — §5.1。書き手はサーバーのみで、この固定は再計算〔`maruhi audit reconcile` の parse → stringify ラウンドトリップ〕の安定性のため)
  - **返した変数が 0 の pull は行を書かない**(暗号文を配布していない — 記録条件「暗号文を応答に含めて返したこと」は不変。旧形でも 0 行だった)
  - **行を分割しない**: 1 行あたりの payload は変数数 × 数十バイト(1 環境の変数上限 1,000〔AUTH_SPEC §12-8〕で数十 KB — §5.3)であり、行サイズ上限(2 MB)の内側。1 pull を N 変数ごとの複数行に割る形は棄却する — 全読み手に「同一 pull の複数行」の再結合を要求し、Q4 の payload 検査量も減らない。配布(§7)の 1 ページの大きさは `limit × payload` でクライアントが `limit` で制御する
  - **要ローテーション検出の入力を欠損させない**(裁定 CZ の線引き — docs/notes/session-47.md。集約は密度の対策であり行上限ではない): 「誰がどの変数のどのバージョンを読んだか」は payload から完全に復元できる — §3.5 `server.lease_issued` の環境単位行(発行時点のアクティブ変数を導出させる)より強く、変数 ID を明示的に持つ。§4.1 手順 3 の (a) は集約行の列挙を展開して判定する(旧形と同値 — 実装 PR がテストで固定)
  - **旧形との混在**: 本改訂以前のサーバーが書いた 1 変数 1 行の行(variable_id 非 NULL・payload に `variables` なし)は書き換えず・バックフィルもしない(§1-4)。同一テーブルに旧形と集約形が混在し、判別は **`variable_id IS NULL`(集約形)/ 非 NULL(旧形)**。全読み手 — §4 の検出、§7 の API フィルタ(`variable_id` — Q4)、Web、CLI(`maruhi audit list` / `verify` / `reconcile`)、監査ヘッド — が両形を扱う
  - **可視性はクラス 2 のまま**(§6 — 本人閲覧〔actor 本人〕の判定は actor 列で変わらない)。**監査ヘッド**(§5.1)は集約行を集約行のまま digest する(既存の累積ハッシュ列・チェックポイント公証の意味論に影響しない)
  - **メタデータのみ pull は引き続き記録しない・リースは変更なし**(`server.lease_issued` は既に環境単位 1 行 — §3.5)
  - 密度(workerd 実測 — 実装 PR のテスト `audit-read-aggregate.test.ts`。識別子は CLI の実発行形 25 文字・行 + 索引): 旧形は 1 変数読み取りにつき約 300 バイト。集約形は 1 pull につき約 250 バイト + 変数数 × 約 80 バイト(識別子長に比例して増える)。100 変数の環境を 1 回 pull → 旧 100 行 / 約 30 KB、集約形 1 行 / 約 8.3 KB(行数 1/100・バイト約 1/3.6)。AUTH_SPEC §12-8 の「余裕の会計」(i) はこの値で更新する
...
- **actor_key_fingerprint を持つデータ系イベント(2026-08-03 セッション 12 改訂)**: データ系イベントの actor は原則 user_id(+ トークン id / auth_method)のみで鍵 FP を持たない(FP を持つのはチェーンミラー §3.4)としてきたが、この例外は「**クライアント署名を伴う操作**」の類型として一般化する — `dek.registered`(登録署名 = CRYPTO_SPEC §5.1。2026-08-02 セッション 09)に加え、`var.version_pushed` / `var.created`(値の書き込み署名 = 同 §4.1)、`var.renamed` / `var.schema_reissued` / `var.deleted` / `env.created` / `env.renamed` / `env.deleted`(メタステートメント署名 = 同 §4.2)が署名者鍵 FP を actor_key_fingerprint 列に写す。監査行(サーバー管理データ)とチェーン外署名(クライアント署名 = サーバーが偽造できない)を突合可能にするための記録であり、§2 のアクターモデル(type=user の key_fingerprint)の範囲内である。`dek.deleted` は署名を伴わない操作のため引き続き FP を持たない(この非対称は「FP = 署名の証跡」の意味論を保つためであり、均しにいかない)
- `dek.registered` / `dek.deleted` の粒度は **1 受信者 1 行**(2026-08-02 セッション 08 提案): §5.1 の列構造は 1 行 1 target(target_user_id は単値)であり、受信者ごとの行にすることで「この受信者宛のラップがいつ登録・削除されたか」を索引(target_user_id, seq)でそのまま引ける。登録はローテーション・メンバー追加時のみの低頻度イベントで、行数は AUTH_SPEC §12-8 のラップ行数上限が束縛する。要ローテーション検出(§4.1)には関与しない(候補集合はチェーンミラーの scope から導出する環境別アクセス窓 — 2026-09-14 ES。旧: 全メンバー × 全環境)が、「誰がどのエポックの DEK を受け取ったか」の証跡を確保する
...
| **`chain.device_revoked`** ★ | `revoke_device`(target_user_id = 対象、payload = { deviceKeyFingerprints }。**§4.1 の検出契機になる**ため ★。2026-09-19 DK) |
...
- **端末の追加・失効(2026-09-19 DK)**: 1 エントリ 1 行(全単射不変)。`chain.device_added` は検出の契機ではないが、§4.1 の端末の窓の開始点として Q1 が読む
...
2. **候補集合(閲覧可能だった集合)**: **環境別のアクセス窓(2026-09-14 ES — 旧: v1 は全メンバーが全環境・全エポックの DEK を受け取るため「在籍区間と存在期間が重なる全 (variable × environment)」)**: M の各在籍区間について、環境 E のアクセス窓 = 「区間内で E が M の scope に含まれていた seq 範囲」(`chain.member_added` / `chain.role_changed` の payload の scope から復元 — `all` は在籍区間の全体、`listed` は含まれる間。scope の変化点で窓が開閉する)。候補 = 「アクセス窓と存在期間が重なる全 (variable × environment)」。`var.created` 〜 `var.deleted`(未削除なら現在まで)の存在区間との重なりで判定する。**削除済み変数も含める**(上流 credential は変数を消しても失効しない)。revoke_server 変種の「環境ごとの開示窓」(下記)と同じ構造であり、実装は 1 つの窓導出を共有する
...
**`change_role` の変種(2026-09-14 ES)**: 降格(member 未満へ)と scope の縮小は `remove_member` と同じ骨格で検出する — 手順 1 の区間は当該 `change_role` の seq で閉じる窓(降格 = 対象 scope の全環境の窓を閉じる、縮小 = 縮小分の環境の窓を閉じる)、手順 2 の候補は閉じた窓の環境に限る、手順 3〜5 は同じ。`rotation.recommended` の `trigger = change_role`。**降格で機密性上の失効は起きない**(対象は reader として新 DEK を受け取り続ける — CRYPTO_SPEC §7)が、閉じた窓の値は「上流の credential を知る者が権限を失った」事実として検出の対象にする(取り下げは admin の判断 — §7)。四眼経由の適用(PF1)は完成した `approve` エントリの seq を起点にする(`triggerChainSeq`)

**`revoke_device` の変種(2026-09-19 DK)**: 同じ骨格で次を差し替える — 手順 1 の区間は失効した各端末の**有効区間**(`chain.device_added`〔または `add_member` / `genesis` の最初の鍵は在籍区間の開始〕〜 `chain.device_revoked`)と、対象者の環境別アクセス窓(手順 2)および端末の scope(`chain.device_added` の payload)の**共通部分**。手順 3 の (a) は remove の変種と同じ **actor.user_id で照合**し、区間だけを上記(端末の有効区間 ∩ アクセス窓 ∩ 端末 scope)で絞る(**`var.read` は署名を伴わない読み取りであり `actor_key_fingerprint` を持たない** — §3.3 の類型「クライアント署名を伴う操作」の外・§5.2 の部分索引の前提。K1 レビュー〔2026-09-20 pullfrog〕で、起草時の「actor 鍵 FP で照合し remove より精密」を訂正 — 設計録 dk-design.md §6 K1-12。端末単位の精度は区間の絞り込みが与え、同じ人の別端末の読み取りは (a) に混入しうる〔過大申告の方向 = 安全側〕。端末登録簿の `tokenId`〔AUTH_SPEC §13-11 — advisory〕を `actor_api_token_id` と突合して (a) を端末に寄せる案は、advisory を根拠ランクの入力にする裁定を要するため採らない — 2026-09-20 所有者裁定〔設計録 §6 K1-12・所有者裁定〕で確定。将来採るなら承認項目 8 の反転として改訂する)。手順 4〜5 は同じ。`rotation.recommended` の `trigger = revoke_device`、payload に対象 user_id と失効 FP 集合。対象者は在籍を続けるため在籍区間は閉じない(降格の変種と同じく「契機 seq で切った窓」で検出する)。scope が空の端末(票だけの端末)の失効は候補が空になり行を書かない

~~環境スコープ role(CRYPTO_SPEC 未決事項 #11)が入った場合は、手順 2 の「全環境」が「M がアクセス権を持っていた環境」に狭まる。チェーンミラーが role / スコープを写しているため、この拡張はクエリの変更だけで成立する(スキーマ変更不要)。~~ **2026-09-14 解消(上記の環境別アクセス窓)**。チェーンミラーが scope を写す(§3.4)ため、クエリの変更だけで成立する(スキーマ変更不要 — 不変)

### 4.2 スキーマが満たすべきクエリ要件

| # | クエリ | 必要な索引 |
|---|---|---|
| Q1 | user_id → 在籍区間と環境別アクセス窓(chain.genesis / member_added / role_changed / member_removed の列。member_added / role_changed の payload の scope が窓の開閉点 — §4.1 手順 2。2026-09-15 K3 訂正: 列挙に role_changed を追加)、および端末の窓(chain.device_added / device_revoked の列 — payload の FP と cap が端末の窓の開閉点。§4.1 の `revoke_device` 変種。2026-09-19 DK。`chain.device_added` の target は actor 自身) | (target_user_id, seq)(不変) |
| Q2 | (variable × environment) の存在区間(var.created / deleted の列) | (variable_id, environment_id, seq) |
| Q3 | user_id × 期間 → 読んだ (variable × environment) の distinct 集合 | (actor_user_id, seq) + イベント種別。**集約形の `var.read`(§3.3 — 2026-09-02)は payload の `variables` 列挙を展開して (variable × environment) を得る**(旧形の列値との和集合。索引は actor 単位の行に効き、行数は集約で 1/変数数に減る) |
| Q4 | (variable × environment) × 期間 → 閲覧・変更した主体一覧(逆引き。インシデント対応用) | Q2 と同じ索引。**集約形の `var.read` は変数 ID を列に持たない**ため、環境単位の行(variable_id IS NULL・event = var.read)を**値の存在区間(その環境での最初の `var.version_pushed` 〜 最後の `var.deleted`)**で seq 範囲に絞ってから payload の `variables` を検査する(値付き pull は環境の全アクティブ変数を返すため、区間内の集約行はほぼ全て当該変数を含み、検査は「ページ上限 × payload 長」で止まる — 区間の外では 1 行も検査しない。下限を `var.created` でなく値の初出に取るのは、値を一度も持たない declared 変数が pull に現れないため — その場合は検査自体を省く)。**変数 ID はクライアント発行でサーバーの一意性は (環境, 変数) 単位**のため、区間は環境ごとに取りその和で束縛する(環境 A で削除済み・B で存命の同一 ID を取りこぼさない。環境フィルタがあればその環境の区間のみ)。§7 の `variable_id` フィルタも同じ形(旧形の列一致との和集合) |
...
- 本節が規定するのは索引の**集合と列順**であり、索引の述語(部分索引化 — 例: `var.read` で常に NULL の `target_user_id` / `target_key_fingerprint` / `actor_key_fingerprint` の索引を `WHERE <列> IS NOT NULL` に限る)は実装裁量(2026-09-02 — 監査ログの成長密度対策 ①。上の DDL は当初形のまま)

- 頻出属性は列に昇格(索引のため)、それ以外は payload JSON。列は NULL 許容とし、イベント種別ごとの必須属性はアプリ層(Effect Schema)で強制する
- **監査ヘッド累積ハッシュ(2026-08-18 セッション 27 起草 — 未決 #2 の解消。CRYPTO_SPEC §6.2 `checkpoint` の入力)**: プロジェクト DO は監査行の追記時に累積ハッシュを維持する — `h_n = lower_hex(SHA-256(LP("maruhi/v1/audit-head", h_{n-1}, seq, row_digest)))`(`h_0` = 空文字列。LP は CRYPTO_SPEC §2.1)。`row_digest` は行の列を固定順(seq, row_id, server_ts, client_ts, event, actor_type, actor_user_id, actor_key_fingerprint, actor_api_token_id, target_user_id, target_key_fingerprint, environment_id, variable_id, epoch, version, chain_seq, payload — 数値は 10 進文字列化、payload は保存された TEXT のバイト列そのまま)で LP 化した SHA-256。**NULL 許容列の LP フィールドはタグ付きバイト列とする: NULL = 1 バイト `0x00`、非 NULL = `0x01` + 値のバイト列**(NULL と空文字列を同一プリイメージにしない — 2026-08-18 pullfrog レビュー対応。LP の長さプレフィックスがタグ込みの境界を確定するため曖昧性はない)。**JSON の正規化は行わない**(保存バイト列を正とする — 正規化差異による照合分裂を構造的に避ける)。**`h_{n-1}` と `row_digest` はともに hex 小文字文字列として LP フィールドに載せる**(`h_0` = 空文字列の規定と整合する一様表現 — `test-vectors/audit-head.json` が固定する。2026-08-28 セッション 35 の明文化)。サーバーは任意の受理済み時点の累積ハッシュを検証可能に保持し(実装形は行への列併置等の実装詳細)、チェックポイントの受理検証(CRYPTO_SPEC §6.4 — 申告ハッシュが計算列に存在すること)と admin の突合(§6)に用いる。追記と累積ハッシュの更新は同一トランザクション — **または、累積ハッシュ列を同一トランザクションで追記された行からの決定論的導出値として保持し、遅延で materialize してよい**(2026-08-28 セッション 35 の追補): その場合、監査ヘッドを読むすべての経路(AUTH_SPEC §16-2 の取得・checkpoint 受理検証)は読み取りの前に列を最新行まで伸ばさなければならず、途中失敗は接頭辞連続な列を残して次の伸長が再開すること(行が真実源・列は導出キャッシュであり、どの読み手も不完全な列を観測しない — 同一トランザクション形と観測上等価。SHA-256 が非同期な実行環境で、追記ホットパス〔var.read の一括記録等〕へハッシュ計算を持ち込まないための許容。裁定の経緯は docs/notes/session-35.md)。**遅延実体化の伸長は 1 呼び出しあたり有界にしてよい(2026-08-28 セッション 38 の追補 — PR #99 レビュー申し送りの解消)**: 上限到達 = 列が MAX(seq) 未到達の呼び出しは、当該読み取り・受理を retryable な型付き応答(AUTH_SPEC §16-2 の `AuditHeadNotReady`)で拒否し、**古い列でチェックポイント受理検証(CRYPTO_SPEC §6.4 の存在・位置検査)や監査ヘッドの応答を行ってはならない**(fail-closed — 検査・応答は列が MAX(seq) に到達した呼び出しでのみ行う)。伸長の進捗は呼び出し間で保存され、失敗応答を含む各呼び出しが必ず前進して収束すること。導入マイグレーションは既存行(append-only で全行残存)から累積ハッシュを再計算して初期化する(遅延 materialize 形では最初の読み取りがこの初期化を兼ねる) — 初期化以前に行われた改竄は検出対象外である(チェックポイントの保証は「公証時点以降の事後改竄」に対するもの — §6 — であり、初期化はその起点を作るだけ)。**D1 側(§5.2 の user / org イベント)は対象外**: チェックポイントの置き場はプロジェクトチェーンであり、プロジェクトに属さない行を載せる場所がない(D1 側の改竄耐性は §6 の従来モデルのまま)
...
- v1 は無期限保持(削除 API を作らない)。DO SQLite は 10 GB / DO まであり、支配的な `var.read` は集約形(§3.3 — 2026-09-02)で密度〔行数の増え方〕を下げ、総量は AUTH_SPEC §12-8 の DO ストレージ総量ガードが受ける。集約行の payload は変数数 × 数十バイト(1 環境の変数上限 1,000 で約 80 KB — 行サイズ上限 2 MB の内側。row_digest〔§5.1〕の SHA-256 入力もこの長さで、追記ホットパスには乗らない〔遅延実体化〕)。監査行の上限・保持期限・削除は置かない(裁定 CZ の棄却案 — docs/notes/session-47.md)
...
- 監査ログは**サーバー管理データであり、チェーンと違って暗号学的な改竄不能性はない**(サーバー = セルフホスト運営者は書き換え能力を持つ)。v1 の脅威モデルでは許容し、次で緩和する:
  ...
  - **監査ヘッドのチェーンへのチェックポイント(2026-08-18 — 未決 #2 の解消。CRYPTO_SPEC §6.2 `checkpoint` / §5.1 の累積ハッシュ)**: **実効権限 admin** のクライアントがチェックポイント発行時にサーバー申告の累積ハッシュをチェーンへ公証する(checkpoint 自体の発行権限は member 以上だが、監査ヘッドの公証は申告取得の実効権限 admin 限定 — AUTH_SPEC §16-2 — に従う。それ以外の発行は公証なし = 空文字列)。
    - **公証済み接頭辞の前進は admin クライアントの発行頻度に依存する(明示的な残余 — 2026-08-18 レビュー第 5 ラウンド)**: member の発行はデータ層のみを公証し、監査接頭辞を前進させない。CRYPTO_SPEC §6.3 の発行契機 (iii) は admin クライアントの基準を「最新の公証ありチェックポイント」に分離してこの前進を admin の同期に連動させるが、admin が長期間発行しないプロジェクトでは、最後の公証時点以降の監査行の事後改竄は突合で検出できない(記録時点の虚偽と同様、本節の脅威モデルの範囲内の残余)。
    - **意味論は「発行時未検証の公証」**: 発行者は全行を閲覧できるが、checkpoint ごとの全行取得・累積ハッシュ再計算は重い。そこで公証時にはサーバー申告値を固定し、完全な検証を後続の明示的な admin 突合へ分離する。サーバーが「この時点でこの累積ハッシュだった」と主張した事実が署名済み・チェーン上に固定され、以後に公証済み接頭辞の行を改竄・削除すると、admin の再計算と矛盾する(否認不能)。
    - **admin の突合は「所属」に加えて「位置」を検査する(2026-08-18 pullfrog レビュー対応 — 陳腐化リプレイの遮断)**: 累積ハッシュ列を再計算しながら、監査ヘッドを公証した(audit_head_hash が非空の)各チェックポイントについて、(a) 公証ヘッドが列に現れること、(b) その出現位置が公証チェックポイント間で非後退であること、(c) 出現位置が、直前のチェックポイント(公証の有無を問わない — ミラー行はすべての checkpoint 受理が書く)自身のミラー行(`chain.checkpointed` — chain_seq で同定)以上であることを検査する。直前のチェックポイントが存在しない初回は (c) を課さない(空虚に真。受理検査 — CRYPTO_SPEC §6.4 — と同一述語・同一の基底ケース)。
    - 所属検査だけでは、悪意サーバーが実在する古い累積ハッシュ h_k を返し続けることで、k 行目以降を改竄可能なまま全チェックポイントを通過できる。(c) により保護接頭辞は単調に前進する。同じ位置下限をサーバー受理時にも検査するため、正直なサーバーでは (b)(c) が構造的に成立する。したがって突合での (b)(c) 違反は良性競合ではなく、「受理ポリシーを執行しないサーバー」= 陳腐化リプレイを可能にする状態の証拠である(所属違反 = 行の改竄そのものの証拠とは区別して報告する)。
    - チェックポイント追記自体がミラー行を書くため、この検査はチェーン・ワイヤに seq を追加せず(C1 裁定の件数非漏洩と両立)、全行を読める admin 側だけで完結する。**記録時点の虚偽(最初から偽の行を書く・書かない)は引き続き非保証**。
...
- **閲覧権限(2026-08-12 改訂 — 旧 v1 暫定案を置換。未決 #1 の解消)**: イベントを 2 つの可視性クラスに分ける。線引きの原則は「**人の行動の監視情報か、開示機構の作動か**」:
  - **クラス 1(チェーン role reader 以上 = 全メンバー)**: クライアント同期で既に配布・検証される事実、および開示機構の作動 — `chain.*` 全部、**`chain_seq` を持つ任意の行**、`env.*`、`var.created` / `var.renamed` / `var.schema_reissued` / `var.deleted` / `var.version_pushed`、`server.*`、`rotation.recommended` / `rotation.dismissed`。チェーンミラーを admin に絞っても全メンバーはチェーン同期で同じ事実を検証・取得済みであり、絞りは見せかけの防御にしかならない。`chain_seq` の追加条件(2026-08-25 deepsec S1)は tamper evidence を可視性述語で隠さないため: 追記境界が `chain.*` 以外の `chain_seq` を defect として拒否し、正直な書き手でこの列を設定できるのは `chainMirrorEvent` = `chain.*` ミラーだけなので正常なクラス 2 行を開示せず、イベント名を名前空間の 1 歩外へ変えた偽 provenance claim だけを全メンバーの検証へ届ける。`server.*` は人ではなく開示機構の記録であり、自分の秘密の開示行使を知る正当な利害が全メンバー(reader 含む)にある(CRYPTO_SPEC §9 の常時明示義務の監査面)
...
- **event 名前空間の前置フィルタ(2026-08-24 deepsec R1 対応)**: 完全一致の `event` フィルタに加え、`event_prefix`(前置一致)を語彙に含める。`maruhi audit verify` のミラー全単射検証(§3.4)が **`chain.` 名前空間の全行**を取得できるようにするためで、既知のミラーイベント名を 1 つずつ完全一致で引く形では**集合外の `chain.*` 行を名乗る偽造行が 1 度も取得されず**、検証が「OK」で終わってしまう(偽造方向の被覆漏れ)。前置一致は SQL の LIKE ではなく `substr(event, 1, ?) = ?` で実装し、ワイルドカード意味論を持たせない。可視性クラス(§6)・カーソル・件数非漏洩の扱いは他のフィルタと同一。クライアントは取得した `chain.*` 行のうち既知のミラー写像に無いイベント名を**検証失敗として扱う**(不明な op を名乗る行 = 偽造の証拠)。この性質は §6 のクラス 1 判定が**名前空間を前置で覆っている**ことに依存する: 写像済みの名前だけを許す実装だと、偽造行がサーバー側の可視性述語で落ち、admin 未満の verify には 1 行も届かない(verify は admin を要求しないため、reader が実行すると「OK」で終わる)。クラス 1 の判定と本フィルタは同じ前置比較を共有すること
- **chain_seq 存在フィルタ(2026-08-25 deepsec S1 対応)**: `chain_seq_present=true` を語彙に含め、NULL でない行だけを返す。`maruhi audit verify` は `event_prefix=chain.` の集合と本フィルタの集合を row_id で和集合にして検査する。前者だけでは `member.add` / `chainx.grant` のように名前空間の 1 歩外で `chain_seq` を名乗る行が一度も取得されず、一覧では素の座標に見え、verify も「OK」で終わるため。正当なミラー行は両集合に入るので重複排除し、同じ row_id の内容が 2 クエリ間で変わったらサーバー応答の自己矛盾として中止する。§6 の可視性述語も `chain_seq IS NOT NULL` をクラス 1 とし、非 admin の verify から tamper evidence を隠さない。クライアントは `chain_seq` を持つ非 `chain.*` 行を明示的な不信ラベル + 整合性違反として扱う
...
4. ~~`var.read` の集約方針(ドッグフーディングの実測後。§3.3 / §5.3)~~ **解消(2026-09-02 オーナー決定 — 実測を待たず前倒し。§3.3 の集約形 = 値付き一括 pull ごとに環境単位 1 行、payload に返した変数の列挙。本改訂を含む実装 PR のマージをもって確定)**
