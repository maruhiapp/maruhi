...
- **`var.read` の集約形(2026-09-02 オーナー決定 — 未決 4 の解消。ドッグフーディング実測を待たず前倒し — docs/notes/hosted-design.md §3-3)**: CI からの定期 pull で最も高頻度になるイベントであり、旧形(1 変数 1 行)では行数が「pull 回数 × 変数数」で増えた。記録単位を**値付き一括 pull 1 回(環境単位)= 1 行**に改める:
  ...
  - **返した変数が 0 の pull は行を書かない**(暗号文を配布していない — 記録条件「暗号文を応答に含めて返したこと」は不変)
  ...
  - **要ローテーション検出の入力を欠損させない**(裁定 CZ の線引き — docs/notes/session-47.md。集約は密度の対策であり行上限ではない): 「誰がどの変数のどのバージョンを読んだか」は payload から完全に復元できる — §3.5 `server.lease_issued` の環境単位行(発行時点のアクティブ変数を導出させる)より強く、変数 ID を明示的に持つ。§4.1 手順 3 の (a) は集約行の列挙を展開して判定する(実装 PR がテストで固定)
