...
| `auth.key_wrap_registered` | kind(`passkey-prf` / `guardian`)、wrapId / groupId、mode、recipientCount | master 鍵ラップ台帳への登録(AUTH_SPEC §13-7 — 2026-09-12 KL3)。actor = ward |
...
| # | クエリ | 必要な索引 |
|---|---|---|
| Q1 | user_id → 在籍区間と環境別アクセス窓(chain.genesis / member_added / role_changed / member_removed の列。member_added / role_changed の payload の scope が窓の開閉点 — §4.1 手順 2。2026-09-15 K3 訂正: 列挙に role_changed を追加) | (target_user_id, seq) |
