...
- **`var.read` の集約形(2026-09-02 オーナー決定 — 未決 4 の解消。ドッグフーディング実測を待たず前倒し — docs/notes/hosted-design.md §3-3)**: CI からの定期 pull で最も高頻度になるイベントであり、旧形(1 変数 1 行)では行数が「pull 回数 × 変数数」で増えた。記録単位を**値付き一括 pull 1 回(環境単位)= 1 行**に改める:
  ...
  - 密度(workerd 実測 — 実装 PR のテスト `audit-read-aggregate.test.ts`。識別子長 8 文字・行 + 索引): 旧形は 1 変数読み取りにつき約 260 バイト。集約形は 1 pull につき約 260 バイト + 変数数 × 約 50 バイト(識別子長に比例して増える)。100 変数の環境を 1 回 pull → 旧 100 行 / 約 26 KB、集約形 1 行 / 約 5.2 KB(行数 1/100・バイト 1/5)。AUTH_SPEC §12-8 の「余裕の会計」(i) はこの値で更新する
...
- v1 は無期限保持(削除 API を作らない)。DO SQLite は 10 GB / DO まであり、支配的な `var.read` は集約形(§3.3 — 2026-09-02)で密度〔行数の増え方〕を下げ、総量は AUTH_SPEC §12-8 の DO ストレージ総量ガードが受ける。集約行の payload は変数数 × 数十バイト(1 環境の変数上限 1,000 で約 50〜70 KB — 行サイズ上限 2 MB の内側。row_digest〔§5.1〕の SHA-256 入力もこの長さで、追記ホットパスには乗らない〔遅延実体化〕)。監査行の上限・保持期限・削除は置かない(裁定 CZ の棄却案 — docs/notes/session-47.md)
