...
- **ラップ対象 B**: 予備鍵(enc / sig)の不透明ブロブ(2026-09-19 DK — 旧: user master 秘密鍵)。直列化形式はクライアント(CLI)の契約であり、サーバーは関知しない(現行 = キーチェーンレコードの JSON。端末鍵のレコードの欄に加えて、クライアントが予備鍵として生成した B は `"kind": "reserve"` を持つ(2026-09-26 K16 — 生成の事実の記録。AEAD の内側にあるのでサーバーは作れず・消せず・付け替えられない。クライアントはこれを持たない B — DK 以前の端末鍵の複製など — を予備鍵として記録・失効させず、予備鍵を分離する。端末の保存先〔キーチェーン〕はこの欄を持つレコードを受け付けない。暗号操作・AAD・ワイヤ形式は不変。設計録は docs/notes/dk-design.md §21)。復元側は自己検証を通してから、**端末鍵の発行にだけ用い、日常の保存先には置かない**)
