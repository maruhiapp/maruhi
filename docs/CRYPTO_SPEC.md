...
- **ラップ対象 B**: 予備鍵(enc / sig)の不透明ブロブ(2026-09-19 DK — 旧: user master 秘密鍵)。直列化形式はクライアント(CLI)の契約であり、サーバーは関知しない(現行 = キーチェーンレコードの JSON。端末鍵のレコードと同じ形。復元側は自己検証を通してから、**端末鍵の発行にだけ用い、日常の保存先には置かない**)
