...
- **明示確認の充足形(2026-09-12 改訂 — 検証済み指紋帳の解釈の規範化。所有者委任による裁定)**: 前項の「帯域外照合の明示確認」は、招待者側・受諾者側とも次のいずれかで充足する:
  1. **読み上げ儀式(既定)**: その実行で FP ワード列を帯域外(通話等)で照合し、最終語を再入力する
  2. **フラグによる機械照合**(`--expect-fingerprint` / `--inviter-fingerprint`): 帯域外で控えた FP の実行時の明示指定。この受諾 1 件への明示的作為であるため、帯域外の記録を照合済みとして扱う(非対話環境で許される唯一の形)
  ...
  - **帳のヒットのみによる無確認の自動通過(完全 auto-pass)は認めない**: 招待リンクは無記名(bearer)で受諾者の同一性を運ばず、帳は過去の別文脈の検証記録にすぎないため、この付与 / 受諾への人間の同意を代替できない。儀式そのものの既定廃止は、受諾のリンク束縛等による本節の改訂(ROADMAP の IV1 / IV2)としてのみ行う
