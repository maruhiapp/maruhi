...
- **明示的な非保証: 有効期間内の OIDC トークンのリプレイ(2026-08-15 追記)**。`claims_digest` が束縛するのは issuer / subject / audience であり、ワークロードの一時公開鍵も nonce も含まない。したがって**まだ有効な OIDC トークンのコピーを入手した者**(ネットワーク捕捉・悪意あるワークフローステップ・ログ流出)は、**自分の一時鍵で**リースを要求し、正当に再ラップされた DEK を受け取れる — サーバーに「正規のジョブ」と「トークンのコピー保持者」を区別する手段はない。露出窓はトークンの `exp - iat`(GitHub Actions では数分)。本節が保証するのは「リース応答を**別のワークロード同一性へ転用できない**」ことであって、同一同一性でのベアラーリプレイの防止ではない。緩和には所持証明が要る(要求する `aud` に一時公開鍵のハッシュを混ぜる / サーバー発行 nonce の 2 往復)。いずれもワイヤ形とポリシー照合の意味論に影響するため、**A3(CI クライアント実装)の前に裁定する**(2026-08-15 の pullfrog レビューで明示化。§14.3 の非保証一覧と同じ性格の記載)
