...
> **クローズ(2026-08-25 再検証)**: 修正後のコードに deepsec を通し、8 論点のうち
> **7 論点が `fixed` 判定**になった(R1 / R2 / R3 / R5 / R6 / R7+R8 / R9)。
> **R4 のみ true-positive のまま残る** — 文書に「残る限界」と書いた
> 「同一 `auth_method` 内での被覆」がそのまま指摘されている(severity は
> MEDIUM → BUG に下げられ、バケット化と抑制マーカーの効果自体は認められた)。
> run: `process 20260825002935-0e1bfd164a1a9165` / `revalidate 20260825003446-4a6d6d6964b0c376`
> (13 ファイル再調査、$11.44)。
>
> この再検証で**新規 4 件**の true-positive が出た。R4 の残存分と併せて
> [`DEEPSEC_FINDINGS_2026-08-25.md`](./DEEPSEC_FINDINGS_2026-08-25.md) で追跡する。
> うち 2 件(`chain_seq` の無ラベル表示・`MARUHI_` prefix の欠落)は本 PR の
> R1 / R3 の**すぐ隣**で、同じ回避が名前空間の 1 歩外に残っていた形である。
