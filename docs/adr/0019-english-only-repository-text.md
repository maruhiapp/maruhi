...
2. **Exceptions are enumerated, not discretionary.** A file may contain non-English text only when:
   ...
   - it is a proper noun whose spelling is the holder's choice (e.g. the `しまぶー` copyright handle in `LICENSE` files), <!-- english-exempt: names the holder's handle verbatim -->
   - its path is listed in `scripts/english-exemptions.txt` (with the reason recorded there).
3. **Enforcement is mechanical, and additive-only during migration.** `bun run check:english` (`scripts/check-english.mjs`) scans the lines a PR *adds* for CJK text and fails the check pipeline first — a ratchet that permits no new Japanese but does not force an unrelated PR to translate the files it touches. Lines marked `english-exempt` and paths in the exemptions file are skipped. The final migration PR switches the gate to `--all`, which scans whole tracked files.
