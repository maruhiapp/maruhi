...
3. **内部の文書は日本語のままでよい**。コード内コメント、ADR、`docs/notes/`、`CRYPTO_SPEC` / `AUTH_SPEC` / `AUDIT_SPEC`、コミットメッセージ、PR の説明は対象外(CLAUDE.md のコーディング規約「コメント・ドキュメントは日本語可、公開 API の JSDoc は英語」を維持する)。**境界は「配布物からユーザーが読むか」**であって「ソースに書いてあるか」ではない
