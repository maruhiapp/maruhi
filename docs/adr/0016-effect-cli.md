...
7. **挙動の変更点**: 値の無い / 数として読めない number フラグは gunshi の内部エラー(exit 1)から `InvalidValue`(usage = exit 2)へ。`rotation dismiss --all=false` は「値を読まずに true」から「書いたとおり false」へ(12 形の #2)。先頭の空引数(`maruhi "" pull`)は読み飛ばしから root の `UnknownSubcommand`(exit 2)へ。`maruhi maruhi` はただの未知のコマンドになった
