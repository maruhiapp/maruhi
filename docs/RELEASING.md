...
- **GitHub Release の公開本文は英語**(ADR-0017 決定 1)。workflow は `gh release create --generate-notes` で PR タイトルから本文を組む(`--draft` なし = タグ push 時点で公開済み)。所有者は公開直後に Release 本文を英語へ直す(`--draft` 化や `--notes-file` 化、PR タイトルの英語化は別判断)
