...
maruhi の未認証面のうち、リクエストごとに**外部(GitHub)へのアウトバウンド
呼び出し**を誘発するのは次の 3 つ(ほかの未認証面 `/auth/config` /
`/auth/github/start` は自己完結の軽量応答)。いずれもサーバー側の防御
(入力サイズ上限・形式事前検査・プロジェクト単位の固定窓)を持つが、
**送信元 IP 単位**の制限は Cloudflare 側でしか掛けられない。ダッシュボードの
...
| `/auth/github/callback` | 10 リクエスト / 分 / IP | 同上(code 交換 + `/user` の最大 2 回)。code の中身はサーバーでは検証できないため、形式検査では遮断できない |
