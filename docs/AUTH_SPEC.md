...
| pending 提案(四眼 — CRYPTO_SPEC §6.2)| プロジェクトあたり 32(期限切れは数えない。`expires_at_ms` の上界 = 受理時サーバー時計 + 30 日 — CRYPTO_SPEC §6.4。2026-09-14 PF1) | 型付き 422 `ProposalLimit`。withdraw / 適用で解放 |
