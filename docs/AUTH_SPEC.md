...
1. **開始 `POST /auth/cli/start`(未認証)**: payload = `{ tokenName?, scopes?, expiresInDays? }`(発行パラメータの意味論は §6 — 旧 device 交換と同一。H1 のサインアップ招待コードは本 payload への添付として設計する — hosted-design.md §2-2)。応答 = `{ flowId, flowToken, userCode, verificationUrl, expiresInSeconds, pollIntervalSeconds }`
   - `flowId` は公開相関子(verificationUrl のクエリに現れる)だが、**推測不能な乱数**とする(起草値 128-bit — 列挙可能だと無関係の第三者が任意のフローの verify へ到達でき、束縛の先着〔§4-1 (4) (iii)〕や保護によるプール埋めの起点になる)。`flowToken` は 256-bit 乱数の bearer secret(ポーリングの資格情報)で、サーバーはハッシュのみ保存する(§5 / §15 と同じ規律)。**flowToken をブラウザチャネル(URL・承認ページ・リダイレクト)に決して載せない** — ブラウザ履歴・referrer が CLI の資格情報の集積になる経路を構造的に断つ(flowId / flowToken を二識別子に分離する理由)
...
3. **ブラウザ脚 `GET /auth/cli/verify?flow=<flowId>`**: サーバーは §3 の 1〜2 段(state 発行・検証、code 交換、ユーザー情報取得)と同じ web OAuth を実行する(state に flow 束縛を含める。GitHub の callback は §3 の単一 URL のまま)。§3 の 3 段目(get-or-create)は (4) の保護 CAS の**後**に行い、**4 段目(セッション発行 §5)は行わない** — CLI ログインの成果物は §4-1 (5) の PAT のみで、Web セッションという副作用を持たない(承認に不要な資格情報を作らない最小権能。ブラウザで Web も使いたいユーザーは §3 を別途通る)。**既存の Web セッションを承認の資格にしない**: 毎回フレッシュな OAuth 完走を要求する。本節のいかなる操作も §5 のセッション能力制限の許可列挙に**追加しない**(セッションクッキーの窃取・Web XSS が「CLI ログインの承認」= 実質のトークン発行へ昇格する経路を、列挙の外側で塞ぐ — ADR-0018 改訂 2 の「発行は端末限定」と整合)
4. **承認**: callback の処理順は次で固定する。(i) §3 の 2 段目(state 検証・code 交換・ユーザー情報取得)を完了し、**GitHub OAuth の完走を確定させる**(ここで失敗したら以降の処理は起きない — 保護が OAuth 完走の後にのみ起きることが §4-1 (1) (c) の占有ブレーキの前提。state 検証だけでは cookie とクエリを自給する非ブラウザ呼び出しを排除できない)。(ii) **フロー行の保護 CAS**: 条件は「存在・未期限切れ・pending」で、「ブラウザ脚到達」を原子的に記録する(以後この行は §4-1 (1) (c) の退避対象から外れる)。**保護はべき等** — 既に保護済みの pending 行への再到達(再読込・プリフェッチ・複数ブラウザ)は成功として扱い、承認ページを再描画する(攻撃者が同じ行を何度保護しても得は増えない)。行がない・期限切れ・pending でない(approved / denied / consumed)場合は、**不可逆な副作用が起きる前に**一様なエラーページで終了する(§4-2 と同じ規律 — 分岐理由をフロー状態で出し分けない)。(iii) get-or-create(§3-3 = §3 の 3 段目。H1 導入後は signupPolicy ゲート・サインアップ招待コードの消費もこの点 — 判定点は不変。コードの消費はアカウント作成の対価であり、以後の再ログインにコードは不要 — この後にフローが期限切れても失われるのは PAT の配布のみで、再ログインで回復する)を行い、結果の user_id をフロー行へ**束縛 CAS**する — 未束縛(NULL)なら束縛、同じ user_id の再到達なら成功(べき等)、**別の user_id が束縛済みなら一様エラーで終了し、チケットは回転させない**(後続の別アイデンティティが先着承認者のチケットを失効させる・フローを乗っ取って別アカウントを CLI に束縛する、の両経路を閉じる。H1 では招待コードの消費より前にこの不一致を検査し、不一致で終わるフローにコードを燃やさない)。(iv) サーバーは**承認ページ**を返す。ページは userCode に加えて**このフローが発行する PAT の付与内容 — tokenName・要求スコープ・有効期限(expiresInDays 反映後)— を人間が読める形で表示し**(値はフロー行に既にあり、新しいワイヤ要素は不要。start の payload は未認証で呼び出し側が選ぶため、承認者が付与内容を一度も見ずに承認する形を許さない — §4-3 のフィッシング残余下では旧 device flow より改善できる位置にある)、**tokenName は未認証入力として不活性描画する**(書式・マークアップの解釈なし・1 行・承認文言との視覚的分離。受理時の文字種制約は §6 — 承認ページだけでなく一覧 API・ダッシュボードにも効く共通保護。名前に承認指示文を仕込むなりすましの残余は §4-3)、「**CLI に表示されたコードと一致する場合のみ承認**」の明示操作を要求する(拒否操作も併置)。承認の資格はページに埋め込む**単回・短命の承認チケット**(256-bit 乱数・ハッシュのみ保存・OAuth 完了と同一トランザクションで発行。同一 user_id の再到達のたびに新規発行し、同一フローの旧チケットは置換失効する — 有効なチケットは常に最新 1 枚。**承認できるのは最後に開いたページのみ**で、先に開いたページの承認は一様拒否になる — 対価は §4-3)であり、セッションではない。承認はフロー行の pending → approved(+ 認証済み user_id の束縛)への CAS。ページの配信規律は §15-3 の招待着地ページと同一(**スクリプトなし** — フォーム POST のみ・CSP `script-src 'none'`): 承認面が github.com から maruhi オリジン(Web = TCB)へ移る対価を、スクリプト面ゼロで最小化する
5. **取得 `POST /auth/cli/poll`(未認証)**: payload = `{ flowId, flowToken }`。状態 approved なら、まずフロー行を approved → consumed へ **CAS し、勝者だけが**この応答の中で §6 の発行(同名ローテーション・発行上限・既定 TTL・`auth.token_created` 監査 — すべて不変)を行い `{ token, tokenId, userId, expiresAtMs }`(旧 device 交換と同じ応答形)を返す(**CAS が発行をゲートする** — flowToken は 1 プロセスに束縛されない bearer であり並行 poll は想定内の入力。単回 = 二重配布の構造的排除は、発行が CAS の後にのみ起きることで成立する。CAS の敗者は一様拒否)。CAS 成功後に発行が失敗した場合、フローは consumed のまま終わり CLI は再ログインする(半配布を残さない fail-closed — 配布されなかった PAT は存在しない)。**PAT の生値はいかなる時点でも DB に置かない**(§10。発行は配布リクエストの処理内でのみ起きる)。pending は型付き応答で継続を指示し、ポーリング間隔の下限は 5 秒(サーバーは超過ポーリングを 429 で拒否してよい)。per-IP レート制限は start と同じ binding パターン
...
- **未認証 start への持続洪水(退避型上限の残余 — 2026-08-31 レビュー起点)**: §4-1 (1) の退避型上限の下でも、正規フローの寿命より速い持続洪水は正規の pending 行を退避させ続け、ログインを事実上妨害できる(受容 — 消費点がサーバー側にあるため per-IP 遮断・運用側 WAF・H3 の退避アラートで検知・対処可能であり、429 型の「安価な粘着占有」より攻撃コストが恒常的に高い形を選んだ)。退避されるのは**ブラウザ脚未到達の行のみ**(§4-1 (4) の保護 CAS が不可逆な副作用より先に行の生存を確定させる)なので、退避で失われるのは start 直後の状態だけであり、アカウント作成・H1 のサインアップ招待コード消費が退避で失われることはない
- **全行保護済みでの start 拒否(429)**: §4-1 (1) (c) で退避可能な行が 1 つも無い場合、start は 429 を返し新規ログインが一時的に不能になる(受容 — この状態を作るには同時に上限件数の OAuth 完走済み pending フローが要り、正規では大規模イベント・攻撃としては行ごとの OAuth 完走コストを要するため安価に維持できない。行 TTL〔15 分〕で自然回復し、上限到達は H3 のアラート対象 — §4-1 (1))
...
- **承認ページの開き直しは旧ページの承認能力を失効させる(チケット置換失効の対価)**: 同じフローの承認ページを複数開いた場合(再読込・複数ブラウザ — 同一 user_id)、承認できるのは最後に開いた 1 枚のみで、先に開いたページでの承認は一様エラーになる(受容 — 複数チケットを並行有効にする複雑さより、開き直せば済む摩擦を選ぶ。最悪でも CLI の再ログインで回復する)。**別 user_id が先着束縛したフローは以後どのアイデンティティでも承認不能**(束縛は NULL → user_id の単調遷移で解除・上書きしない — 受容: 主な正規到達経路は複数 GitHub アカウント持ちの取り違え〔誤ったアカウントで開き → 正しいアカウントで開き直し〕で、回復は CLI の再ログイン = 新しいフロー。**H1 の招待制下では例外**: 1 番目の到達は NULL → 束縛であり (iii) の不一致検査が存在しないため、単回のサインアップ招待コードは誤アカウント側で消費される — 再ログインだけでは回復せず招待の再発行を要する。運用手当〔再発行〕か消費点の移動〔start 時の予約 + poll 時の確定消費 — §4-1 (1) の有界化規則と干渉するため骨格に触る〕かは **H1 側の裁定に送致**し、本仕様は分岐の存在のみ記録する。承認ページ側の文言も §4-2 の規律どおり「別のアカウントでログイン済み」等の理由を出し分けない)
