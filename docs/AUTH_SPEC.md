...
- **認可**: セッション主体 = 可(§5 の許可列挙「プロジェクト一覧」— 実装は `SESSION_ALLOWED_ENDPOINTS` への追加を同じ改訂で行う)。トークン主体 = 候補と**スコープの交差**のみを応答する(read 以上を与えるスコープが対象プロジェクトを覆う行のみ。`*` スコープは全件)— スコープ外は不出現であり、他所の「スコープ外 = 404」と同じ情報量
