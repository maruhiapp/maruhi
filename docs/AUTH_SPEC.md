...
- **導出形**: D1 投影 `project_members(project_id, user_id)`(role・状態列を持たない集合)を**候補索引**とし、一覧時に候補ごとにプロジェクト DO へ membership を確認する。応答は確認を通過した行のみ — **投影はいかなる認可判定にも使わない**(認可・role の真実源は従来どおりチェーン。CRYPTO_SPEC §6.4 の「2 つの真実源の禁止」と両立する導出キャッシュ)。DO が非メンバーと答えた候補行(stale ghost)は応答から除外し、読取時に削除して収束させる
...
- **受理ポリシー**: ページング = `after`(projectId の排他カーソル・省略可)+ サーバー固定 100 件 / ページ(project_id 昇順。D1 候補ページが満杯のときのみ `nextAfter` を返す)。1 呼び出しの DO 確認をページ上限に有界化する。レート制限は置かない(コストは本人の投影行数に有界で、他者が膨らませられない)。監査イベントは記録しない(値・鍵に触れない自己情報の読み取り)。セッション主体への CSRF ヘッダーも要求しない(§11-4 の「状態」= 監査行・計数への書き込みを持たない。ghost 行の読取時削除は §12-6 の旧鍵ラップ掃除と同じ「チェーン truth への導出キャッシュ収束」であり、クロスサイトに誘発されても正当な一覧と同じ収束が起きるだけで利得がない)
