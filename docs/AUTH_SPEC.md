...
3. **ブラウザ脚 `GET /auth/cli/verify?flow=<flowId>`**: サーバーは §3 と同じ web OAuth を実行する(state に flow 束縛を含める。GitHub の callback は §3 の単一 URL のまま)。**既存の Web セッションを承認の資格にしない**: 毎回フレッシュな OAuth 完走を要求する。本節のいかなる操作も §5 のセッション能力制限の許可列挙に**追加しない**(セッションクッキーの窃取・Web XSS が「CLI ログインの承認」= 実質のトークン発行へ昇格する経路を、列挙の外側で塞ぐ — ADR-0018 改訂 2 の「発行は端末限定」と整合)
4. **承認**: OAuth 完了 → get-or-create(§3-3。H1 導入後は signupPolicy ゲート・サインアップ招待コードの消費もこの点 — 判定点は不変)→ サーバーは**承認ページ**を返す。ページは userCode を表示し、「**CLI に表示されたコードと一致する場合のみ承認**」の明示操作を要求する(拒否操作も併置)。承認の資格はページに埋め込む**単回・短命の承認チケット**(256-bit 乱数・ハッシュのみ保存・OAuth 完了と同一トランザクションで発行)であり、セッションではない。承認はフロー行の pending → approved(+ 認証済み user_id の束縛)への CAS。ページの配信規律は §15-3 の招待着地ページと同一(**スクリプトなし** — フォーム POST のみ・CSP `script-src 'none'`): 承認面が github.com から maruhi オリジン(Web = TCB)へ移る対価を、スクリプト面ゼロで最小化する
