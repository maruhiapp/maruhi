...
- `checkpoint` エントリ自体は**汎用チェーン追記 API(§11 の append)で受理する**(データ登録を伴わない純チェーン op であり、複合を要しない — `create_environment` / `rotate_epoch` を複合限定にした §6 の判断とは逆向きの整理。トークンスコープは write — §6 の表の「チェーンエントリの op で決まる」に `checkpoint` = write を追加する)。受理検証(合意規則 + 保存状態との突合)は CRYPTO_SPEC §6.2 / §6.4。突合失敗は 422(型付き `CheckpointStateMismatch` — 理由: `manifest-mismatch` / `values-digest-mismatch` / `audit-head-unknown`)
