...
- (環境, エポック) のラップ集合の**初回登録**(環境作成時のエポック 1、ローテーション後の新エポックの一括登録)は、**要求受信者集合との完全一致**を要求する: 欠落・集合外宛・鍵不一致・重複はすべて 422 で拒否する。要求受信者集合 = **現メンバー集合 + 当該環境を開示スコープに含む有効な grant_server のサーバー鍵**(2026-08-12 改訂 — §12-4 の複合完全集合と同一の定義。旧「現メンバー集合との完全一致」はサーバー鍵宛ラップの導入により受信者クラス横断の定義へ一般化した)
...
- 認証はリクエスト同梱の OIDC トークン(JWT)で行う。maruhi トークン・セッションは使わない(§12-3 の表の外)。**認証と認可を厳密に分離する**(2026-08-12 レビュー反映 — 判定の帰属が曖昧だと存在秘匿が破れる):
- **認証(プロジェクト状態に依存しない検証。失敗は 401)**: (1) issuer が**サーバー実装の対応 issuer 一覧**(v1 = GitHub Actions `https://token.actions.githubusercontent.com` のみ)に含まれること、(2) 署名が issuer の JWKS(OIDC discovery 経由で取得・TTL キャッシュ。取得失敗時は fail-closed)で検証でき、alg が RS256 / ES256 のみであること(対称鍵 alg・`none` は拒否)、(3) `exp` / `iat` の時刻検証(clock skew 許容 ±60 秒)。**チェーン・grant・ポリシーを参照する判定を認証段に含めてはならない**
- **認可(プロジェクト・チェーン状態に依存する判定。失敗は一律 404)**: チェーン導出の有効 grant_server の lease_policy(CRYPTO_SPEC §6.2)のいずれかの要素と照合する — issuer_url の一致、`aud` = 要素の audience(推奨値はデプロイメントの origin)、claim_constraints の**完全一致**(制約に列挙された claim のみ評価し、列挙外の claim は関与しない)。加えて対象環境が開示スコープ(scope_environments)に含まれること。**認証を通過した主体に対しても、認可の失敗理由(プロジェクト不存在 / grant なし / ポリシー不一致 / スコープ外環境)を区別できる応答を返さない**(§11-2 の存在秘匿 — 有効な OIDC トークンの保持者にプロジェクト存在の探索を許さない)。評価意味論の拡張(prefix 等)は本章の改訂で行い、チェーン形式には影響しない(CRYPTO_SPEC §6.2 の構造 / 意味論の分離)
...
- 認証(§14-1 — 失敗は 401)→ 認可(§14-1 — 失敗は一律 404)→ サーバー鍵宛ラップの存在(欠落 = 503 型付き `LeaseUnavailable`、reason: `server-wraps-missing` — grant 済みだが再ラップ未了の状態〔CRYPTO_SPEC §7〕を不透明な失敗にしない。**認可を通過した主体にのみ返る**ため存在秘匿と両立する)
