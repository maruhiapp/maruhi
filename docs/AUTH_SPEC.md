...
- **サインアップ制御(§3)との合成(2026-09-01 H1)**: `signupPolicy = invite` 下でアカウント未保持の相手を招く場合、受諾には先にアカウント(= Web サインアップ §3)が要る。サインアップ招待コードは本節の招待リンクとは**別チャネル**で渡す — リンク形式は変えず、本節の招待トークンにサインアップ権限を焼き込まない(リンクの漏洩半径を広げない — session-47 §8 第 2 周の棄却)
