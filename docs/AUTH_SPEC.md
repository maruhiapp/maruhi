...
- **既定 TTL(2026-08-28 起草 — SECURITY_REVIEW 2026-08-14 L-2 の解消。ADR-0018 改訂 2・3 項。2026-08-30 W3a で実装 + 申し送り解消)**: device flow で発行するトークンは既定の有効期限を持つ(既定値: 発行から **90 日**。expires_at へ発行時に固定 — セッション §5 のスライディング更新とは意図的に非対称: トークンには定期再認証を強制する)。期限切れトークンは検証時に 401 で拒否する(失効と同じ扱い。期限切れ・失効・不明の区別をワイヤに出さない)。更新は再ログイン = 同名ローテーション(上記)。セルフホストでの値の調整は受理ポリシーの引き上げと同様に許す(合意規則ではない — 定数は api-schema の `DEFAULT_TOKEN_TTL_DAYS`)。**既存(無期限)トークンの移行規則(2026-08-30 W3a 裁定 CE)**: 導入マイグレーションが expires_at NULL の既存行へ「**適用時点 + 既定 TTL(90 日)**」を再アンカーする — 発行時起点(created_at + 90 日)は 90 日より古い既存トークンを適用と同時に一斉無効化する(静かな停止)ため棄却、非遡及は L-2 を既存行に恒久温存するため棄却(棄却案の比較は docs/notes/session-44.md)。加えて検証側は **NULL の expires_at を期限切れとして扱う(fail-closed)**: 移行未適用のまま新コードだけをデプロイした場合も無期限トークンは復活せず、再ログイン(同名ローテーション)が expires_at 付きの行を発行して自己回復する。**発行時の明示 TTL 指定(同裁定 CF — リース非対応実行環境の申し送りの解消)**: `POST /auth/device/exchange` は `expiresInDays`(整数 **1..365** — 上限はワイヤ Schema で強制)を受け付ける。リース(§14)非対応の実行環境(GitLab CI / k8s / cron 等 — 対応 issuer は v1 = GitHub Actions のみ)で PAT を無人利用する場合の逃し弁で、既定 90 日のまま明示時のみ最長 1 年まで延ばせる(発行は device flow = 人間の承認つき経路のみ)。無期限の既定へ戻す選択肢は採らない(L-2 の再導入 — PR #103 レビュー申し送りのとおり)。対応 issuer の拡張(GitLab 等)は排他でない長期経路として別途扱う(§14 の将来拡張)。**期限の自己観測(同裁定 CI — 第 2 次探索)**: `GET /auth/me` はトークン主体に提示トークンの `tokenExpiresAtMs` を返す(§16-2 の tokenScopes と同じ「自分が提示した資格情報の属性」— 新しい情報を開示しない)。無人 PAT が期限切れを突然の 401 でなく事前警告として扱うための材料で、スコープ限定トークンでも一覧(トークン主体は `*` × admin — 上記)を開かずに自分の期限だけを知れる
