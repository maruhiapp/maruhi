...
- **承認ページの開き直しは旧ページの承認能力を失効させる(チケット置換失効の対価)**: 同じフローの承認ページを複数開いた場合(再読込・複数ブラウザ — 同一 user_id)、承認できるのは最後に開いた 1 枚のみで、先に開いたページでの承認は一様エラーになる(受容 — 複数チケットを並行有効にする複雑さより、開き直せば済む摩擦を選ぶ。最悪でも CLI の再ログインで回復する)。**別 user_id が先着束縛したフローは以後どのアイデンティティでも承認不能**(束縛は NULL → user_id の単調遷移で解除・上書きしない — 受容: 主な正規到達経路は複数 GitHub アカウント持ちの取り違え〔誤ったアカウントで開き → 正しいアカウントで開き直し〕で、回復は CLI の再ログイン = 新しいフロー。承認ページ側の文言も §4-2 の規律どおり「別のアカウントでログイン済み」等の理由を出し分けない)
