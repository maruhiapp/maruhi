...
- **(2026-08-31 裁定 DH)** 本節は**サインアップ(アカウント新規作成)の唯一の入口**である — §4 の CLI ログインはアカウントを作らない(アイデンティティの照会のみ)。H1 の signupPolicy ゲートとサインアップ招待コードの受理は、本節 3. の「不在 → 作成」分岐の直前に置く(hosted-design.md §2-2)

**サインアップ制御(2026-09-01 H1 — hosted-design.md §2-2 / §8 gap 1 の実装。本改訂を含む実装 PR のマージをもって所有者承認とする)**:

- **`signupPolicy`(`open` | `invite` | `closed`。既定 `open` = 従来挙動と同一)**はデプロイメント単位のサーバー受理ポリシー(§12-11 schemaPolicy と同じクラス — チェーン・署名に載せない。悪意サーバーは自分の受理ポリシーを常に無視できるため、チェーン化しても敵対保証は増えない)。置き場は D1 の deployment 設定テーブル(`deployment_settings` — key/value)。変更手段は運営の wrangler / SQL 経路のみ(管理 UI・認証付き設定エンドポイントは作らない — 攻撃面の増分を避ける。手順は `docs/SELF_HOSTING.md`)。**未知の保存値は `closed` として扱う(fail-closed — 運営の誤設定が黙って `open` に化けない。実効値は `/auth/config` の advisory で観測できる)**
- **判定点は本節 3. の「不在 → 作成」直前の単一ゲート**。既存ユーザーのログイン・トークン検証・プロジェクト招待の受諾・リカバリー・チェーン API には一切影響しない(新規作成だけを塞ぐ)。`closed` = 新規作成の全拒否。`invite` = 有効なサインアップ招待コードの提示があるときのみ作成。**判定は受理時点の設定**(§12-11 と同じ規則): 実装はポリシー条件とコード消費 CAS を**アカウント作成と同一の D1 トランザクション内の条件**として畳み、読み取りと作成の間に設定遷移との競合窓を作らない
- **拒否の形**: OAuth 完走後(「新規かどうか」の判定材料はそこまで存在しない — 構造上の帰結)に、スクリプトなしの案内ページ(§15-3 の招待着地ページと同じ配信規律 — CSP `script-src 'none'`・英語)で拒否する。**拒否時に users / linked_identities / organizations / memberships の行を作らない(fail-closed)**。監査は `auth.signup_denied`(AUDIT_SPEC §3.1 — actor は user_id なしの type=user・固定窓上限つき・プロバイダ識別子を記録しない)。この記録が H3 の「サインアップ拒否の計数」トリップワイヤ(hosted-design.md §5-2)の材料になる
- **サインアップ招待コード**: `maruhi_sgn_` + Base62 の 256-bit 乱数 bearer・**単回(消費 CAS)**・期限つき(起草値 7 日 — §15 と同じ)。保存は提示文字列全体の SHA-256 ハッシュのみ・生値は発行時に一度だけ表示(§5 / §15 の規律の踏襲)。コードは**アカウント作成の許可だけ**を運ぶ — プロジェクト・org・role の一切と結びつかない。プロバイダ識別子(GitHub ID / login)・メールの allowlist は作らない(§2 の規則・§10 の禁止事項 — hosted-design.md §2-2 の棄却案を再導入しない)。発行は運営操作(リポジトリ同梱のスクリプト + wrangler d1 経路 — 発行 API・UI は作らない)
- **コードの運搬と事前検証**: サインアップリンクは `GET /auth/github/start?signup_code=<code>`。サーバーは**開始時に事前検証**(存在・未消費・未失効 — per-IP レート制限つき)し、無効なら GitHub へのリダイレクトが起きる**前**にスクリプトなし案内ページで終了する(無効コードのために OAuth ダンスを走らせない fail-fast。独立の事前検証エンドポイントは設けない — 検証点は開始ハンドラに畳む)。有効なコードは**その SHA-256 ハッシュ**を `__Host-` クッキー(HttpOnly)に載せて callback まで運ぶ(callback が必要とするのはハッシュ照合と消費 CAS のみで、生値の再登場点が存在しない — 生値のワイヤ出現は start リクエストの 1 回だけになり、ブラウザのクッキーストアに生値が残らない。「生値は発行時に一度だけ」〔§5 / §15〕の規律の運搬面への適用 — PR #133 第 2 次探索)。**クッキー値は発行時の OAuth state に束縛し、callback は state 一致のときのみ採用する**(`__Host-` / path=/ のクッキーは同一ブラウザの後続の無関係な OAuth 完了にも同送されるため、束縛が無いと持ち越されたコードが別フローで消費されうる — PR #133 レビュー対応)。クッキーは HTML / 302 を返す callback の全終端(成功・拒否・CLI ブラウザ脚)で単回失効し、型付きエラー終端(Set-Cookie を運ばない 400 / 429)に残る値は state 束縛が無害化する(10 分の maxAge で自然消滅)。事前検証は存在オラクルにならない(256-bit 乱数・単回 — §15 招待トークンと同水準)。コードが start URL のクエリに現れること(ブラウザ履歴への残存)は受容する — コードは単回・短命・作成許可のみで、§4-1 の flowToken(PAT 取得資格 — ブラウザチャネル禁止)とは運ぶ権限の重みが異なる
- **消費のタイミング**: アカウント作成(users / linked_identities / パーソナル org / 監査)と**同一の D1 batch(トランザクション)内の CAS**。作成に失敗した試行がコードだけ燃やす形・作成が成功したのにコードが未消費で残る形の両方を構造的に排除する。`open` 下で提示されたコードは(事前検証はされるが)消費されない — ゲートがコードを要求しない
- **`GET /auth/config`(§4)は `signupPolicy` を advisory として返す**(公開情報 — ランディングの「ベータは招待制」と同じ内容。session-47 §10-2 の改訂裁定)。検証・認可規則の入力にはしない。本フィールドを持たない旧サーバーに対し、クライアントは従来どおり進む(advisory の欠落でログインを壊さない)
- **CLI(§4)との関係**: CLI ログインはアカウントを作らない(裁定 DH)ため、本ゲートは CLI 経路に存在しない(start の payload にコードは載らない)。CLI は start 前に advisory を確認し、`invite` / `closed` では新規希望者へ「Web でサインアップしてから `maruhi login`」の案内を出す(hosted-design.md §2-2 (i)(ii) — **誤操作ガードであって認可ではない**。受理の正は本節のサーバーゲート。サーバー側のみの中間状態でも fail-closed は成立する — 同 §9 H1 行)
...
**原則: CLI ログインはアカウントを作らない(2026-08-31 追補裁定 DH — session-48.md)。** 本節は**既存アカウントの認証と PAT の配布のみ**を行う。サインアップ(アカウント新規作成)の唯一の入口は §3 の Web ログインであり、H1 の signupPolicy ゲート・サインアップ招待コードの受理もそこに置かれる(本節の payload には載らない)。アカウントを持たないアイデンティティがブラウザ脚を完走した場合はサインアップ案内(§4-1 (4) (ii))へ誘導し、**承認まで一切の不可逆な副作用を起こさない**。同じく裁定 DH により、**start はサーバーに状態を作らない**(無記録 — フロー行が生まれるのは「既存アカウント保持者が OAuth を完走した瞬間」のみ)。この 2 つで、未認証面の状態管理(旧: 定員・退避・保護 CAS)と、承認前の副作用の順序問題(旧: 招待コードの先行消費)は構造ごと消滅する。

### 4-1. フロー

1. **開始 `POST /auth/cli/start`(未認証・無記録 — 裁定 DH)**: payload = `{ tokenName?, scopes?, expiresInDays? }`(発行パラメータの意味論は §6 — 旧 device 交換と同一。H1 のサインアップ招待コードは**本 payload には載らない** — 受理は §3 の Web サインアップのみ〔裁定 DH〕)。応答 = `{ flowId, flowToken, userCode, verificationUrl, expiresInSeconds, pollIntervalSeconds }`。**サーバーはこの時点で何も保存しない** — フローの真正性は署名(下記)で無状態に検証する
   - `flowId` は公開相関子(verificationUrl のクエリに現れる)だが、**推測不能な乱数**とする(起草値 128-bit)。`flowToken` は CLI にとって不透明な bearer 資格情報で、実装は**自己完結の署名形式**とする: 256-bit 相当以上の乱数エントロピー + **flowId** + 期限を署名対象に含む、フロー署名鍵(§4-2)による MAC。**flowId を署名対象に含めることは要件**である — start は未認証・無償なので誰でも正当な flowToken を自分に発行でき、束縛が無いと「被害者の flowId(公開相関子)+ 自前の flowToken」の組み替えで他人のフローを poll できてしまう(旧規定の「(flowId, flowToken) の組が保存行と一致」検査の無記録化後の後継 — 検査の削除ではなく MAC 署名入力への移設)。サーバーは poll 受理時に MAC・期限・**署名内 flowId と提示 flowId の一致**を検証する(保存も、ハッシュ保存も、しない — 行が無いのだから)。**flowToken をブラウザチャネル(URL・承認ページ・リダイレクト)に決して載せない** — ブラウザ履歴・referrer が CLI の資格情報の集積になる経路を構造的に断つ(flowId / flowToken を二識別子に分離する理由 — 不変)
   - `verificationUrl` は flowId・期限・userCode・**発行パラメータ(tokenName / scopes / expiresInDays — 承認ページの表示と行作成に必要)**を運び、これら全体を覆う**ブラウザ脚用 MAC(vsig)**を付す。vsig は flowToken と**ドメイン分離**された MAC で(§4-2 — 用途タグにより一方が他方として検証を通らない)、URL の知識はポーリング資格を一切与えない。vsig の無い / 改竄された / 期限切れの verify 到達は、GitHub へのリダイレクトが起きる**前に**一様なエラーページで終了する(でっち上げの flowId のために OAuth ダンスを走らせない — fail-closed)
   - `userCode` は照合用の短い表示コード(**秘密ではない** — CLI と承認ページの双方に表示し、人間が突合する摩擦装置。旧 device flow のコード入力と同水準の明示性を承認に残す)
   - フローの期限は起草値 15 分(署名に含める)。per-IP レート制限をハンドラ最初に置く(旧 device 交換と同じ Workers Rate Limiting binding のパターン。起草値 10 回/分/IP — 無記録化により、これは DB 保護ではなく CPU 保護)。サーバー未設定は §3 の自己診断どおり 503 `SetupIncomplete`(GitHub に到達する前に fail-closed)
   - 旧規定の「フロー行の寿命と総量の有界化」(日和見削除 + デプロイメント全体上限 + 退避)は**裁定 DH で構造ごと置換された**: start が行を作らないため、未認証面に埋めるべきプールが存在しない。行の総量規律は §4-1 (4) (iii)(作成点)に移った
...
3. **ブラウザ脚 `GET /auth/cli/verify?flow=<flowId>&…`**: まず vsig と期限を無状態に検証し(§4-1 (1) — 失敗は一様エラーページ)、次に §3 の 1〜2 段(state 発行・検証、code 交換、ユーザー情報取得)と同じ web OAuth を実行する(state に flow 束縛〔vsig 済みパラメータ一式〕を含める。GitHub の callback は §3 の単一 URL のまま)。§3 の 3 段目は**アカウントの照会のみ**へ置換し(get-or-create ではない — 裁定 DH。作成しない)、**4 段目(セッション発行 §5)は行わない** — CLI ログインの成果物は §4-1 (5) の PAT のみで、Web セッションという副作用を持たない(承認に不要な資格情報を作らない最小権能。ブラウザで Web も使いたいユーザーは §3 を別途通る)。**既存の Web セッションを承認の資格にしない**: 毎回フレッシュな OAuth 完走を要求する。本節のいかなる操作も §5 のセッション能力制限の許可列挙に**追加しない**(セッションクッキーの窃取・Web XSS が「CLI ログインの承認」= 実質のトークン発行へ昇格する経路を、列挙の外側で塞ぐ — ADR-0018 改訂 2 の「発行は端末限定」と整合)
4. **承認**: callback の処理順は次で固定する。(i) §3 の 2 段目(state 検証・code 交換・ユーザー情報取得)を完了し、**GitHub OAuth の完走を確定させる**(ここで失敗したら以降の処理は起きない — フロー行の作成が OAuth 完走の後にのみ起きることが (iii) の総量規律の前提。state 検証だけでは cookie とクエリを自給する非ブラウザ呼び出しを排除できない)。(ii) **アカウント解決(照会のみ — 作成しない)**: 認証済みアイデンティティを linked_identities で引く。**不在なら「サインアップ案内ページ」を返して終了する** — §3 の Web ログイン(= サインアップの唯一の入口。H1 では signupPolicy ゲート・サインアップ招待コードの受理もそこ)への導線と、**フロー再開リンク(= verificationUrl — state に含まれる vsig 済みパラメータ一式から復元できる)**のみを載せたスクリプトなしページ。このページ自身は単回の OAuth callback 応答であり**再読込ではフローを再開できない**ため、案内は必ず verify URL へ戻す形にする(CLI 側に表示されたままの verificationUrl を開き直しても同じ)。フロー行は作らず・チケットも発行せず・**一切の不可逆な副作用を起こさない**(「このアイデンティティにアカウントが無い」という情報は当人にのみ返るため、オラクルではない)。ユーザーがサインアップを終えて再開リンク(verificationUrl)を開けば、同じフローが期限内なら (i) からやり直して続行できる。(iii) **フロー行の作成 CAS(create-or-match)**: フロー行はこの瞬間に**初めて**生まれ、生まれた時点で認証済み user_id・発行パラメータ(vsig 済み URL から)・期限・承認チケットを持つ(旧規定の「保護」「束縛」に相当する状態が作成と同時に確定する — 中間状態が存在しない)。同じ flowId の行が既にある場合: **同じ user_id の再到達**(再読込・複数ブラウザ)は成功として扱い、承認チケットを置換発行して承認ページを再描画する(べき等)。**別の user_id が既に居るなら一様エラーで終了し、チケットは回転させない**(後続の別アイデンティティが先着承認者のチケットを失効させる・フローを乗っ取って別アカウントを CLI に束縛する、の両経路を閉じる)。行が期限切れ・approved / denied / consumed の場合も一様エラーページ(§4-2 と同じ規律 — 分岐理由をフロー状態で出し分けない)。**行の総量規律はこの作成点に置く**: 期限切れ行の日和見削除を作成 batch の先頭に同梱し、デプロイメント全体の同時未消費行の上限(起草値 1,000)を超える作成は一様エラーページで拒否して H3 のアラートに載せる(上限到達には「既存アカウント × OAuth 完走」を上限件数ぶん同時併走させる必要があり、無償で埋められた旧 start プールと違って安価な粘着占有が成立しない — 退避機構は不要。TTL 15 分で自然回復。上限値はセルフホストの受理ポリシーとして調整可)。(iv) サーバーは**承認ページ**を返す。ページは userCode に加えて、**どのアカウントとして承認しようとしているか(認証済みアイデンティティの表示名)**と、**このフローが発行する PAT の付与内容 — tokenName・要求スコープ・有効期限(expiresInDays 反映後)— を人間が読める形で表示し**(前者はアカウント取り違えの可視化 — §4-3 の詰み分岐の主経路対策。後者は、start の payload が未認証で呼び出し側の選ぶ値であるため、承認者が付与内容を一度も見ずに承認する形を許さない — §4-3 のフィッシング残余下では旧 device flow より改善できる位置にある)、**tokenName は未認証入力として不活性描画する**(書式・マークアップの解釈なし・1 行・承認文言との視覚的分離。受理時の文字種制約は §6 — 承認ページだけでなく一覧 API・ダッシュボードにも効く共通保護。名前に承認指示文を仕込むなりすましの残余は §4-3)、「**CLI に表示されたコードと一致する場合のみ承認**」の明示操作を要求する(拒否操作も併置)。承認の資格はページに埋め込む**単回・短命の承認チケット**(256-bit 乱数・ハッシュのみ保存・行の作成 / 再到達と同一トランザクションで発行。同一 user_id の再到達のたびに新規発行し、同一フローの旧チケットは置換失効する — 有効なチケットは常に最新 1 枚。**承認できるのは最後に開いたページのみ**で、先に開いたページの承認は一様拒否になる — 対価は §4-3)であり、セッションではない。承認はフロー行の awaiting → approved への CAS(user_id は作成時に確定済み)。拒否操作は awaiting → denied への CAS。ページの配信規律は §15-3 の招待着地ページと同一(**スクリプトなし** — フォーム POST のみ・CSP `script-src 'none'`): 承認面が github.com から maruhi オリジン(Web = TCB)へ移る対価を、スクリプト面ゼロで最小化する
5. **取得 `POST /auth/cli/poll`(未認証)**: payload = `{ flowId, flowToken }`。サーバーはまず flowToken の MAC・期限・**署名内 flowId と payload の flowId の一致**を**無状態に検証**する(いずれの失敗も一様な型付きエラー — §4-2。flowToken は当該フロー専用であり、他フローの flowId との組み替えは資格にならない)。検証成功後に flowId で行を引く: **行が無ければ型付き pending**(ブラウザ脚が未到達なだけ — 無記録 start の正常系)。行が approved なら、まず approved → consumed へ **CAS し、勝者だけが**この応答の中で §6 の発行(同名ローテーション・発行上限・既定 TTL・`auth.token_created` 監査 — すべて不変。発行パラメータは行の保持値)を行い `{ token, tokenId, userId, expiresAtMs }`(旧 device 交換と同じ応答形)を返す(**CAS が発行をゲートする** — flowToken は 1 プロセスに束縛されない bearer であり並行 poll は想定内の入力。単回 = 二重配布の構造的排除は、発行が CAS の後にのみ起きることで成立する。CAS の敗者は一様拒否)。CAS 成功後に発行が失敗した場合、フローは consumed のまま終わり CLI は再ログインする(半配布を残さない fail-closed — 配布されなかった PAT は存在しない)。**consumed / denied の行は flowToken の期限 + 余裕(起草値 +5 分)まで削除しない** — 先に消すと「行なし = pending」に化け、CLI が完了済みフローを無限に待つ(掃除の対象は期限経過後のみ — §4-1 (4) (iii) の日和見削除)。**PAT の生値はいかなる時点でも DB に置かない**(§10。発行は配布リクエストの処理内でのみ起きる)。pending は型付き応答で継続を指示し、ポーリング間隔の下限は 5 秒(サーバーは超過ポーリングを 429 で拒否してよい)。per-IP レート制限は start と同じ binding パターン

### 4-2. 検証・エラー・監査の規律

- **一様拒否は資格不一致に対して**: flowToken の MAC 検証・期限検証・**flowId との組一致検証(署名内 flowId ≠ 提示 flowId — §4-1 (1))**に失敗する poll、vsig 検証に失敗する verify 到達は、一様な型付きエラー / エラーページで拒否する(フローの存在・状態のオラクルを作らない — §11-2 と同じ規律)。正当な flowToken の保持者(= フロー作成者自身)には pending / denied / expired を型付きで区別して返してよい(自分が作ったフローの状態であり新しい情報を運ばない)。承認チケットの不明・期限切れ・使用済みも一様拒否
- **フロー署名鍵(裁定 DH)**: start の無記録化に用いる MAC(**flowToken 用と vsig 用の 2 系統**)は **HMAC-SHA-256(WebCrypto)**・サーバー側秘密鍵とする。**2 つの MAC は用途タグでドメイン分離する**(署名入力の先頭に固定の用途識別子を置く — 例: `cli-flow-token` / `cli-verify-url`。または鍵を用途ごとに HKDF で派生させる)。**署名入力の連結は一意に復号可能な符号化とする**(長さ前置き等 — フィールド境界の曖昧さで別の入力列が同じバイト列に潰れない)。この 2 句が無いと、署名対象の重なる vsig(ブラウザチャネルを流れる値)が flowToken として検証を通る混同経路が実装裁量で生まれ、「URL の知識はポーリング資格を与えない」(§4-1 (1))が崩れる。鍵は初回使用時に自動生成して D1 に保存する(**セルフホストのセットアップ手順を増やさない** — Workers Secret の追加登録を要求しない)。同時初回使用の競合は冪等な作成で解消する(先勝ち — `INSERT` の一意制約 + 読み戻しで、後着の生成値を破棄して既存鍵を使う。§6 の初期化と同じ D1 batch の作法。2 リクエストが別々の鍵を書き合って進行中フローを失効させる分岐を仕様段階で閉じる)。この鍵は E2EE の鍵素材ではない(CRYPTO_SPEC の対象外 — 平文シークレットに一切触れず、短命なログインフロー資格の完全性のみを守る。独自プロトコルの発明ではなく、標準プリミティブによる自己完結トークンの完全性保護)。鍵のローテーション = 進行中フロー(≤ 15 分)の失効であり、CLI の再ログインで回復する(許容)。なお無記録の保留フローは「DB バックでない」唯一の資格情報になるが、**セッションではない**(単独では何も許可しない — 承認状態・成果物の PAT はすべて DB バックであり、stateless セッション禁止〔ADR / §5〕とは対象が異なる。callback 前の取り消し操作は存在しないが、15 分 TTL と承認必須がそれを不要にする)
...
- **start / poll への洪水(裁定 DH 後の位置づけ)**: start は無記録・poll は無状態検証で始まるため、未認証の洪水がサーバーに残せる状態は**存在しない** — 影響は CPU とレート制限枠に縮退する(受容 — per-IP 制限・運用側 WAF)。旧規定(退避型上限とその残余・全行保護済み 429)は前提〔無償で作れる pending プール〕ごと消滅し、本項が置換する。フロー行の作成面(§4-1 (4) (iii))の上限到達は一様エラーページ + H3 アラート(到達には「既存アカウント × OAuth 完走」の同時併走が上限件数ぶん必要で安価に維持できない。TTL で自然回復)
...
- **承認ページの開き直しは旧ページの承認能力を失効させる(チケット置換失効の対価)**: 同じフローの承認ページを複数開いた場合(再読込・複数ブラウザ — 同一 user_id)、承認できるのは最後に開いた 1 枚のみで、先に開いたページでの承認は一様エラーになる(受容 — 複数チケットを並行有効にする複雑さより、開き直せば済む摩擦を選ぶ。最悪でも CLI の再ログインで回復する)。**別 user_id が先に行を作ったフローは以後どのアイデンティティでも承認不能**(行の user_id は作成時に確定し解除・上書きしない — 受容: 主な正規到達経路は複数 GitHub アカウント持ちの取り違え〔誤ったアカウントで開き → 正しいアカウントで開き直し〕であり、承認ページのアイデンティティ表示〔§4-1 (4) (iv)〕で取り違えに気づける。回復は常に CLI の再ログイン = 新しいフローで足りる — **裁定 DH により承認前に消費されるものが何も無い**ため、旧規定が H1 側へ送致していた「招待コードが誤アカウント側で燃える」分岐は消滅した〔サインアップと招待コードの受理は §3 にのみ存在し、CLI ログインのフローは何も消費しない〕。なお誤アカウントが maruhi アカウントを**持たない**場合はサインアップ案内で終わり行が作られないため、正しいアカウントで開き直せば**同じフローのまま**続行できる。承認ページ側の文言は §4-2 の規律どおり「別のアカウントでログイン済み」等の理由を出し分けない)
...
- **セッション主体の能力制限(2026-08-28 W0 裁定 — ADR-0018 改訂 2・1 項。実装は Wave 3 W2b — セッションを持つ Web 画面の初出〔W2〕より前)**: セッションクッキーは XSS に最も晒される資格情報であり(同一オリジンの XSS は CSRF ヘッダーも自分で付けられる)、Web の境界原則(読み取り + 失効系のみ)は UI・バンドルの省略では強制にならない。セッション主体が呼べる API を**肯定列挙**で制限する — 許可: **認証・自己情報系**(§3 のフロー・ログアウト・`GET /auth/me`・`GET /auth/recovery/status`・`GET /auth/key-wraps`〔§13-7 — 2026-09-12 KL3。ラップ・分片を運ばない状態表示のみ〕・**`GET /auth/devices`**〔§13-11 — 2026-09-19 DK。端末登録簿の読み取り。表示名と鍵 FP・公開鍵のみ、秘密を運ばない〕)、**読み取り**(チェーン取得〔§11〕・環境一覧・メタデータのみ pull〔§12-7〕・監査読み取り〔AUDIT_SPEC §7〕・要ローテーションフラグビュー・招待一覧〔§15-2〕・トークン一覧〔§6〕・**プロジェクト一覧〔§11-5 — 返すのは本人の membership のみ。設計文書 S4〕**)、**失効系**(招待の失効〔§15-2〕・トークンの指定失効〔§6〕)。**列挙外はセッション主体に対して拒否する**(fail-closed — 新設エンドポイントの既定は「セッション不可」であり、セッションに開く場合は本列挙への追加を同じ改訂で行う。拒否の位置は §11-2 の存在秘匿・§12-3 の判定順と整合させ、実装 PR は受理経路の固定テストで実効性を保証する。実装形は §12-10 (1) と同じ型を推奨: エンドポイント契約への宣言焼き込み = 単一実装点とし、ハンドラごとの手動検査を持たない)。とくに次はセッションからの正当な導線が存在せず明示的に拒否する: 値付き一括 pull(§12-7 — セッション経由の監査証跡汚染 = SECURITY_REVIEW L-1 の発生面自体を消す)、DEK の取得・登録・削除(§12-6 — 削除は署名を伴わない唯一の破壊系)、チェーン追記・init(§11)、環境・変数の全 mutation(§12-4 / §12-5)、招待の発行・受諾(§15-2)、rotation dismiss(AUDIT_SPEC §7)、リカバリーブロブの登録・取得(§13-2)、予備鍵ラップ台帳(旧 master 鍵ラップ台帳 — 2026-09-19 DK)の登録・取得・削除・保護者の分片取得・ハンドオフの要求・照会・承認・取得・取消(§13-7 — セッションに開くのは状態表示の `GET /auth/key-wraps` のみ)、端末の追加要求・端末登録簿の書き込み・削除(§13-11 — 端末限定。2026-09-19 DK)。署名を要する操作は鍵を持たない XSS には元々成立しないが、明示拒否に含めて防御を署名検証の実装詳細に依存させない。CLI・`maruhi ui` はトークン主体であり影響を受けない

## 6. API トークン

- 形式: `maruhi_pat_` + Base62 乱数(256-bit 相当)。プレフィックスで種別判別・secret scanning 対応
- スコープ: プロジェクト単位 × 権限(read / write / admin)。実効権限は min(トークンスコープ, 所有者のチェーン role)(§9-2)。**環境ごとの実効アクセスはチェーン上の scope(CRYPTO_SPEC §6.2 — 2026-09-14 ES)が決める**。将来: トークン単位の環境スコープ(チェーン scope をさらに絞るチェーン外 ACL — 加法的に追加できる形で未着手)、エージェント用の短命リーストークン(Phase 3、CRYPTO_SPEC と連動)
...
- **操作が要求する権限水準(2026-08-02 決定。2026-08-03 改訂)**: チェーン取得 = read。チェーン追記はエントリの op で決まる(`checkpoint` と `revoke_device` は payload — `checkpoint` は audit_head_hash の空 / 非空、`revoke_device` は target_user_id が呼び出し主体自身か他人か〔2026-09-19 DK〕— も含めて決まる)— `create_environment` / `rotate_epoch` = write(**ただしこの 2 op は §12-4 の複合リクエスト経由でのみ受理し、汎用チェーン追記 API では型付きエラーで拒否する** — チェーンエントリと付随データの原子性を汎用経路が迂回できないようにするため)、`checkpoint` = **audit_head_hash が空なら write、非空なら admin**(汎用追記経由 — §16-2。2026-08-18)、`add_member` / `remove_member` / `change_role` / `grant_server` / `revoke_server` / **`set_approval_policy` / `propose` / `approve` / `withdraw`** = admin(四眼の 4 op は内側 op と同じ水準 — 2026-09-14 PF1)、**`add_device` と自分の端末の `revoke_device` = write、他人の端末の `revoke_device` = admin**(2026-09-19 DK — §11-1)。プロジェクト作成(genesis init)= admin。op ごとの認可(誰がその op を実行できるか)の真実源は引き続きチェーン role(CRYPTO_SPEC §6.2)であり、この表はトークンスコープ側の必要条件である。データプレーン(変数・環境・DEK)の op 別水準は §12-3 の表に規定する
...
- **発行の上限(2026-08-02 追加、2026-08-27 deepsec S7 原子化)**: 別名トークンはユーザーあたり 100 本まで(超過は 429。同名ローテーションは上限に達していても可能)。上限判定と新規 insert は同じ `INSERT … SELECT … WHERE` で原子的に行い、異なる token 名の並行発行が同じ under-limit を観測して上限を超える形を許さない。同名ローテーションは既存行の検出・旧 id を載せた `auth.token_created`・置換を D1 batch で直列化する。トークン名は 128 文字以下かつ**制御文字・双方向制御文字(bidi)を含まない**(2026-08-31 追記 — ワイヤ Schema で拒否: 名前は承認ページ §4-1 (4)・一覧 API・ダッシュボードに呼び出し側由来のテキストとして描画されるため、表示面共通の保護を受理時に置く。**実装は H1b が引き取る** — §4 新エンドポイントと同時に既存 `tokenName` Schema を締め上げる。**非遡及**: 制約は受理時のものであり、旧 Schema 下で保存済みの名前は掃除しない — 公開前で実データは運営者由来のみ、かつ表示面は元より不活性描画が前提)、スコープは 100 エントリ以下、スコープの project はプロジェクト ID 形式または `"*"` のみ(認証済み主体による api_tokens の肥大 DoS の遮断)

- **端末鍵との対応(2026-09-19 DK)**: CLI ログインで発行するトークン(既定名 `cli:<hostname>`)は 1 端末に 1 本であり、端末鍵と同じ端末に住む。端末の失効(`revoke_device` — CRYPTO_SPEC §6.2)はチェーンの事実でありトークンとは独立だが、クライアントは失効時に当該端末のトークンの指定失効(本節)を提案し(SHOULD — 候補と期限の材料は本節 (1) の一覧 `GET /auth/tokens`: 登録簿の `tokenId` と一致する行、無ければ既定名 `cli:<hostname>` と一致する行を候補に出し、その行の `expires_at` を示す。`GET /auth/me` の `tokenExpiresAtMs`〔本節の既定 TTL 項・裁定 CI〕は**提示トークン**の期限であり、失効させる別端末のトークンの材料にはならない)、利用者が断った場合は「そのトークンは有効なまま残る」を、当該トークンの**実際の有効期限**(既定 90 日だが発行時に最長 365 日まで延長でき、セルフホストは `DEFAULT_TOKEN_TTL_DAYS` で変えられる)とともに表示する(CRYPTO_SPEC §14.2-11 の非保証)。**成立条件**: 一覧はトークン主体に `*` × admin を要求する(裁定 CH)一方、自分の端末の `revoke_device` は write 水準(§11-1)なので、write 止まりのトークンで失効を実行したクライアントは一覧を読めない — その場合は候補提示と期限表示を省き、「この端末のトークンは残る。`*` × admin のトークンを持つ端末か Web(トークン一覧・指定失効)で失効せよ」の事実だけを伝える(失効の導線自体を `*` × admin 前提にはしない — 既定のログイントークンは `*` × admin であり通常は成立する。2026-09-20 K1 レビュー対応、設計録 §6 K1-15)。サーバーは `revoke_device` の受理でトークンを失効させない(登録簿は advisory で認可の入力にしない — §13-11。チェーン受理の副作用にトークン失効を置く案は、失効対象の同定を advisory な行に依存させるため採らない — 設計録 dk-design.md §6 K1-15。所有者裁定で反転しうる)。対応は端末登録簿(§13-11)の `tokenId`(任意・advisory)で持つ
...
- **`add_device` / `revoke_device`(CRYPTO_SPEC §6.2。2026-09-19 DK)は汎用追記 API で受理する**(付随データを持たない)。トークン水準(§6 の表 — トークンスコープ側の必要条件。op の認可の真実源はチェーン role〔同 §6.2: `add_device` と自分の端末の `revoke_device` は role 不問・reader 可〕): `add_device`、および**対象 = 自分**の `revoke_device` = **write**(§12-3 の reader 自己バックフィルと同水準 — 端末の追加はその後のバックフィル〔write〕を伴い、失効はその後の rotate〔write〕を伴う。read トークンでは行えない)。**対象 = 他人**の `revoke_device` = **admin**(`remove_member` と同じ)。K1 レビュー(2026-09-20 pullfrog)で「admin 一律」を訂正 — reader / member が自分の端末を足せず盗難端末を失効できない形になっていた。`revoke_device` の受理副作用 = 対象端末の申告行の削除(§16-1)・要ローテーション検出(AUDIT_SPEC §4.1)・ミラー(AUDIT_SPEC §3.4)。端末数の上限は §12-8
...
- **プロジェクト数 / org の受理上限(2026-09-02 H2 — docs/notes/hosted-design.md §3-3 / §8 gap 2。裁定 CZ。本改訂を含む実装 PR のマージをもって所有者承認とする)**: org あたりの**アクティブなプロジェクト数**に受理ポリシー上限を置く(起草値 **100** — 定数は `apps/server/src/policy.ts`。§12-8 の表にも載せる)。DO の実体化 = 運営ストレージの消費であり、認証済みテナントあたりの作成面を有界にする(H1 のサインアップ制御が新規アカウント面を閉じたのと対)。合意規則ではない(§12-8 と同じ性格 — セルフホストでの引き上げは自由)
  - **判定点**: 上記順序の「org 権限確認(D1)」の**後**・「DO genesis 受理」の**前**。判定材料は D1 `projects` の当該 org の行数(`proj_org` 索引)。403(org 非メンバー)が上限判定に先行するため、org の外の主体に当該 org のプロジェクト数の情報(上限到達の有無)は返らない
  - **「アクティブ」の定義**: v1 にプロジェクト削除 API は存在しないため、**当該 org の全 `projects` 行**を数える。削除 API を導入する改訂は、本定義を「削除済み(tombstone)行を除く」へ改め、削除で枠が解放されることを同時に明記しなければならない(§12-8 の「削除で解放される」原則の org 単位版 — 本規定はその再訪条件)
  - **拒否の形**: 型付き **429 `ProjectLimit`**(payload = `limit`)。org 単位の数量上限であり、ユーザー単位の数量上限(§6 トークン発行上限)・プロジェクト単位の pending 招待上限(§15-2)と同じ 429 の語彙に揃える(§12-8 のプロジェクト内資源上限 = 422 とは層が異なる)。黙って劣化させない
  - **部分失敗修復経路(上記)を上限で塞がない**: 上限到達時も DO には「新規初期化を認めない(report-only)」指示つきで init を問い合わせ、DO が already-initialized を返す場合は通常どおり修復(行欠損 + 認証主体 = genesis actor → 行挿入)または 409 へ進む。**新規(未初期化)の genesis のみを 429 で拒否する**。上限判定だけで DO 問い合わせを省くと、DO 側で確定済みだが D1 行を欠くプロジェクト(クラッシュ窓)の修復が恒久に拒否され「D1 に見えないが DO に実在する」不整合が固定されるため、上限到達時の判定は DO の直列化の中で「未初期化か」と合成して下す
  - **並行 init の競合**: カウント(D1)→ DO 受理(確定点)→ `projects` 行挿入(D1・従属)の 3 段は原子化できず、上限直下で並行する複数の init が同じ under-limit を観測して上限を僅かに超過しうる。これは**受容する**(受理ポリシーであり暗号境界ではない — §13-3 の取得計数と同じ論法。超過幅は並行度に有界)。トークン発行上限(§6)の `INSERT … SELECT` 原子化を `projects` 行挿入へ適用する形は採らない: 挿入は DO 受理の**後**に来る従属書き込みであり、そこで拒否すると DO 確定済みのプロジェクトが行を持たない不整合を自ら作る(修復経路の毀損と同型)。行を DO 受理の前に条件付き挿入する形も採らない(順序「DO が確定点・行は従属」の反転 — verifyChain が拒否した genesis の行が残り、tombstone なしの v1 では枠を恒久に食う)
  - **org 単位の含意**: org 作成 API は未実装で全ユーザーがパーソナル org(§9-1)のみを持つため、v1 の実効は**ユーザー単位 100 プロジェクト**である。org 作成 API 導入時にこの含意は消える(上限自体は org 単位のまま)
  - **拒否経路が消費する共有資源(session-47 §10 の教訓)**: D1 の索引付き count 1 回。上限到達時はさらに DO への report-only 問い合わせ 1 回 — 未初期化 genesis の場合、名前指定の DO 実体化(空のテーブル群)が起きる。これは認証済み主体が任意 ID の GET で既に到達できる既知残余(A-4)と同一クラスであり、本上限が新しい消費面を作るものではない(問い合わせを省いて修復経路を塞ぐ選択肢より、この消費を受容する)
...
| 一括 pull(メタデータのみ — §12-7)・環境一覧 | read | reader 以上 | 不問(全環境) |
| 変数の作成(declared 作成・activation 複合 — §12-5 — を含む)・push・改名・スキーマ設定・削除、環境の改名、DEK ラップ登録 | write | member 以上 | 環境 ∈ scope(DEK ラップ登録は受信者側も — §12-6) |
| DEK ラップ登録(**受信者がすべて呼び出し主体自身の端末鍵**の場合 — 2026-09-19 DK) | write | **reader 以上** | 環境 ∈ 呼び出し主体(署名した端末)の実効 scope(受信者側も) |
| 環境の作成 | write | member 以上 | scope = all(CRYPTO_SPEC §6.2) |
| 環境の削除、DEK ラップの削除(§12-6 の修復経路) | admin | admin 以上 | 環境 ∈ scope |

- 判定順は §11 の先例と同一: 認証(401)→ 書き込み系はサイズ先行検査(413。資源保護は意味論的判定に優先)→ トークンスコープ(スコープ外 = 404、水準不足 = 403)→ チェーン導出メンバーシップ(非メンバー = 404)→ チェーン role(不足 = 403)→ **scope(403 `InsufficientScope` — 環境の存在判定と同段: 環境の存在はチェーン導出で既知。2026-09-14 ES)**→ 意味論的検査。**認可時点の二重判定**(署名を伴う操作 — 下記)は scope にも及ぶ: 受理時点と宣言ヘッド時点の両方で環境 ∈ scope(CRYPTO_SPEC §6.3 の 3′)
...
- **reader の自己バックフィル(2026-09-19 DK)**: reader が自分の新端末(予備鍵を含む)へ自分の DEK を包み直す経路(CRYPTO_SPEC §7 の端末追加バックフィル)。reader は当該 DEK を正当に保持しており、自分の鍵へ包み直しても他者の能力は増えない。受信者に他人の鍵が 1 つでも含まれれば従来どおり member 以上。scope・role の判定は署名した端末の実効権限で行う(CRYPTO_SPEC §6.2 — 2026-09-19 DK)
...
- **受信者集合 R(E)(2026-09-14 ES)**: 作成複合の (3) のラップ完全集合の対象は **受信者集合 R(E)(CRYPTO_SPEC §6.2 — scope に E を含む現メンバー + 開示スコープ内の有効 grant_server)**。作成者は scope = all でなければならない(合意規則 `environment-out-of-scope` — チェーンエントリの検証で落ちる)ため、R(E) は all-scope メンバー(+ 開示 grant)になる。ローテーション複合の新エポックのラップ完全集合の対象も R(E)。actor は E ∈ scope(合意規則)
- **add_member 後・scope 拡大後のバックフィルは複合化しない(意図的な非対称 — 不変。2026-09-14 ES で scope 拡大を追加)**: 「メンバーはいるがラップがない」中間状態は残る。対象の scope の全環境 × 全エポックのラップは 12-8 のリクエスト上限を超えうる(環境 100 × エポック多数 × 対象メンバー)ため 1 リクエストの原子性で覆えず、また不足は §6.3 のラップ先一致検査と 12-6 の追記経路で収束可能なため、複合の対象にしない
...
- 受信者の同定は **user_id と enc 公開鍵の両方**とし(不変。2026-09-19 DK 以後、同じ user_id が複数の enc 公開鍵〔端末鍵〕を持つため、公開鍵は端末を同定する)、チェーン導出の現メンバーの**有効な端末鍵**と両方が厳密一致し、**かつ対象環境がその端末の実効 scope に含まれる**(2026-09-14 ES / 2026-09-19 DK — CRYPTO_SPEC §6.2。scope 外は 422 `scope-out-of-range` — サーバー宛の開示スコープ外と同じ理由コード)でなければ受理しない。保存のスロットは **(environment_id, epoch, recipient_user_id, recipient_enc_pub_hex)**(2026-09-19 DK — 旧 (environment_id, epoch, recipient_user_id)。端末ごとに 1 スロット。上書き禁止・完全一致・不足分追記・修復経路は不変)。user_id だけでは「チェーン上の鍵と異なる鍵へのラップ」(実質ゴーストメンバー)を検出できず、公開鍵だけでは HPKE info の recipient_user_id(CRYPTO_SPEC §5)と照合できない
...
- (環境, エポック) のラップ集合の**初回登録**(環境作成時のエポック 1、ローテーション後の新エポックの一括登録)は、**R(E)(scope 内の現メンバー + 開示スコープ内の有効 grant_server — CRYPTO_SPEC §6.2。2026-09-14 ES)**との**完全一致**を要求する(§12-4 のラップ完全集合と同一の対象 — 2026-08-12 改訂。判定は受信者クラスを跨いで同一に適用する): 欠落・対象集合外宛・鍵不一致・重複はすべて 422 で拒否する
...
- **上書き禁止 409 の応答内容(2026-08-15 セッション 25 所有者裁定 — Wave 2 B2、PR #69 申し送りの解消)**: `DekWrapExists`(409)応答は `{epoch, recipientUserId}` に加えて、占有しているラップの**保存済み受信者 enc 公開鍵**を `storedRecipientEncPubHex` として載せる(MUST)。公開鍵は非機密であり(全歴史鍵はチェーンで全メンバーへ配布済み)、開示先は登録経路の認可水準(member 以上)に限られる。用途: 再追加メンバーへのバックフィルの 409 で、占有スロットが旧鍵ラップ(修復経路の削除 → 再登録の対象)か現行鍵ラップ(登録済み = 冪等)かを、クライアントが**鍵履歴からの推定でなく保存行との厳密比較**で判定する — ラップの復号可能性は受信者 enc 鍵の一致と同値(HPKE)であり、これが判定の完全な材料である。
- **再追加受理時の旧鍵宛ラップ掃除(2026-08-15 セッション 25 所有者裁定 — Wave 2 B2)**: `add_member` の受理時、サーバーは対象 user_id 宛(受信者クラス member)の保存済みラップのうち、**受信者 enc 公開鍵が追加された鍵(= 追加された端末鍵。再追加は端末 1 つから始まる — 2026-09-19 DK)と一致しないもの**を同一受理タスク内で削除する。`revoke_device` では削除しない(`remove_member` と同じく、同一鍵の再登録で復帰する既存規律 — 2026-09-19 DK)。これはチェーン導出真実へのストレージ収束(§6.3 の「ラップ先は現メンバーの鍵と厳密一致」不変条件の強制)であり、削除対象は現行チェーンの下で定義上復号不能なゴーストラップのみ — 正当な現行鍵ラップは対象にならず、上書き禁止(可用性攻撃の遮断)は不変のまま維持される。対象の履歴アクセスは追加後のバックフィル(全エポックの新鍵宛ラップ — CRYPTO_SPEC §7)が担う。削除は `dek.deleted`(actor = system、payload に原因と対応 chain seq — AUDIT_SPEC §3.3)として受信者ごとに記録する。受信者クラス server はこの掃除の対象外(鍵の変わる再 grant はサーバー鍵 FP = スロット識別子自体が変わるため stale スロットが発生しない)。remove_member 時は削除しない(同一鍵での再追加時に既存ラップがそのまま有効に復帰する — 掃除は「鍵が変わった」ことが確定する再追加時のみ)
...
- **エポック 1 とローテーションのラップ登録は 12-4 の複合リクエストに移った**ため、本節の独立登録 API が残る経路は「add_member 後の新メンバー宛バックフィル(対象の scope の環境)」「**scope 拡大後の拡大分の環境のバックフィル**(2026-09-14 — 拡大を署名した actor が全エポックを登録する。包含規則により actor は DEK を持つ)」「**grant_server 受理直後の、grant 実行者(owner)による開示スコープ内全環境 × 全エポックのサーバー宛バックフィル**(2026-08-12 — add_member 後のバックフィルと同型。CRYPTO_SPEC §7 のラップ実行者規則)」「修復経路の再登録」「**四眼経由で適用された add_member / scope 拡大(メンバー宛)・grant_server(サーバー宛)の、適用を完成させた承認者(owner)によるバックフィル**(2026-09-14 PF1 — CRYPTO_SPEC §7。受理規則は他の経路と共通: 署名者 = 呼び出し主体、受信者 = R(E)、上書き禁止)」「**`add_device` 後の、同じ人の端末による新端末宛の全エポックのバックフィル**(2026-09-19 DK — 新端末の実効 scope の環境。予備鍵を含む — CRYPTO_SPEC §7。reader の自己バックフィルは §12-3)」の 6 つである(受理規則は全経路共通のまま。R(E) は端末軸 — CRYPTO_SPEC §6.2)。**登録者(署名者)も対象環境を scope に含む**こと(DEK を持てない者の登録は構造上ありえないが、受理条件として明記。署名者 = 呼び出し主体〔上記 (1)〕なので、この軸は §12-3 の呼び出し主体の scope 判定と同一 = 403 `InsufficientScope`。scope は**署名した端末の実効 scope**で判定し、呼び出し主体の user_id と署名端末の user_id の一致は §5.1 の signer_user_id 束縛が担う — 2026-09-19 DK。受信者軸の 422 `scope-out-of-range` とは別段で、判定順は §12-3 のとおり呼び出し主体の scope〔403〕が先)
...
- 監査: var.read を**値付き一括 pull ごとに環境単位 1 行**記録する(集約形 — 返した変数の列挙〔variableId / epoch / version〕を payload に持つ。AUDIT_SPEC §3.3。2026-09-02 改訂 — 旧: 変数ごとに 1 行。返した変数が 0 なら記録しない)
...
- **メタデータのみモードは `var.read` を記録しない(「読んでいないものを読んだと記録しない」)**: `var.read` の記録条件は暗号文の配布である(AUDIT_SPEC §3.3)。メタデータのみの応答を記録すると、要ローテーション検出(AUDIT_SPEC §4)の「確実に取得した」ランクに値を読んでいない操作が混入し、検出の意味論を壊す。逆方向の規律(読んだものは必ず記録する)は一括 pull 側の「返した変数を全て payload に列挙した環境単位 1 行」が担う(2026-09-02 集約形)
...
| プロジェクト数 / org | アクティブ 100(判定点・「アクティブ」の定義・429 `ProjectLimit` は §11-3 — 2026-09-02 H2) |
| DO ストレージ総量 / プロジェクト | 警告 8 GB / 拒否 9 GB(`databaseSize` 実測 — 下記ガード。2026-09-02 H2) |
| pending 提案(四眼 — CRYPTO_SPEC §6.2) | プロジェクトあたり 32(期限切れは数えない。`expires_at_ms` の上界 = 受理時サーバー時計 + 30 日 — CRYPTO_SPEC §6.4。超過は型付き 422 `ProposalLimit`、withdraw / 適用で解放 — 2026-09-14 PF1) |
| 有効な端末数 / メンバー / プロジェクト | 16(`add_device` の受理時に判定。超過は型付き 422 `DeviceLimit`。`revoke_device` / `remove_member` で解放 — 2026-09-19 DK。CRYPTO_SPEC §6.4) |
...
- **DO ストレージ総量ガード(2026-09-02 H2 — 旧「Phase 2 予告」の本規定化。docs/notes/hosted-design.md §3-3 / §8 gap 3。本改訂を含む実装 PR のマージをもって所有者承認とする)**: 上記の個別上限に加え、プロジェクト DO の SQLite 実測量(`SqlStorage.databaseSize`)に**警告 8 GB / 拒否 9 GB の 2 段**の閾値を置く(起草値。単位は 10 進 GB = 10^9 バイト — プラットフォームの床「10 GB」が 10 進・2 進いずれの単位であっても拒否閾値がその下に来る保守側の解釈)。10 GB の SQLITE_FULL は「読み取り可・書き込み不能」の床であり、そこへ到達させないことが目的。プラットフォームは床でも素の DELETE 文を通すが、maruhi の削除操作は tombstone・削除ステートメント・監査行の INSERT を同一タスクで伴う(§12-4 / §12-5 / AUDIT_SPEC §3.3)ため床では失敗する — **床はテナント自身では解消不能**であり、運営にも DO ストレージへの介入手段がない。監査ログ(audit_events)の無期限保持(AUDIT_SPEC §5.3)を覆う唯一の防衛線である。「削除で解放される」の実効性は、DO SQLite が DELETE 後に `databaseSize` を縮めること(workerd 実測 — auto-vacuum 相当。アプリから `VACUUM` は発行できず `auto_vacuum` PRAGMA も不許可 = プラットフォーム挙動への依拠)に基づく。旧予告文は「監査ログの集約方針(AUDIT_SPEC §3.3)の実測判断と同時に設計する」としていたが、H0(hosted-design.md §3-3 — 裁定 CZ)が集約を「ドッグフーディング実測後」へ分離しガードを H2 へ前出しする裁定を確定したため、本規定はガード単独で成立させる(集約は密度〔行数の増え方〕の対策、ガードは総量の対策 — 独立に成立する)
  - **測定点と頻度**: 対象面(下記)の各受理プログラムが、メンバーシップ・role 判定と環境 / 変数の存在検査・レイアウトのサポート範囲検査の**後**(§11-2 — 非メンバーへガードの結果 = プロジェクト状態を返さない。存在の 404 はメンバー向けであり、サポート範囲超過には「更新が必要」の正直なエラーを先に立てる — §12-5 裁定 CR)・CAS・署名検証・数量ポリシー等の意味論的検査の**前**(資源保護は意味論的判定に優先 — §12-3 の判定順と同じ位置づけ)に、DO の直列化(permit)の中で `databaseSize` を読む。**毎受理で読む**(SQLite のページ数 × ページ長の即時値で I/O を伴わない — 「閾値近傍のみ読む」等の段階化は状態を増やすだけで利得がない)
  - **拒否が効く面(書き込み受理面 = プロジェクト内容の成長面)**: 値 push(§12-5)・変数の作成(declared を含む)/ activation / 改名 / スキーマ再発行(§12-5)・環境の改名(§12-4)・環境作成複合(§12-4)・DEK ラップ登録(§12-6 — バックフィル・修復再登録)・汎用チェーン追記(§11)のうちアクセス集合を拡げる `add_member` / `grant_server`(その自然な後続であるラップバックフィルが拒否対象のため、入口で揃えて拒否する — 「メンバーはいるがラップがない」中間状態を上限で量産しない)・**schemaPolicy の変更**(§12-11 — 内容の成長面ではないが、退出・解放・セキュリティ是正のいずれにも要らず、変更のたびに監査行を積む〔admin の反復で非有界〕ため含める。取得は読み取りで通る)。拒否は型付き **422 `DataLimitExceeded`**(resource = `project-storage-bytes`、limit = 拒否閾値バイト)— 本表の累積上限(`project-ciphertext-bytes`)と同じ語彙・同じ区分(413 は要求実体のサイズ、429 は主体単位の数量であり意味が異なる。既存の `DataLimitExceeded` 宣言を持たない汎用チェーン追記には本改訂で宣言を加える)。上限到達でリクエストが黙って劣化・部分受理される形は作らない
  - **拒否下でも受理し続ける面(明示列挙 — ガードが自分の解消手段・退出経路・セキュリティ是正を塞がない)**: (a) **読み取り**すべて — 一括 pull(値付き。**`var.read` の監査追記を伴うが受理する**: 拒否するとテナントが値を取り出せず退出経路〔hosted-design.md §1 約束 5〕を毀損する。値の取り出しはストレージ保護に優先する)・メタデータのみ pull・環境一覧・チェーン取得・自分宛 DEK 取得・監査読み取り・要ローテーションフラグ・schemaPolicy 取得・プロジェクト一覧の membership 確認・監査ヘッド取得(**派生列が最新のとき** — 下記の例外)、(b) **削除系** — 環境削除・変数削除・DEK ラップ削除(§12-4 / §12-5 / §12-6。削除は本表の「削除で解放される」原則の解放手段であり、ガードが自分の解消手段を塞ぐ形〔fail-closed の自殺〕を作らない)、(c) **失効・権限縮小系** — `remove_member` / `revoke_server` / `change_role`(セキュリティ是正を総量で止めない。change_role は降格の経路を含むため昇格も含めて通す — 昇格の書き込みはチェーン容量で有界。**remove / revoke の受理は要ローテーション検出〔AUDIT_SPEC §4.1〕の `rotation.recommended` を候補 (variable × environment) 対ごとに 1 行書く — 候補は対象の在籍 / grant 区間と存在期間が重なる全変数で tombstone を含むため、書き込み量はプロジェクトの変数履歴に比例し定数ではない**。上界は本表の行数上限〔変数行 5,000 / 環境 × 環境行 1,000 = 理論値 5 × 10^6 対〕で、残メンバー / grant ごとに繰り返せる。検出を切り詰めない〔上流 credential のローテーション促しを欠損させる — 裁定 CZ の棄却「監査行上限」と同型〕代わりに、下記の余裕の会計に非定数項として名指しする)、(d) **ローテーション複合**(§12-4 — remove 後の義務ローテーション〔CRYPTO_SPEC §7〕はセキュリティ是正であり、書き込み量はメンバー数 × ラップ 1 行 + マニフェスト最新 1 通 + スナップショット最新 1 通に有界。境界 checkpoint が非空公証を同梱する場合のみ下記の例外が効く。**再暗号化の値 push は拒否対象**であり、空き容量を作るまで新しい値は書けない — 一貫した帰結。既読値の取り消しは元より非保証〔CRYPTO_SPEC §14.3〕でローテーションの目的は将来値の保護にあり、拒否下では将来値も書けないため保護は毀損しない)、(e) **リース**(§14 — ワークロードの読み取り。監査・先着束縛・窓の書き込みは有界)、(f) **ヘッド申告**(§16-1 — 読み取り同期の付随・メンバーごと最新 1 行の upsert)、(g) **standalone checkpoint**(§16-2 — チェーン容量〔32 MiB〕と環境ごと最新 1 通のスナップショットで有界。空の公証 = CLI の既定〔裁定 M-b〕。非空公証は下記の例外)、(h) **rotation.dismissed**(フラグ数に有界な監査行 — 取り下げは有効フラグの存在を要し、フラグは remove / revoke の受理でのみ増える)。**例外 — 監査ヘッド派生列の実体化**: `GET /audit-head`(§16-2)と非空 audit_head_hash の checkpoint 公証(standalone・境界複合とも)は、累積ハッシュ列(AUDIT_SPEC §5.1 — 遅延実体化)が MAX(seq) に未到達で**実体化を要する場合に限り**拒否対象とする(422 `project-storage-bytes`)。実体化は監査行数に比例する書き込み(行あたりハッシュ 1 行 + 索引 — 監査表の数十 %)であり、未公証のまま拒否閾値に達したプロジェクトの初回実体化は拒否閾値〜床の 1 GB の余裕を単独で食い切りうる(下記の有界性の論証が唯一成立しない項)。列が最新なら読み取りのみで拒否下でも通り、監査行そのものの読み取り(AUDIT_SPEC §7)は常に通る。**余裕の会計**: (a)〜(h) が拒否下に書き込む量のうち定数・構造で有界でない項は 2 つ — (i) (a) の `var.read` 行(集約形 — AUDIT_SPEC §3.3、2026-09-02: 1 pull 1 行で約 250 バイト + 変数数 × 約 80 バイト〔行 + 索引の workerd 実測。識別子は CLI の実発行形 25 文字 — 長さに比例〕。100 変数の環境で約 8 KB / pull → 9 GB から床まで約 12 万回の pull。旧形〔1 変数 1 行・約 300 バイト〕では同じ環境で約 3.3 万回の pull〔= 約 330 万変数読み取り〕だった。時間比例)、(ii) (c) の失効時 `rotation.recommended`(変数履歴 × 環境に比例 — 現実的規模では数百〜数千行だが、理論上界は本表の行数上限が与える)。(e) のリース監査行(窓 300 回/時 × エポック数)は時間比例だが約 600 行/時と遅く脚注に留める。他は定数または「最新 1 行 / 1 通」の upsert。拒否閾値から床までの 1 GB の余裕がこれらを吸収し、警告(8 GB)から拒否(9 GB)までの 1 GB が運営の対応時間を、集約(AUDIT_SPEC §3.3 / 未決 4)が (i) の構造的な解を与える
  - **警告(8 GB)の形**: 運用ログのみ(H3 の監視・アラート基盤は未実装 — 通知・トリップワイヤは H3 が hosted-design.md §5-2 の「警告閾値到達プロジェクト数」として引き取る)。**観測点は拒否が効く面に加えて、拒否はしないが監査行を書く読み取り面 — 値付き一括 pull((a) の `var.read`)とリース((e))— にも置く**: 支配的な成長項が `var.read` のプロジェクトは成長面の書き込みを伴わずに 8 GB → 9 GB を通過するため、観測点を成長面だけに置くと警告帯が運営の対応時間を買う設計が pull 主体で成立しない(PR #134 レビュー)。`databaseSize` は即時値でホットパスに置いても費用は無視できる。メタデータのみ pull 等の監査行を書かない読み取りは観測点を持たない。メッセージは**静的文字列のみ**(§11-5 / hosted-design.md §5-1 — プロジェクト ID = capability〔§11-2〕・ユーザー ID 等のリクエスト由来識別子を書かない)で、DO インスタンスの生存中に警告域・拒否域それぞれ 1 回(毎受理で出すとログが書き込み計数器になる)。運営側の特定手段: Workers Logs のイベント封筒が持つ Durable Object の id(`idFromName(projectId)` の像 — 一方向であり capability を含まない)を、テナントの報告・拒否エラーの発生と突合する。ログ本文に非可逆の識別子(ID のハッシュ等)を足す形は、突合の手掛かりを一つ増やすだけで H3 の集計メトリクス(件数)に情報を足さないため採らない
  - **既存規定との関係**: 本ガードは本表の個別上限(環境 100・変数 1,000 等)の意味論を変えない(独立に適用され、いずれか一方の超過で拒否)。監査行に上限・削除を置かない(AUDIT_SPEC §1-4 — 裁定 CZ の棄却案を再導入しない)。quota 到達を監査イベントにしない(裁定 DC-3 — テナントへは型付きエラー、運営へはログ / メトリクスの 2 チャネル)。チェーン・署名・E2EE の意味論に触れない(受理ポリシー — 合意規則ではない。悪意サーバーは自分のガードを常に無視できるため、チェーン化しても敵対保証は増えない — §12-11 と同じ性格)。セルフホストでの閾値変更は自由(定数は `apps/server/src/policy.ts`。ただし床 10 GB の下に置くこと)
...
1. **未知フィールドの拒否(strict 受理)**: security-critical mutation payload の受理スキーマは、**未知フィールドを Schema 検証エラー(400)で拒否しなければならない**(既定の「黙って除去」を禁止する)。対象は**署名済み構造・暗号文・鍵材料を運ぶすべての mutation** であり、**本仕様が規定するエンドポイント**では次の全てを指す(実装の有無を問わない — 承認済み・未実装の面も実装時に本基準の対象。新設・改訂時も本基準で判定し、本列挙を更新する): チェーン追記(§11-4。genesis エントリを運ぶプロジェクト作成 = init を含む)、環境作成・ローテーション複合(§12-4)、値 push・メタ操作(§12-5 — declared 作成・activation 複合〔2026-08-30〕を含む)、DEK ラップ登録(§12-6)、リカバリーブロブ登録(§13-2)、リース請求(§14)、招待の作成・受諾(§15-2 — 受諾署名 = 公開鍵を user_id に束縛する鍵宣言クラス)、ヘッド申告の提出(§16-1 — 2026-08-19 時点で未実装。実装時に本基準の対象)。チェーン層の合意規則「未知 op = チェーン無効」(CRYPTO_SPEC §6.2)と対になる API 受理層の原則であり、公開前のため後方互換条項を持たない。これにより「旧実装へ新フィールドを送る」形の失敗方向が、黙殺して受理(fail-open)から構造的拒否(fail-closed)へ固定される。複合リクエストは 1 リクエスト = 1 decode なので、受理時拒否は原子的であり部分受理は構造的に起きない。実装は security-critical な payload スキーマを `strictPayload` で包む単一実装点で行い、ハンドラごとの手動検査を持たない。ラッパーの decode と encode は内側スキーマへ `{ onExcessProperty: "error" }` を渡す(ネストと Union を含むパース全体)。Effect v4 rc.113 以降、スキーマ AST 注釈 `parseOptions` はパーサに読まれない(渡した ParseOptions がネストと Union を含めてパース全体に適用される。rc.112 までの実証は docs/notes/session-32.md に残るが、現行の強制機構ではない)。`HttpApi.ParseOptions` は payload だけでなく、同じエンドポイントの成功・エラーの符号化と path params・ヘッダにも同じ options を渡す。`Schema.TaggedError` はスタック由来の非列挙フィールドを持つため、エラー応答をその options で符号化すると HttpApiSchemaError になり HTTP 500 へ落ちる。そのため strict は payload スキーマだけに置き、成功・エラー・params・headers には付けない。ヘッダ codec は受信ヘッダ全体を見るため、API 全体への `onExcessProperty: "error"` も使わない。共有スキーマ自体は包まない(他エンドポイントの応答へ波及させない)。実装 PR は strict の実効性を受理経路の固定テストで保証しなければならない
...
CRYPTO_SPEC §4.2 レイアウト v2 の書き込みゲート(発見 F)と schema-locked 受理(発見 E)を、**プロジェクト単位のサーバー設定 `schemaPolicy`** として一元化する(設計比較・棄却案 — チェーン op 化・環境メタ焼き込み・環境単位設定 — は docs/notes/session-46.md 裁定 CV)。値は 3 段:
...
## 13. 予備鍵ラップ台帳 API との接続(2026-08-09 セッション 18 起草。2026-09-12 KL3 改訂 — §13-6 以降を追加。2026-09-19 DK 改訂 — 予備鍵化・旧端末経路の削除・§13-11 を追加)

CRYPTO_SPEC §8(**予備鍵**ラップ台帳 — 2026-09-19 DK。旧「master 鍵ラップ台帳」)のサーバー保存・配布面の規定。台帳のラップ対象は予備鍵のブロブであり、端末鍵は台帳に載らない(端末の追加は §13-11 の要求行と CRYPTO_SPEC §6.2 の `add_device`)。台帳のすべてのラップ・分片は**サーバーから見て不透明な暗号文**であり、KEK の素材(リカバリーコード・PRF 出力・分片の平文・一時秘密鍵)はいかなる API ペイロードにも含まれない — サーバーはブロブを復号・解釈できない(ゼロ知識の維持)。他ユーザー・他文脈への移植は AAD / info の束縛により復号失敗となる(CRYPTO_SPEC §8。サーバー側の追加検査を要しない)。**§13-1〜13-5 はリカバリーコード経路(不変)、§13-6 以降が KL3 で追加された経路**。
...
- 2026-09-19 DK: ブロブの中身は予備鍵(CRYPTO_SPEC §3)
...
- ciphertext はタグ込み 16 バイト以上・**16 KiB 以下**(受理ポリシー。ブロブの実体は予備鍵(旧 master 鍵 — 2026-09-19 DK)レコードの JSON 直列化 — 数百バイト — であり大きな余裕を持つ。上限は D1 への肥大暗号文蓄積の遮断)
...
guardian_shares (
  ...
  enc_hex, ciphertext_hex,              -- HPKE(guardian-wrap 形。info は端末を含まない — 同じ s_i を端末ごとに封印)。ciphertext は 48 バイト
...
- **2026-09-19 DK**: `blob_suite / blob_nonce_hex / blob_ciphertext_hex`(旧 `source = 'device'` の同送ブロブ)は削除する(列の除去は K3 / K4 のマイグレーション)。受理ポリシー(§13-8)の「分片 1..5 / グループ」は**論理分片**(保護者数)の上限で、端末行はその 16 倍(§12-8 の端末数)まで
...
| 要求の照会(承認者) | `GET /auth/handoff/:requestId`(200) | `*` × admin トークン。呼び出し主体が ward のいずれかのグループの分片保持者(いずれかの端末鍵で)であること。**ward 本人の照会は無い**(旧端末の承認経路は削除 — 2026-09-19 DK)。**それ以外・不明・失効は一律 404**(§11-2 と同じ存在秘匿)。応答 = `{ wardUserId, wardLogin, expiresAtMs, roles: [ { groupId, mode, shareIndex } ] }`(呼び出し主体が取れる承認の形) |
| 承認 | `POST /auth/handoff/:requestId/approvals`(204) | `*` × admin トークン。`source = group_id` で当該グループの `share_index` の分片保持者のみ(照合は保存行から — 承認に使う端末鍵の FP が分片行のいずれかと一致。ワイヤ申告値で認可しない)。`source = "device"` は受け付けない(Schema 400 — 2026-09-19 DK)。同一 (request, source, share_index) の二重承認は 409。承認は 20 回 / 時 / 承認者 |
...
- 受理ポリシー(合意規則ではない — セルフホストでの引き上げ可): passkey-prf ラップ ≤ 5 / user、保護者グループ ≤ 5 / user、分片 1..5 / グループ(`all` は ≥ 2)、`params` ≤ 4 KiB、ラップ暗号文はタグ込み 16 バイト以上 16 KiB 以下(§13-4 と同じ)、分片暗号文 = 48 バイト・enc = 32 バイト(固定長)、承認 ≤ グループ数 × 5 / 要求(= 各グループの分片。2026-09-19 DK — 旧「1 + …」の device 1 件を除く。承認行の主キーと役割検査が作りうる行数の天井と一致させる)
...
GuardianShare = { shareIndex, guardianUserId, guardianKeyFingerprintHex, guardianEncPubHex, encHex, ciphertextHex }   // 端末ごとに 1 要素(同じ shareIndex が複数 — 2026-09-19 DK)
...
HandoffApproval = { source: groupId, shareIndex, encHex, ciphertextHex }                                              // blob は無い(2026-09-19 DK — device 経路の削除)
...
| 登録簿の登録・表示名の更新 | `PUT /auth/devices/:fp`(204。body: `{ encPubHex, sigPubHex, label, tokenId? }`) | `*` × admin トークン(§13-2 と同水準 — 登録簿は user 全域の自己情報〔§6 のトークン一覧・§13-7 の台帳と同じ棚〕であり、プロジェクト限定・低権限トークンには開かない。新端末は先に §4 でログインしており、既定のログイントークン〔`*` × admin〕を持つ。セッション主体は拒否)。`fp` は body の公開鍵から再計算した値と一致すること(400)。行は user あたり 32 まで(429 — §12-8 の「有効な端末数 16 / メンバー / プロジェクト」とは別の上限で、実運用上の余裕として置く: CLI がプロジェクト横断で同じ端末鍵を使う運用〔CRYPTO_SPEC §3 — チェーンは強制しない〕では有効な端末は user あたり最大 16 で、32 は失効済み・未承認の行の余裕。プロジェクトごとに別の鍵を登録する運用では登録簿の行が先に埋まりうる) |
| 登録簿からの削除 | `DELETE /auth/devices/:fp`(204 / 404) | 同上(advisory の削除 — チェーンの失効とは独立) |
| 追加要求の作成 | `POST /auth/devices/requests`(200 → `{ expiresAtMs }`。作成系の成功は HttpApi の既定 200 — §13-7 と同じ。body: `{ encPubHex, sigPubHex, label }`) | `*` × admin トークン(新端末自身 — 端末は先に §4 でログインしている)。user あたり 5 回 / 時(429)。同じ FP の要求・登録簿の既存行との衝突は 409 |
...
- 各 mutation は strict 受理(§12-10 (1))。`label` は §6 のトークン名と同じ受理規律
...
invitations (
  ...
  scope_kind      TEXT NOT NULL,      -- 'all' | 'listed'(付与予定の scope — CRYPTO_SPEC §6.2。2026-09-14 ES)
  scope_environments TEXT NOT NULL,   -- environment_id の JSON 配列(発行文の scope_environments_lp_hex の入力列。表示・add_member の入力)
...
- **scope 列(2026-09-14 ES)**: `scope_kind` / `scope_environments` を追加する。発行文(`head_hash` / `head_seq` / `role` / scope)+ `issue_signature` は招待者クライアントが `add_member` の前に検証する材料
...
- **scope(2026-09-14 ES)**: 発行 body の `scopeKind` / `scopeEnvironmentIds` は形式検査のみ(kind の閉集合・all なら空配列・256 要素以下・各 id は §12-1 形式。**存在検査はしない** — 合意規則は add_member 受理時に verifyChain が検査する)。一覧行(`InvitationSummary`)・受諾応答に scope を載せる。**role = admin の招待の発行は owner のみ**(不変)。発行者の scope が招待 scope を包含することは発行時に検査しない(add_member 受理時の包含規則が最終判定 — 発行時の状態は変わりうる)が、CLI は通信前に検査して案内する

### 15-3. 招待リンクの形式(クライアント仕様)

```
https://<web-origin>/invite#v=2&i=<invite_id>&k=<link_seed_hex>&p=<project_id>&h=<head_hash_hex>&s=<head_seq>
  &iu=<inviter_user_id>&ie=<inviter_enc_pub_hex>&is=<inviter_sig_pub_hex>&r=<role>&sk=<scope_kind>&se=<environment_id の comma 区切り>&il=<inviter_github_login>&sig=<issue_signature_hex>
```
...
- **フラグメント(`#` 以降)はサーバーへ送信されない**(不変): `i` は招待 id(クライアント採番の ULID — 発行署名が覆う)、`k` はリンク鍵の種(CRYPTO_SPEC §6.5 — 受諾クライアントは `K_priv` を導出してリンク署名を作る。永続化しない)、`p` / `h` / `s` は招待リンクアンカー(CRYPTO_SPEC §6.3 (a))、`iu` / `ie` / `is` は招待者の同一性(FP は `ie` ‖ `is` から導出 — 旧 `if` は廃止)、`r` は付与予定 role、`sk` / `se` は付与予定 scope(2026-09-14 ES — 下記)、`sig` は **発行署名**(`invite_issue_signed_bytes` — `i / p / K_pub / h / s / r / iu / ie / is / sk / se` を覆う。`k` と `il` は覆わない)。受諾クライアントは `sig` を `is` で検証してから他の何にも進まない(失敗 = 受諾しない)
...
- **`sk=<scope_kind>&se=<environment_id の comma 区切り>`(2026-09-14 ES)**: 付与予定の scope(`sk=all` なら `se` は空)。発行署名が覆う(`r=` と同じ地位)。受諾クライアントは role と scope を表示し、受諾応答の scope と食い違えばエラー。CLI: `maruhi invite create --role <role> [--env <environment-id>]…`(`--env` は反復可。省略 = all。role = owner は招待不可 — 不変)。`maruhi member add` は招待行の scope で `add_member` を署名する(`--env` は持たない)
...
- CLI: `maruhi invite create [--role <role>] [--env <environment-id>]… [--github <login>]`(招待 id の採番 → リンク鍵の生成 → 発行署名 → 発行〔発行文 + 署名を送る — scope を含む〕→ リンクの組み立てと表示 → 発行ピン〔`linkPubHex` / role / 期限 / 宛先 login〕の保存)/ `maruhi invite accept '<link>' [--from <login>] [--inviter-fingerprint <fp>]`(発行署名の検証 → 相互確認〔CRYPTO_SPEC §6.5 充足形〕→ 鍵生成〔未生成時〕→ 共同署名 → 受諾 → アンカーのピン留め)/ `maruhi member add [<invite-id>] [--github <login>] [--expect-fingerprint <fp>]`(発行ピン照合 → 両署名の再検証 → 充足形の判定 → add_member)/ `maruhi key publish [--gh]`(自分の sig 公開鍵を OpenSSH 行で表示し、`--gh` で `gh ssh-key add --type signing` を呼ぶ)。裏付け元は CLI の非機密設定 `identityBacking`(`github-signing-keys` 既定 / `none`)
...
- 配布は**現メンバーの最新申告のみ**(attesterUserId + attesterKeyFingerprintHex を付す — §12-2 の検証材料と同型)。`remove_member` 受理時に対象の申告行を削除する(CRYPTO_SPEC §6.4)。**端末ごとの申告(2026-09-19 DK)**: 保存・配布はメンバーごとでなく **(attester_user_id, attester_key_fingerprint_hex) ごと**に最新 1 行(端末は独立に同期するため、端末を跨いだ seq の単調性は課さない — 後退の 409 は同じ端末の保存行に対してのみ)。`revoke_device` の受理時に当該端末の申告行を、`remove_member` の受理時に対象の全端末の申告行を削除する。受理ポリシーの窓(下記 1 時間 60 回)はメンバー単位のまま。**申告行のサーバー受理時刻は保存してよいが配布しない**(申告が運ぶ行動情報を「チェーン同期の到達点」に限定する — CRYPTO_SPEC §6.6 / AUDIT_SPEC §6)
- 受理ポリシー(起草値 — レビューで調整): メンバーあたり**固定窓 1 時間 60 回**(超過 429)。保存はメンバーごと(2026-09-19 DK 以後は端末ごと — 上限 16 端末)最新 1 行でありストレージ肥大はない
