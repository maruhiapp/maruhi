...
- **`add_device` / `revoke_device`(CRYPTO_SPEC §6.2。2026-09-19 DK)は汎用追記 API で受理する**(付随データを持たない)。トークン水準は `add_member` 等と同じ **admin**(§6)。`revoke_device` の受理副作用 = 対象端末の申告行の削除(§16-1)・要ローテーション検出(AUDIT_SPEC §4.1)・ミラー(AUDIT_SPEC §3.4)。端末数の上限は §12-8
