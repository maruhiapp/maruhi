...
- **DO ストレージ総量ガード(2026-09-02 H2 — 旧「Phase 2 予告」の本規定化。docs/notes/hosted-design.md §3-3 / §8 gap 3。本改訂を含む実装 PR のマージをもって所有者承認とする)**: 上記の個別上限に加え、プロジェクト DO の SQLite 実測量(`SqlStorage.databaseSize`)に**警告 8 GB / 拒否 9 GB の 2 段**の閾値を置く(起草値。単位は 10 進 GB = 10^9 バイト — プラットフォームの床「10 GB」が 10 進・2 進いずれの単位であっても拒否閾値がその下に来る保守側の解釈)。10 GB の SQLITE_FULL は「読み取り可・書き込み不能」の床であり、そこへ到達させないことが目的。監査ログ(audit_events)の無期限保持(AUDIT_SPEC §5.3)を覆う唯一の防衛線である。旧予告文は「監査ログの集約方針(AUDIT_SPEC §3.3)の実測判断と同時に設計する」としていたが、H0(hosted-design.md §3-3 — 裁定 CZ)が集約を「ドッグフーディング実測後」へ分離しガードを H2 へ前出しする裁定を確定したため、本規定はガード単独で成立させる(集約は密度〔行数の増え方〕の対策、ガードは総量の対策 — 独立に成立する)
  ...
  - **拒否下でも受理し続ける面(明示列挙 — ガードが自分の解消手段・退出経路・セキュリティ是正を塞がない)**: (a) **読み取り**すべて — 一括 pull(値付き。**`var.read` の監査追記を伴うが受理する**: 拒否するとテナントが値を取り出せず退出経路〔hosted-design.md §1 約束 5〕を毀損する。値の取り出しはストレージ保護に優先する)・メタデータのみ pull・環境一覧・チェーン取得・自分宛 DEK 取得・監査読み取り・要ローテーションフラグ・監査ヘッド取得・schemaPolicy 取得・プロジェクト一覧の membership 確認、(b) **削除系** — 環境削除・変数削除・DEK ラップ削除(§12-4 / §12-5 / §12-6。削除は本表の「削除で解放される」原則の解放手段であり、ガードが自分の解消手段を塞ぐ形〔fail-closed の自殺〕を作らない)、(c) **失効・権限縮小系** — `remove_member` / `revoke_server` / `change_role`(セキュリティ是正を総量で止めない。change_role は降格の経路を含むため昇格も含めて通す — 昇格の書き込みはチェーン容量で有界)、(d) **ローテーション複合**(§12-4 — remove 後の義務ローテーション〔CRYPTO_SPEC §7〕はセキュリティ是正であり、書き込み量はメンバー数 × ラップ 1 行 + マニフェスト最新 1 通 + スナップショット最新 1 通に有界。**再暗号化の値 push は拒否対象**であり、空き容量を作るまで新しい値は書けない — 一貫した帰結。既読値の取り消しは元より非保証〔CRYPTO_SPEC §14.3〕でローテーションの目的は将来値の保護にあり、拒否下では将来値も書けないため保護は毀損しない)、(e) **リース**(§14 — ワークロードの読み取り。監査・先着束縛・窓の書き込みは有界)、(f) **ヘッド申告**(§16-1 — 読み取り同期の付随・メンバーごと最新 1 行の upsert)、(g) **standalone checkpoint**(§16-2 — チェーン容量〔32 MiB〕と環境ごと最新 1 通のスナップショットで有界)、(h) **schemaPolicy 変更・rotation.dismissed**(設定 1 行・フラグ数に有界な監査行)。(a)〜(h) が拒否下に書き込む量はいずれも既存構造で有界であり、拒否閾値から床までの 1 GB の余裕がこれを吸収する — 唯一の非有界項は (a) の `var.read` 行(1 行約 200 バイト。9 GB から床まで約 500 万回の変数読み取り)で、警告(8 GB)から拒否(9 GB)までの 1 GB が運営の対応時間を、集約(AUDIT_SPEC §3.3 / 未決 4)が構造的な解を与える
