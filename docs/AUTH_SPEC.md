...
- 2026-09-19 DK: ブロブの中身は予備鍵(CRYPTO_SPEC §3)。既存の `recovery_wraps` 行(master 鍵の複製)は移行で予備鍵のラップへ**置換**される(再発行と同じ upsert — 移行手順は docs/SELF_HOSTING.md "Updates"〔K3 の PR に同梱〕)
