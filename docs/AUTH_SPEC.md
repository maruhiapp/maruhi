...
- **承認ページの開き直しは旧ページの承認能力を失効させる(チケット置換失効の対価)**: 同じフローの承認ページを複数開いた場合(再読込・複数ブラウザ — 同一 user_id)、承認できるのは最後に開いた 1 枚のみで、先に開いたページでの承認は一様エラーになる(受容 — 複数チケットを並行有効にする複雑さより、開き直せば済む摩擦を選ぶ。最悪でも CLI の再ログインで回復する)
