...
- **コードの運搬と事前検証**: サインアップリンクは `GET /auth/github/start?signup_code=<code>`。サーバーは**開始時に事前検証**(存在・未消費・未失効 — per-IP レート制限つき)し、無効なら GitHub へのリダイレクトが起きる**前**にスクリプトなし案内ページで終了する(無効コードのために OAuth ダンスを走らせない fail-fast。独立の事前検証エンドポイントは設けない — 検証点は開始ハンドラに畳む)。有効なコードは `__Host-` クッキー(HttpOnly)に載せて callback まで運ぶ。**クッキー値は発行時の OAuth state に束縛し、callback は state 一致のときのみコードを採用する**(`__Host-` / path=/ のクッキーは同一ブラウザの後続の無関係な OAuth 完了にも同送されるため、束縛が無いと持ち越されたコードが別フローで消費されうる — PR #133 レビュー対応)。クッキーは HTML / 302 を返す callback の全終端(成功・拒否・CLI ブラウザ脚)で単回失効し、型付きエラー終端(Set-Cookie を運ばない 400 / 429)に残る値は state 束縛が無害化する(10 分の maxAge で自然消滅)。事前検証は存在オラクルにならない(256-bit 乱数・単回 — §15 招待トークンと同水準)。コードが start URL のクエリに現れること(ブラウザ履歴への残存)は受容する — コードは単回・短命・作成許可のみで、§4-1 の flowToken(PAT 取得資格 — ブラウザチャネル禁止)とは運ぶ権限の重みが異なる
