...
- **端末鍵との対応(2026-09-19 DK)**: CLI ログインで発行するトークン(既定名 `cli:<hostname>`)は 1 端末に 1 本であり、端末鍵と同じ端末に住む。端末の失効(`revoke_device` — CRYPTO_SPEC §6.2)はチェーンの事実でありトークンとは独立だが、クライアントは失効時に当該端末のトークンの指定失効(本節)を提案し(SHOULD — 登録簿の `tokenId`、無ければ既定名 `cli:<hostname>` の一致で候補を出す)、利用者が断った場合は「そのトークンは有効なまま残る」を、当該トークンの**実際の有効期限**(`GET /auth/me` の `tokenExpiresAtMs` — §16-2。既定 90 日だが発行時に最長 365 日まで延長でき、セルフホストは `DEFAULT_TOKEN_TTL_DAYS` で変えられる)とともに表示する(CRYPTO_SPEC §14.2-11 の非保証)。サーバーは `revoke_device` の受理でトークンを失効させない(登録簿は advisory で認可の入力にしない — §13-11。チェーン受理の副作用にトークン失効を置く案は、失効対象の同定を advisory な行に依存させるため採らない — 設計録 dk-design.md §6 K1-15。所有者裁定で反転しうる)。対応は端末登録簿(§13-11)の `tokenId`(任意・advisory)で持つ
