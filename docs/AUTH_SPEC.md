...
1. **開始 `POST /auth/cli/start`(未認証・無記録 — 裁定 DH)**: payload = `{ tokenName?, scopes?, expiresInDays? }`(発行パラメータの意味論は §6 — 旧 device 交換と同一。H1 のサインアップ招待コードは**本 payload には載らない** — 受理は §3 の Web サインアップのみ〔裁定 DH〕)。応答 = `{ flowId, flowToken, userCode, verificationUrl, expiresInSeconds, pollIntervalSeconds }`。**サーバーはこの時点で何も保存しない** — フローの真正性は署名(下記)で無状態に検証する
   ...
   - `verificationUrl` は flowId・期限・userCode・**発行パラメータ(tokenName / scopes / expiresInDays — 承認ページの表示と行作成に必要)**を運び、これら全体を覆う**ブラウザ脚用 MAC(vsig)**を付す。vsig は flowToken とは独立の値で、URL の知識はポーリング資格を一切与えない。vsig の無い / 改竄された / 期限切れの verify 到達は、GitHub へのリダイレクトが起きる**前に**一様なエラーページで終了する(でっち上げの flowId のために OAuth ダンスを走らせない — fail-closed)
