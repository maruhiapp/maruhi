...
- **コードの運搬と事前検証**: サインアップリンクは `GET /auth/github/start?signup_code=<code>`。サーバーは**開始時に事前検証**(存在・未消費・未失効 — per-IP レート制限つき)し、無効なら GitHub へのリダイレクトが起きる**前**にスクリプトなし案内ページで終了する(無効コードのために OAuth ダンスを走らせない fail-fast。独立の事前検証エンドポイントは設けない — 検証点は開始ハンドラに畳む)。有効なコードは `__Host-` クッキー(HttpOnly)に載せて callback まで運ぶ。事前検証は存在オラクルにならない(256-bit 乱数・単回 — §15 招待トークンと同水準)。コードが start URL のクエリに現れること(ブラウザ履歴への残存)は受容する — コードは単回・短命・作成許可のみで、§4-1 の flowToken(PAT 取得資格 — ブラウザチャネル禁止)とは運ぶ権限の重みが異なる
