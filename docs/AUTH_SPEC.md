...
1. **開始 `POST /auth/cli/start`(未認証)**: payload = `{ tokenName?, scopes?, expiresInDays? }`(発行パラメータの意味論は §6 — 旧 device 交換と同一。H1 のサインアップ招待コードは本 payload への添付として設計する — hosted-design.md §2-2)。応答 = `{ flowId, flowToken, userCode, verificationUrl, expiresInSeconds, pollIntervalSeconds }`
   ...
   - **行の寿命と総量の有界化**: start は未認証で D1 に行を作る面であり、per-IP 制限は per-colo の best-effort なので、それだけでは総量が有界にならない。よって (a) 期限切れ・consumed・denied の行は**日和見削除**する(start の挿入処理が期限切れ行の削除文を同じ batch に同梱する — §5 セッションの「resolve 時の掃除」と同じ非イベント扱いで、監査には記録しない)、かつ (b) **デプロイメント全体の同時 pending 上限**(起草値 1,000 行)を置き、上限判定と挿入は単文の `INSERT … SELECT … WHERE` で原子的に行う(§6 の発行上限と同じ S7 原子化パターン。超過は 429)。上限は「TTL 15 分 × 正規の最大流入」に対して十分上であり、正規ユーザーが触れる値ではない(§6 / §12-8 / H2 と同じく未認証・準未認証の作成面には例外なく明示上限を置く、の一貫)
...
4. **承認**: OAuth 完了 → get-or-create(§3-3。H1 導入後は signupPolicy ゲート・サインアップ招待コードの消費もこの点 — 判定点は不変)→ サーバーは**承認ページ**を返す。ページは userCode に加えて**このフローが発行する PAT の付与内容 — tokenName・要求スコープ・有効期限(expiresInDays 反映後)— を人間が読める形で表示し**(値はフロー行に既にあり、新しいワイヤ要素は不要。start の payload は未認証で呼び出し側が選ぶため、承認者が付与内容を一度も見ずに承認する形を許さない — §4-3 のフィッシング残余下では旧 device flow より改善できる位置にある)、「**CLI に表示されたコードと一致する場合のみ承認**」の明示操作を要求する(拒否操作も併置)。承認の資格はページに埋め込む**単回・短命の承認チケット**(256-bit 乱数・ハッシュのみ保存・OAuth 完了と同一トランザクションで発行)であり、セッションではない。承認はフロー行の pending → approved(+ 認証済み user_id の束縛)への CAS。ページの配信規律は §15-3 の招待着地ページと同一(**スクリプトなし** — フォーム POST のみ・CSP `script-src 'none'`): 承認面が github.com から maruhi オリジン(Web = TCB)へ移る対価を、スクリプト面ゼロで最小化する
