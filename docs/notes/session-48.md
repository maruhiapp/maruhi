...
- H3 トリップワイヤへ「token 請求の自前カウント」と「ログインフロー行の退避・上限到達」を追加(hosted-design.md §3-4 追記 — secondary は残量観測 API がないため自前計数が唯一の観測手段。退避は正規運用で起きない事象 = 洪水の検知)。check-token 監視は消滅
