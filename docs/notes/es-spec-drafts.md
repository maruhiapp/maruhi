...
>   - **原則 1 — 義務の履行可能性(認可段。2026-09-14 — 設計録 3-ter)**: エントリが §7 / AUTH_SPEC §12-6 の義務(rotate・バックフィル)を生むとき、**actor の scope はその義務の環境集合を包含**していなければならない(拒否理由 `scope-not-contained`。`all` は全てを包含し、`listed` は `listed` の部分集合のみを包含する — `listed` は `all` を包含しない)。根拠は暗号的必然: DEK のラップは DEK 保持者が行う(§7)ため、環境 E へ人を入れられる(バックフィル)のも、E から人を外せる(rotate に旧 DEK が要る)のも E の DEK を持つ者だけである。op ごとの**義務の環境集合は §7 / §12-6 の義務定義から導出**する(列挙ではない — 義務が増えれば本規則は自動的に伸びる): `add_member` = 新 scope(バックフィル)/ `change_role` = (新 \ 旧)〔バックフィル〕∪ (旧 \ 新)〔rotate〕∪(member 未満への降格なら新 scope)〔rotate〕/ `remove_member` = 現 scope(rotate)。変化のない環境に義務はないため、`change_role` で旧 ∪ 新 全体を要求する形(2026-09-14 の一時的な訂正)より正確で、dev 専任 admin が {dev, prod} のメンバーの role を prod に触れずに変えられる。owner は all のため常に通る
...
>   - **原則 2 — 署名者集合による認可(2026-09-14 — 設計録 3-ter)**: すべてのエントリは**署名者集合 S** で認可される。直接追記では S = {actor}、提案経由では S = {提案者} ∪ {`approve` の actor 全員}。op に必要な承認数 `required(op)` は、方針が有効かつ op が対象なら `required_approvals`、それ以外は 1。エントリ(直接追記)または提案の適用は、**適用時点で owner である S の distinct な要素数 ≥ required(op)**(required = 1 のときは通常の role 規則で足りる)のときに限り有効。**不変条件(方針の単調性)**: 方針が有効な間、owner 集合または方針を変えうる op(`set_approval_policy`、owner role を確立する `add_member` / `change_role`、owner を減らす `remove_member` / `change_role`)は `ops` の列挙に依らず常に対象であり、owner 数を `required_approvals` 未満にする op は無効 — 定足数未満の署名者で方針を弱める・迂回する経路を持たない。以下の個別規則はこの原則と不変条件からの導出である
