...
**位置づけ**: KL3 フェーズ 1(設計セッション)の成果物。設計の全体像・裁定の反復記録・実装分割は docs/notes/integration-options.md §3 補足 19。本ファイルは正本 3 文書(CRYPTO_SPEC / AUTH_SPEC / AUDIT_SPEC)へ反映する**改訂本文の案**であり、承認前に正本は書き換えない。承認 = 本ドラフトを正本へ反映する仕様 PR(K1)のマージ。承認後の版番号は CRYPTO_SPEC 0.9-draft / AUTH_SPEC(現行 +1)/ AUDIT_SPEC 1.6-draft を仮置きする(IV1 は次番)。
