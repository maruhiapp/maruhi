...
>
> - **リンク鍵**: 招待者クライアントは招待ごとに 32 バイトの一様乱数 `k`(種)を生成し、Ed25519 鍵ペア `(K_priv, K_pub)` を導出する(§3 の署名鍵と同じ導出 — 種 = 秘密鍵の seed)。`K_pub` を発行 API へ渡し(AUTH_SPEC §15-2)、`k` はリンクのフラグメントにのみ載せる。**サーバーは `k` / `K_priv` を一度も受け取らない**。招待者クライアントは `k` を永続化せず(表示 = リンクの組み立て直後に参照を捨てる)、`K_pub` を発行ピン(非機密ローカル状態 — AUTH_SPEC §15-3)に保持する
> - **発行署名**: 招待者はリンクの内容にチェーン署名鍵(Ed25519)で署名し、リンクに載せる:
>
>   ```
>   invite_issue_signed_bytes = LP("maruhi/v1/invite-issue",
>                                  project_id, link_pub_hex, head_hash_hex, head_seq, role,
>                                  inviter_user_id, inviter_enc_pub_hex, inviter_sig_pub_hex)
>   ```
>
>   - エンコーディングは §2.1(バイナリは hex 小文字、数値は 10 進文字列)。検証鍵は署名対象内の `inviter_sig_pub_hex`(自己束縛 — §5.1 / 旧 §6.5 の受諾署名と同型)であり、検証の成立は「`inviter_sig_pub` の秘密鍵の保持者がこのリンク(この link_pub・このヘッド・この role)を発行した」ことの帰属。招待者が**誰か**は裏付け元(下記)または帯域外照合が示す
>   - 意味論: リンク経路上の改竄(アンカー・role・招待者鍵の差し替え)を検証失敗に落とす。招待者の公開鍵を自分のチェーンへゴースト追加した第三者は招待者名義のリンクを作れない(逆方向フィッシングの機械的な遮断 — 補足 21 裁定 E)
>   - 受諾クライアントは発行署名の検証失敗を**受諾しない**理由とする(帯域外照合で上書きしない)
...
>   - サーバーは受諾時に両署名を検証する(AUTH_SPEC §15-2)。招待者クライアントは `add_member` の前に、一覧行の `link_pub_hex` が**自分の発行ピンの `K_pub` と一致する**ことを確認した上で(不一致 = 行のすり替え)、両署名を独立に再検証する。**発行ピンを持たない端末は `K_pub` を固定できないため IV1 の検証は成立せず**、下記の充足形 1〜3 へ戻る
...
>   - 裏付け元へ送る情報は **login のみ**(プロジェクト・鍵・値・利用状況を送らない)。ホストは固定(`api.github.com`)。応答は公開情報として扱い、照合に**失敗**(鍵が無い)しても**不能**(取得できない・上限・オフライン)でも、下記の充足形 1〜3 へ戻る(fail-closed: 裏付け元は儀式を**省く**根拠にしかならず、儀式を**免除しない**方向へは働かない)
...
>      - 招待者側: (i) 一覧行の `link_pub` が発行ピンの `K_pub` と一致し、(ii) リンク署名・受諾署名の検証に成功し、(iii) 裏付け元が「受諾の sig 鍵は、発行時に名指しした相手(`invite create --github <login>` — 発行ピンに保持。無ければ実行時の `--github <login>` / 対話入力)の鍵である」と照合できた — このとき **確認入力なしに** `add_member` へ進んでよい。名指しは招待の発行時に行われた明示的作為であり、受諾単位の同意はそこで表明済みである
...
>      - (ii) の暗号検証の**失敗**(`K_pub` 不一致・いずれかの署名の失敗)は充足形 1.〜3. へ**戻さず拒否**する(壊れた署名を人間の読み上げで上書きしない)。(i)(iii) の**不能**(発行ピンの無い端末・裏付け元 `none`・未登録・取得不能)は 1.〜3. へ戻る
...
> - **サーバーは招待の秘密を一切保持・返却しない**: 行に載るのはリンク**公開**鍵で、リンク鍵の種はサーバーを通らない(発行応答にもトークン相当の値は無い)。行が漏れても受諾は偽造できない(旧 token_hash と同じ「検証できるが作れない」性質)
...
> | op | エンドポイント | 認可 |
> |---|---|---|
> | 発行 | `POST /projects/:projectId/invites`(body: `{ role, linkPubHex }`。応答: `{ id, role, expiresAtMs }`) | トークンスコープ admin × チェーン role admin 以上(**role = admin の招待の発行は owner のみ**)。**セッション主体は拒否**(§5 — 発行はリンク鍵の生成と発行署名を要し、鍵なし Web には置けない — ADR-0018 改訂 2)。`linkPubHex` は形式検査(32 バイト hex)のみ・UNIQUE 違反は 409(クライアントは種を生成し直す) |
...
> - **一覧行**(`InvitationSummary`)は `tokenHashHex` を `linkPubHex`(旧行は null)に置き換え、受諾ブロックに `linkSignatureHex` を足す。招待者クライアントはこれを発行ピンの `K_pub` と突合し(不一致 = 行のすり替え → add_member しない)、両署名を再検証する
...
> - CLI: `maruhi invite create [--github <login>]`(リンク鍵の生成 → 発行 → 発行署名 → リンクの組み立てと表示 → 発行ピン〔`linkPubHex` / role / 期限 / 宛先 login〕の保存)/ `maruhi invite accept '<link>' [--from <login>] [--inviter-fingerprint <fp>]`(発行署名の検証 → 相互確認〔CRYPTO_SPEC §6.5 充足形〕→ 鍵生成〔未生成時〕→ 共同署名 → 受諾 → アンカーのピン留め)/ `maruhi member add [<invite-id>] [--github <login>] [--expect-fingerprint <fp>]`(発行ピン照合 → 両署名の再検証 → 充足形の判定 → add_member)/ `maruhi key publish [--gh]`(自分の sig 公開鍵を OpenSSH 行で表示し、`--gh` で `gh ssh-key add --type signing` を呼ぶ)。裏付け元は CLI の非機密設定 `identityBacking`(`github-signing-keys` 既定 / `none`)
...
> - **発行ピン(招待者側の非機密ローカル状態)**: `invites/<projectId>.json` の `issued[<invite id>] = { linkPubHex, role, expiresAtMs, expectedGithubLogin? }`。`linkPubHex` は IV1 の検証の真実源であり(サーバー申告の `link_pub` と突合する)、ピンの無い端末では第 4 形は成立しない(儀式へ劣化 — CRYPTO_SPEC §6.5)。**リンク鍵の種・秘密鍵はピンに書かない**
