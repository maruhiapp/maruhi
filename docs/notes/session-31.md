...
- **M1 の暫定方式 = スナップショット行の追記(書き直さない)**
  ...
  破るため撤回): サイズ閾値を超えたら「現在の fold 結果 + 自分が
  畳んだ接頭辞の終端位置」を**スナップショット行としてログへ追記**
  する。fold は「最新スナップショット行の状態 ⊔ その終端位置より
  後の全行」— join の冪等性・可換性により、位置情報が無い / 壊れて
  いる場合は全行 fold へフォールバックしても正しさは変わらない
  (コストだけが変わる)。**同座標 conflict の証拠行は fold 結果に
  証拠として残る**(スナップショットに畳まれても消えない)
