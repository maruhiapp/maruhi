...
**最終推奨: 1-E(strict 受理)+ 1-E′(成功定義の昇格)+
...
**最終推奨: 所有者がベクター再生成 + マニフェスト再発行の一時コストを
受け入れるなら案 2-E、受け入れないなら案 2-D**。保証水準はどちらも
本 finding を塞ぐ(2-E がわずかに強く、恒久的に単純)。どちらでも
「既定 = 厳密・例外は明示材料の提示のみ」の規範は同一。
...
**最終推奨: 意味論 = 案 3-D(単調 join)、保存形 = 案 3-E
...
> (strict 受理 + 成功定義の昇格 / 複合束縛の明文化〔2-D か 2-E かは
> 所有者裁定〕/ 床の単調 join + append-only 保存形)は各裁定の
> **最終推奨**を基に仕様書の改訂を先に起草し、Status 行と PR 本文の
> 「要裁定」で所有者裁定を仰いでから実装する。`packages/crypto`
> 変更はベクター先行・人間レビュー必須。隣接 cleanup や M2〜M4 へ
> 広げない。
