...
- 人間の規律が残る面は非署名の運搬部分のみ: 座標は §12-5 の
  サーバー側再構成が担保済み、`reencryption` 等の advisory flag は
  設計上 security-critical でない — 残余は実質空集合
...
1. **案 2-F**(推奨): チェーン re-genesis を許容できるなら最強 —
   検査規則が最少で、エポック境界の発行時アンカーという恒久的な
   構造利得が付く。maruhi のこれまでの選択(公開前に払って恒久的に
   単純へ)と一貫する
...
コンパクション方針(3-E の唯一の残課題を M2 と接続して解く):

- **検証済み checkpoint 基準以下の観測行を切り詰める**。チェーンが
  基準を恒久保持するため、ローカルログの成長は checkpoint 周期で
  有界になる。コンパクションのロックが取れなければ追記を続ける
  (fail-safe)
