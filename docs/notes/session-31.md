...
修正案(2026-08-19 改訂 — §7 裁定 1 の最終推奨〔1-E + 1-E′〕に
追随。当初の capability 必須ゲート設計〔旧修正案 1〜3〕は §7 で
任意 UX へ格下げ済みのため置き換える。裁定 3 と同じ粒度の読み替え):

1. **主防衛 = strict 受理(1-E)**: security-critical mutation
   payload の受理を未知フィールド拒否にする(公開前の全面適用)。
   最初の作業は Effect v4 `onExcessProperty` の HttpApi 受理経路への
   適用方法の確認(§7 裁定 1 の実装コスト行)
2. **成功の定義 = 検証可能な効果確認(1-E′)**: create / init rotate
   の受理後は、チェーン / metadata-only pull で配布されたマニフェストを
   取得し、自計算した `(version, epoch, signed-bytes hash)` と照合して
   から床へ記録・成功報告する(確認材料の種別対応と値 push の適用外は
   §7 裁定 1 第 3 次)
3. capability(`/auth/config` の `apiCapabilities` 等)は**任意の
   UX 追加**(送信前の親切な誘導文言)であり、防衛としては数えない。
   後続 PR でよい

固定テスト(§9 DoD の 2 構成と同一):

- strict 導入後のサーバー: 未知フィールドを含む複合 payload を
  decode 段で拒否
- strict 未導入の旧サーバー: サーバーが 200 でも pull で manifest
  欠落 → 床を前進させず失敗(受理後照合)
...
5. M1-A2 修正案 2(1-E′ の受理後照合 — チェーン / metadata-only pull
   での効果確認)と同じ確認経路へ統合
...
- M1-A6(§7 裁定 2 のはしご — 2-F / 2-E / 2-D、第 5 次で 2-H・第 6 次で
  2-G′〔推奨〕を追加 — docs/notes/session-32.md §4-2 / §5-1 — から
  所有者が選ぶ)
- M1-B1
...
**第 5 次・第 6 次(2026-08-19 — 本ノートの追記後)**: 案 2-H(専用アンカー op の
複合原子同梱)の追加は docs/notes/session-32.md §4-2、案 2-G′(checkpoint の
複合原子同梱 — 2-G の棄却理由の誤りを訂正して再生)は同 §5-1 を参照。
はしごは **2-G′(推奨)** / 2-H / 2-F / 2-E / 2-D になった。
...
- read-modify-write がクリティカルパスから消える。M1 の運用では
  プロセス間ロック自体が不要になる(コンパクションもスナップ
  ショット行の**追記**で行い、物理回収は M2 まで持たない — 下の
  コンパクション方針)。証拠はログが伸びる方向でしか変化しない
...
- **M1 の暫定方式 = スナップショット行の追記(書き直さない)**
  (2026-08-19 pullfrog レビュー反映 — 当初の「ロック下で書き直す」
  方式は、ロックフリー追記との競合でコンパクション実行中に着地した
  観測を無言で消し、3-E の中心的不変条件をそれを守るための機構で
  破るため撤回): **最新スナップショット行以降に積まれた量**(行数 /
  バイト数 — 総ファイルサイズではない。2026-08-19 pullfrog レビュー
  反映: ログ全体は単調増加なので総サイズ基準は一度超えると恒久成立し、
  床書き込みのたびにスナップショットが積まれてしまう。相対量は
  スナップショットごとに 0 へ戻り、契機が再武装する)が閾値を
  超えたら、「現在の fold 結果 + 自分が畳んだ接頭辞の終端位置」を
  **スナップショット行としてログへ追記**する。fold は「最新
  スナップショット行の状態 ⊔ その終端位置より後の全行」— fold
  コストの有界化はこの相対基準そのものが担う(スナップショット
  以降の未畳み分が常に閾値以下)。join の冪等性・可換性により、
  位置情報が無い / 壊れている場合は全行 fold へフォールバックしても
  正しさは変わらない(コストだけが変わる)。**同座標 conflict の
  証拠行は fold 結果に証拠として残る**(スナップショットに畳まれても
  消えない)
- **直列化規則(一意): M1 のログは追記しかしない**。通常の観測
  追記とスナップショット追記はともに O_APPEND のみ(ロック不要 —
  並行スナップショット 2 本は無害: fold は最新の 1 本を使い、
  join の冪等性で二重畳みも正しい)。**ファイルサイズの物理回収は
  M1 では行わない** — 書き直し・切り詰めはロックフリー追記と原理的に
  競合する(排他を取っても追記側が参加しなければ効かない)ため、
  M1 の機構から排除する。fold コストはスナップショット行が有界化し、
  ファイルサイズの成長は規模感(下記)から M2 まで受容する。
  物理回収は M2 の checkpoint 基準移行と同時に、追記側の共有ロック
  参加を含めて設計する
...
コンパクション = M1 は閾値契機のスナップショット行方式・M2 到達後に
checkpoint 基準連動へ移行**。第 4 次で構造の変更は不要と確認 —
残余(初回クライアント・使い捨て CI の床不在)は設計どおり
M2 / M4 の領分。
...
置き換わる** — M1 ではプロセス間ロック自体が存在しない(追記のみ。
コンパクションもスナップショット行の追記で、物理回収は M2 の設計
事項)。fail-open の懸念も消える: 追記は証拠を失わないので
「床なしへの fail-open」に相当する状態が生じない。§3 の同座標
typed conflict(修正案 3 / 4)と再検査(修正案 5)は fold 側の
規則としてそのまま生きる。§6 PR-F2 の分割 3 は「floor 保存形の
追記専用化 + fold」と読み替える。3-E を採らない場合のみ §3 の
記述が原文どおり生きる。
