...
**推奨: 案 1-A を主防衛、案 1-C を二層目として併用し、
案 1-D を新設スキーマから適用する**。1-A が入口(送信前)を塞ぎ、
1-C が出口(受理結果)を検証する二層で、どちらか単独より強い。
1-B は不採用。capability 名は `environment-manifest-v1` を起草値
とし、粒度は「ワイヤ形式が変わる機能単位」とする。
...
**推奨: 案 2-A**。案 2-B は既裁定の蒸し返し(CLAUDE.md が禁じる形)
であり、得られる単純化に対して移行コストと非対称の導入が見合わない。
...
**推奨: 案 3-A**。案 3-B は監査意味論との衝突が本質的
(full pull を自動化する限り回避不能)で筋が悪い。実装の複雑化は
M1-A3 の型の選択肢(判別可能な部分床型)で吸収する。
