...
追記(PR #99 レビュー対応): 完了の節目は**再開経路**(resumeReencryption)にも
等しく適用する — 初回実行がクラッシュした場合、発行へ到達していないのは再開側で
完了した実行だけだからである(Bugbot 指摘で実装漏れを修正)。残る未カバー窓は
「最後の push 成功後・checkpoint 発行前」のクラッシュで、次の実行は up-to-date
経路(未完了なし)に入り発行しない。これは**記録済みの境界**とする: up-to-date
経路からの発行は「確認だけの実行」のたびに checkpoint を打つことになり(§6.3 の
周期性と乖離した無条件発行)、窓の閉じ方として過剰である。この窓は契機 (iii) の
7 日 staleness 提案が拾い、§6.3 の発行は SHOULD なので隙間は仕様上許容される
(pullfrog 増分レビューの指摘どおり、変更なし・境界の記録のみ)。
