...
- チェックポイント: 受理検証(保存突合 — 偽公証の拒否・削除済み環境エントリの
  拒否)・スナップショット同梱検証(per-variable 非後退・欠落 = tombstone
  なしの消失の拒否・列挙省略の拒否)・監査累積ハッシュの維持と admin 照合
  フロー(所属 + 位置 — **非前進ヘッドの連続公証**〔陳腐化リプレイ〕が位置
  検査で検出されることを含む — ループ 3 第 2 ラウンド指摘 1)・チェック
  ポイントより古いマニフェスト配布の拒否・基準なし環境の値付き配布での警告
