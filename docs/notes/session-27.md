...
- **合意規則(チェーン有効性)は形式と座標整合のみ**: payload 構造(hex 長・
  ...
  チェーン導出現エポックとの厳密一致。manifest_sig_hash / values_digest /
  audit_head_hash の**内容**はチェーン検証では検証不能(§5.2 DEK コミット
  メントの「形式は合意規則、内容は照合側」と同じ線引き)
- **サーバー受理検証(受理ポリシー)**: manifest_sig_hash が保存済みの当該
  manifest_version のマニフェストと一致、values_digest が保存行から再計算した
  値と一致、audit_head_hash が保存済み累積ハッシュと一致。過去の
  manifest_version の公証も許す(単調検査のみ)— 並行メタ操作との 409 競合を
  作らない。偽公証(存在しない状態への署名)の持ち込みは受理段で落ちる
- **クライアント検証(チェックポイントの消費)**: 検証済みチェーンの最新
  チェックポイントに対し、(i) 配布マニフェストの manifest_version が
  チェックポイント以上、等号なら sig_hash 一致(巻き戻しの拒否)、(ii) pull /
  lease 応答に同梱される「チェックポイント時点の値スナップショット列挙
  (variable_id, version, value_sig_hash)」のダイジェスト = チェーン上の
  values_digest を照合した上で、配布された各変数の version がスナップショット
  以上・等号なら value_sig_hash 一致・スナップショットにあって配布にない変数は
  検証済み tombstone(マニフェスト整合込み)がない限り欠落として拒否。
  **これで床なしクライアント(アンカーでチェーン鮮度を得た後)に、チェック
  ポイント時点までの値・メタの巻き戻し・欠落・差し替えの検出が初めて届く**
...
- 追加の negative(authorization 類型): `authz-checkpoint-reader`(role
  ...
  (エントリ時点の現エポックと不一致)。payload 構造検査(認可段より前 —
  §6.2 の段順): hex 長不正・環境エントリの重複 ID・昇順違反の扱い
  (scope_environments と同じ「生成は昇順 SHOULD・検証は集合」なら昇順違反は
  positive — 意図をベクターで固定)
