...
- 裁定 1: 本 PR のマージ = 承認
- 裁定 2: はしごは §4-2(第 5 次 — 2-H 追加)→ §5-1(第 6 次 — 2-G′ 再生)で
  改訂。**最終推奨 = 2-G′**、はしご = 2-G′ / 2-H / 2-F / 2-E / 2-D。
  公開前限定の選択肢は 2-F / 2-H / 2-G′ / 2-E
- 裁定 3: session-31 の最終推奨(意味論 = 3-D 単調 join、保存形 = 3-E
  ...
  加えた形の確認待ち(§5-3 で天井を確認済み)。承認されれば裁定 2 の確定と
  合わせて §6.3 床節の改訂を起草する
...
- **2-H との決定的な差**: 境界ごとに**値込みの完全な基準**が即時に張られる。
  ...
  **残余の窓そのものが消える** — 床なしクラスへのチェックポイント整合
  (規則 1 + 2 = マニフェストと値の両方の非後退・エポック基準)が
  全エポック境界から常時有効になる。2-H(anchor はマニフェストのみ)にも
  2-F にも無い利得
...
- 構築順序: pull(再暗号化に必要な読み取りと同一)→ manifest 署名 →
  ...
  ラップの原子受理。値の内容突合の 422(pull と受理の間の並行 push)は
  checkpoint の既存の有界再試行と同じ扱い
- 移行・更新順序: 2-H と同一(旧 CLI は checkpoint op を未知 op として
  拒否する fail-closed。CLI / CI 先行更新 → 全環境の移行 rotate)
- AUTH_SPEC 側の改訂面: §12-4 複合の同梱物へ checkpoint エントリを追加、
  §6.3 の発行 SHOULD (i) に「境界 checkpoint は複合の必須同梱(カバー =
  当該環境のみ)、再暗号化完了後の checkpoint は従来どおり」の区分を追記
...
- **実装ガード(PR-F1 への指示)**: `strictPayload(...)` はロード時に
  ...
  (固定テストの要件は据え置き — 二重の防衛)
- 1-E′ の値 push 除外の将来解除オプション(記録のみ・本シリーズでは
  採らない): 値の (variable_id, version, value_sig_hash) listing
  (暗号文なし)は `var.read` の記録条件(= 暗号文の配布 — §12-7)に
  該当しない読みが可能で、push の効果確認・3-F intent 解決に使える。
  ただし新しい配布クラスの監査意味論(値の変更タイミングの可視化)の
  検討を要するため、必要が生じた時に独立に裁定する
