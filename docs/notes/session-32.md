...
- マニフェスト検証が単一検査になる: epoch = 宣言ヘッド時点の現エポック
  (strict)、**または検証済みチェーン上の anchor と (environment_id, epoch,
  manifest_version, manifest_sig_hash) が完全一致**。actor = issuer 規則は
  不要(2-F と同じ論法: 投機的相乗りは hash を予知できず、事後の相乗りは
  自分のマニフェストの hash がアンカーに載っていない)
- 2-F の波及効果を全て保持する: エポック境界の発行時チェーンアンカー、
  M1-A4(受理確認 = チェーン同期だけで床コミット材料が揃う)、M1-A3
  (env create 直後の床のマニフェスト記録がチェーン導出で確立)、1-E′
  (複合の成功確認がチェーン同期で完結)。**加えて 2-E が防げない形を防ぐ**:
  2-E の束縛は manifest → entry の向きなので、同一 issuer(鍵漏洩・強制)が
  同じ composite_entry_hash を参照する 2 通のマニフェストを発行する
  equivocation が可能なまま残る。2-H / 2-F はチェーンが 1 つの hash を
  名指しするため、境界マニフェストの equivocation が全クライアントに
  チェーン公開値として検出可能になる
...
- 更新順序: 旧 CLI は anchor op を含む新チェーンを「未知 op = チェーン無効」で
  拒否する(fail-closed 方向で安全)。サーバー先行更新は §12-10 /
  session-28 §2-2 の既存順序と同じ
