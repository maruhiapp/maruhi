...
- **2-E に対する検出差(2026-08-19 pullfrog レビュー 2 巡で縮小 —
  ...
  よって 2-H / 2-F が 2-E に対して新たに閉じるのは **(i) アンカーを持つ版
  (= 境界マニフェスト)の equivocation のみ**と書き切る。
  境界跨ぎの巻き戻し配布は、本則の単調性論法そのものにより必ず epoch の
  後退であり、床を持たないクラスにも既存のリポジトリアンカー受理規則
  (§14.3 (b) — 環境エポック ≥ アンカー)が既に落とすため、差として
  二重計上しない。規則 3 の価値は検出クラスの獲得ではなく、
  **(a) アンカーを持つ版への照合(規則 1)を下方回避できなくする下限**と
  **(b) M2 checkpoint の周期的な非後退カバーに対する境界即時の基準供給**に
  ...
  床・基準を持つクライアントは 2-E でも §6.3 床規則 (b) /
