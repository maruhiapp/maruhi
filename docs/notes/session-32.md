...
- 2-F の波及効果を全て保持する: エポック境界の発行時チェーンアンカー、
  ...
  equivocation が可能なまま残る。2-H / 2-F(上の MUST 形)はチェーンが
  1 つの hash を名指しするため、境界マニフェストの equivocation が
  **床・チェックポイント基準を持たないクライアント(§9.1 のワークロード・
  初回同期 — §14.3-3)を含む全クライアント**にチェーン公開値として
  検出可能になる(床・基準を持つクライアントは 2-E でも §6.3 床規則 (b) /
  チェックポイント整合規則 1 で検出できる — 差が出るのは床なしクラス)
