...
| **オープンベータ** | 自由(レート制限つき) | 無料 + quota(§3) | 全員 | 招待制ベータでの quota 実測 + GitHub クォータ余裕の確認(§3-4)+ CLI ログインのスケール経路の裁定(§8 gap 9) |
...
- **プロバイダ識別子の許可リストは作らない**: GitHub login(可変)・GitHub ID による
  ...
  メール allowlist は「メールを識別子に使わない」(AUTH_SPEC §1)に反する。招待コードは
  どちらの識別子にも依存しない
...
- **拒否経路の共有クォータ消費の遮断(2026-08-30 PR #113 Bugbot レビュー対応 —
  ...
  拒否されるログイン試行も枠を燃やす。H1 の設計要件: (i) CLI は device flow を**開始する前**に
  `/auth/config` の `signupPolicy` を確認し、`invite` なら「既存アカウントの再ログインか、
  招待コードの提示か」を確認してから開始する(fail-closed の誤操作ガード — 認可ではない。
  受理の正はサーバーのまま)。(ii) 未招待の新規希望者には device flow を開始せず waitlist /
  招待の案内を出す。(iii) H3 のトリップワイヤに「サインアップ拒否の計数」を加える(§5-2 —
  拒否は交換時にサーバーへ到達するため観測可能)。**残余(受容)**: 敵対的な枠の枯渇は
  maruhi を介さず可能(client_id は公開情報で、device flow の開始とコード入力は
  github.com に対して直接行える)— サーバー設計では防止不能であり、恒久解は device flow への
  依存自体を除く gap 9(web-flow ハンドオフ)。CLI の事前確認が消すのは善意の無駄打ちのみ
...
DO 内の个別上限(AUTH_SPEC §12-8: 累積暗号文 1 GiB・環境 100・変数 1,000/環境 等)は
...
| web OAuth callback(サーバー → code 交換 + /user + /user/emails) | 2,000 回/時(token 請求)+ 5,000 回/時 | ログイン 1 回 = token 請求 1 + API 2〜3 |
...
| L3 | 利用規約(ToS)・プライバシーポリシー(GDPR / 各国法制の適用整理を含む)・許容利用ポリシー(AUP)。ゼロ知識の線引き(§1)と脅威モデル(§6)を法文書の記述と整合させる | 招待制ベータ前 |
...
| # | 対象 | 内容 | 段 |
|---|---|---|---|
| 1 | AUTH_SPEC §3 / §4 | `signupPolicy`(open / invite / closed)の判定ゲート + サインアップ招待コードの受理・消費(§2-2)。プロジェクト招待(§15)との合成経路(login への添付)を含む | H1 |
...
| 9 | AUTH_SPEC §4 | CLI ログインのスケール経路(device flow 50 回/時の突破 — §3-4 の (a) web-flow ハンドオフが第一候補)。招待制ベータの実測後に裁定 | オープンベータ前 |
...
| 段 | 内容 | 停止しても安全な理由 |
|---|---|---|
| **H1** | サインアップ制御 — `signupPolicy` + サインアップ招待コード(AUTH_SPEC §3 / §4 改訂は実装 PR 側)。§15 招待との合成経路込み | 既定 `open` = 現行挙動と同一。セルフホスト・既存ユーザーに影響ゼロ |
...
| **H4** | 法務・商務 — §7 の人間タスク(エージェントは草稿支援まで)+ アカウント削除(gap 6)・通知経路(gap 8)の設計 | 文書・外部手続きが主。製品変更は gap 6 / 8 の独立 PR |
| **H5** | 公開儀式 — SECURITY.md + 脅威モデル文書(§6)+ 公開前チェックリスト残(Deploy ボタン検証・CLI 配布・公証)→ **public 化** | 公開は不可逆だが、それ自体が目的の到達点。以後も招待制ベータを開けずに留まれる |
| **H6** | 招待制ベータ開放 — 招待コード発行の運用開始・「最初の 5 分」体験の充足確認(ADR-0014 改訂 1 の体験要件) | ゲート(§4)を満たすまで H5 で停止可能。開放後も `signupPolicy` を closed に戻せる(新規のみ停止 — 可逆) |
