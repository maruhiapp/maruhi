...
- **設定値を配布しない**: `GET /auth/config` 等の未認証応答に `signupPolicy` を載せない
  (schemaPolicy の advisory 配布と異なり、未認証者への開示はサインアップ面の偵察材料に
  しかならない — 人間向けの案内はランディングの文言で足りる。session-47 §8 第 4 周)
...
2. **トリップワイヤ監視**(H3): check-token 応答の rate limit ヘッダー(x-ratelimit-remaining)
   の閾値アラート + device 交換の失敗率。コード入力 50 回/時の直接観測は不能なので、
   サインアップ・交換レートからの推定で代える(§6)
...
| GitHub クォータ | check-token の x-ratelimit-remaining / 交換失敗率 | remaining < 20% / 失敗率 5% |
