...
- 時期の推奨: **設計(仕様)は招待制ベータ前に KL3 と一緒に確定し、実装はベータと並走(ゲートにしない)**。理由: ラップ台帳のデータモデルを先に決めれば初期ユーザーのラップを後で移行せずに済む。招待制ベータの人数は少なく、リカバリーコードの保管は個別に念押しできる。個人利用者には保護者がいないので、価値が出るのはチーム利用からで、ベータの声で優先度が測れる
