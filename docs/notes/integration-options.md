...
| ドライバ | 1 インターフェース × 2 種類。**第一級 = Vercel / Cloudflare Workers の `http` ドライバ**(maruhi 自身のコード・一括 upsert・型付きエラー・実行時の供給網なし)。**テール + GitHub = `exec` ドライバ**(ベンダー CLI を stdin で駆動。版固定・自動インストールなし・ベンダーテレメトリ off・ログ抑止)。Netlify は第 2 波(http)。プリセットは両種類とも宣言的(JSON + モック応答) |
