...
### 補足 19: KL3 設計録 — master 鍵ラップ台帳(2026-09-12 — フェーズ 1 設計セッション。所有者承認待ち)

KL3 = 封印バックアップを CRYPTO_SPEC §8 の一般化として設計する回。仕様改訂の**ドラフト本文**は docs/notes/kl3-spec-drafts.md(CRYPTO_SPEC §8 / AUTH_SPEC §13 / AUDIT_SPEC §3.1 の差し替え・追記案。正本は承認まで書き換えない)。本補足は設計の全体像・裁定の反復記録・KL2 保留項目の裁定・実装分割・承認依頼項目を持つ。
