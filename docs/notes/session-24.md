...
1. 重複キーは jti ではなく **SHA-256(トークン生バイト列)**(jti の有無・
   意味論が issuer 依存 — §2 表。トークンハッシュは issuer に何も要求しない)
...
- 発行時に「SHA-256(トークン) → 一時公開鍵」をプロジェクト DO に記録。
  同一トークン + 同一鍵 = 冪等許可 / 同一トークン + 別鍵 = 401 `token-replayed`
...
- A3 クライアントの必須変更なし。SHOULD 2 点: (1) トークンは lease 要求の
  直前に発行する(先着窓の最小化)、(2) GitHub(ランタイム発行型)は
  `token-replayed` に対し新規トークンで 1 回だけ自動再試行してよい
