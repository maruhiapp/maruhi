...
server → CLI の層順でコミット。
**PR #31 は 2026-08-04 マージ済み**(squash merge `d0b9107`。§2 の裁定は所有者
承認済み。§6 の仕様同期 2 件 — AUTH_SPEC §12-8 / §12-4 — はマージ後の
セッション 15.5 で反映済み)。
...
## 2. 裁定の細部(複数案比較 → 推奨で仮進行 → 確定)

2026-08-04 所有者判断: 2-1〜2-3 / 2-5〜2-7 は提示案どおり承認。2-4 は実装側へ
委任され、下記の再検討(新案 3 つを含む 5 案比較)を経て確定した。
...
### 2-3. リクエスト Schema のライフサイクル narrow(確定)
...
### 2-4. 削除ステートメントの配布チャネル(確定 — 委任後の再検討済み)
...
**カスケード時の配下ステートメント削除の確定経緯**(2026-08-04 所有者委任 →
5 案比較で再検討): ① 全残し — 削除済み環境の配下ステートメントには配布
チャネルが存在せず(pull は 404・環境一覧は環境 tombstone のみ)、配布されない
サーバー保存物はクライアントが検証できないため検出材料として機能しない。
② deleted tombstone のみ残す折衷 — 環境 tombstone が単独で全配下の復活不能を
保証するため追加の検出価値ゼロ、名前だけ長く残る劣化形。③ name 編集済みで
残す — 署名が name を含む全フィールドに掛かるため編集で証拠価値を失い不成立。
④ signed bytes hash ダイジェストのみ残す — 本設計でサーバーは信頼されない
当事者であり、equivocation の証拠力はクライアント保持の検証済みコピー +
チェーンにある。サーバー自身のストレージ内ダイジェストはサーバーに対する
証拠にならない。⑤ 削除(採用)— 検出材料としての価値が構造的にゼロで、
「削除で解放される」原則(§12-8)と E2EE の削除意味論(変数名自体が情報を
持ちうる)に一貫する。運用上の追跡可能性は監査ログ(append-only。name
スナップショット + author 鍵 FP — AUDIT_SPEC の設計)が独立に保持する。
...
### 2-6. meta-versions 上限(確定 — §12-8 の表にない受理ポリシー追加)
...
承認後に AUTH_SPEC 側で行う想定(→ マージ後のセッション 15.5 で AUTH_SPEC
§12-8 に反映済み)。
...
- **AUTH_SPEC §12-8 の表への `meta-versions` 追記**(§2-6 は所有者承認済み):
  「metaVersion 行数 / 変数(環境)= 1,000(status deleted は対象外)」を
  受理ポリシー表に追加する仕様同期 PR を推奨(→ セッション 15.5 で反映済み)
- **AUTH_SPEC §12-4 のカスケード対象の明文化**(レビュー③ minor): 環境削除の
  カスケード対象列挙(「配下の変数・バージョン・ラップ済み DEK」)に「変数メタ
  ステートメント」を明記する改訂を人間に提案する(根拠 = 環境 ID のチェーン
  再利用不能。§2-4 の裁定の仕様側固定)(→ セッション 15.5 で反映済み)
