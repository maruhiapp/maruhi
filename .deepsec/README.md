...
Currently configured project: `maruhi` (target: `..`).

maruhi policy (why this is opt-in, how credentials work, what not to
commit): [`docs/DEEPSEC.md`](../docs/DEEPSEC.md).
...
`--project-id maruhi` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/maruhi/`.
