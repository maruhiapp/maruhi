...
export const renameEnvironmentProgram = (
  ...
  Effect.gen(function* () {
    ...
    // DO ストレージ総量ガード(§12-8): 環境の改名はステートメント行 +
...
const requirePullContext = (actor: DataActor, environmentId: string, cache: StateCache) =>
  Effect.gen(function* () {
    ...
    // 最新マニフェスト(§12-7 の同梱材料)。null はマニフェスト
...
export const pullEnvironmentProgram = (
  ...
  Effect.gen(function* () {
    ...
    // 監査(AUDIT_SPEC §3.3 — 集約形): 値付き一括 pull は環境単位
...
/**
 * メタデータのみモード(§12-7): 値(暗号文)と DEK を返さず、
