...
describe("maruhi device approve", () => {
  ...
  it("チェーンに別の cap で載っている鍵の再実行は、何も追記・記録せず要求を残して拒否し、同じ cap のコマンドを出す(DK K10-1)", async () => {
    ...
    expect(errors).toContain("revoke it and re-add it instead");
    expect(state.appendedTo).toEqual([]);
    expect(state.registryPuts).toEqual([]);
    expect(state.requestCancels).toEqual([]);
...
describe("初回同期の端末登録(device-sync — K4-3 / K4-4 / K4-9)", () => {
  ...
  it("cap がこの端末の cap を越える端末の印は包まずに消し、載せた端末の再試行と足し直しを案内する(DK K11-8)", async () => {
    ...
    const wide = await makeTestUser(owner.userId);
    const built = await buildChain([
      { actor: owner, operation: genesisOp(owner) },
      { actor: owner, operation: createEnvironmentOp(ENV_ID, dek) },
      {
      ...
      { actor: owner, operation: addDeviceOp(wide) },
    ]);
    const { server, state } = await makeServer({
      built,
      withEnvironment: true,
    ...
    const env = await startEnv(server.origin, built.projectId, dev2);
    await markPendingBackfill(env, server.origin, built.projectId, wide);
    await runCli(["invite", "create", "--role", "member"], env.layer);
    expect(state.registered).toEqual([]);
    expect(env.errors.join("\n")).toContain(
      `the backfill of DEK wraps to your device ${wide.fingerprintHex} (cap owner/all) on project ${built.projectId} is unfinished, and this device's cap member/${ENV_ID} does not cover it, so this machine stops retrying it. The device that added it (\`maruhi device list\` shows which) always has a cap that covers it`,
    );
    expect(await readPendingBackfills(env, server.origin)).toEqual([]);
  });
