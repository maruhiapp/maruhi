...
describe("maruhi login", () => {
  it("device flow → 交換 → maruhi トークンのみキーチェーンへ保存する", async () => {
    const github = fakeGitHub({ pendingPolls: 2, accessToken: "gho_githubTokenValue" });
    ...
    expect(receivedGithubToken).toBe("gho_githubTokenValue");
    ...
    expect(stored).not.toContain("gho_githubTokenValue");
    // トークン生値は端末出力にも出ない
