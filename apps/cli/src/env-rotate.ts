...
function describeSendFailure(input: {
  ...
  return Effect.gen(function* () {
    ...
    return cliError(
      `${input.cause}. The chain shows it was not accepted (environment ${input.environmentId} is still at epoch ${probe.value.currentEpoch}). It is safe to simply re-run`,
    );
  });
}
  ...
  return Effect.gen(function* () {
    ...
    const confirmed = yield* Effect.gen(function* () {
      ...
      if (environment.currentEpoch !== input.newEpoch) {
        return yield* Effect.fail(
          cliError(
            `The resynced chain is now at epoch ${environment.currentEpoch} (possibly a concurrent rotation right after acceptance)`,
          ),
        );
      }
      ...
      Effect.mapError((error) =>
        cliError(
          `The rotation (epoch=${input.newEpoch}) was accepted, but the post-acceptance check failed: ${error.message}. Environment ${input.environmentId}'s epoch has advanced and no current values have been re-encrypted — resolve the cause and re-run to resume re-encryption without advancing the epoch`,
        ),
      ),
    );
...
/** rotate 前の状況警告(マニフェスト移行の 2 態 + 床なし)。pull 警告の直後に積む。 */
