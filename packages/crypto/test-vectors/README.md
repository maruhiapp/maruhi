...
12. **メンバー鍵の一意性(2026-08-03 決定 — セッション 10。CRYPTO_SPEC §6.2)**: `add_member` は対象の enc / sig 公開鍵の**いずれか**が現メンバー集合の**同種**鍵と一致する場合に拒否する(negative `authz-add-member-duplicate-key` / `-duplicate-enc-key` / `-duplicate-sig-key` で固定。期待理由 `duplicate-member-key`)。判定は enc / sig の個別鍵単位(FP = enc‖sig の一致判定ではない — 片鍵流用の negative 2 件がこれを固定する)。禁止範囲は**現メンバー集合のみ**: 削除済みメンバーの同一鍵 re-add(同一人物の復帰)は拒否しない(実装テストの positive で固定)。検査順序は role 規則 → `duplicate-member`(user_id)→ `duplicate-member-key`(鍵)— 既存 negative `authz-admin-adds-admin`(削除済み user-member-0002 の同一鍵 re-add を insufficient-role で固定)と `add_member duplicate` 実装テスト(既存 user_id の再追加を duplicate-member で固定)の期待理由はこの順序に依存する。**このベクター自体も PR レビューでの人間確認対象**(§6.2 の合意規則改訂と同時にコミットし、実装より先行する)
