...
- 値なしスキーマ(名前・型・説明・必須のみをエージェントへ開示。配信は `maruhi schema` CLI — MCP 配信は後回し〔次行〕。`.env.schema` ファイルは正にしない)— **仕様起草済み(2026-08-30 S0)**: CRYPTO_SPEC 0.8-draft §4.2 レイアウト v2 / AUTH_SPEC 0.16-draft §12-11 スキーマポリシー。CLI・付帯面と実装分割(S1〜)は docs/notes/value-free-schema-design.md、裁定は docs/notes/session-46.md(検討メモ: docs/notes/value-free-schema.md)。**S1 は 2026-08-31 実装完了(PR #116 — テストベクター先行 + `packages/crypto` のレイアウト v2 encode / verify・declared 対応。v1 ベクターは純追記で不変。CI に参照検証器を常設)**。**順序の正式化(2026-08-30 H0 裁定 DB — session-46 §9 申し送りの解消): S1〜S3(署名・受理・CLI 検証面)は招待制ベータ開放前に着地する(H 系列のゲート)。S4〜S5(import / export / lint — 生成物・付帯 UX)はゲートにせずベータと並走可**
