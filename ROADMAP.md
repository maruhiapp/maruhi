...
- ユーザーに見える文言を英語へ統一(ADR-0017)。CLI はコマンド単位で ADR-0016 の引数層移行と同じ PR で行う(テストが日本語の部分文字列で挙動を固定しているため一括置換は危険)。**ただし引数層を先に移した `pull` / `run` / `env create` の 3 コマンドは移行 PR が二度と来ない**ので、この 3 つ分は独立作業として行う。web(`Root.tsx` の `lang="ja"`)・docs サイト・README も別作業
- docs サイト(Blume)
