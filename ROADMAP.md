...
- ヘッドゴシップ検証(環境マニフェスト・定期チェックポイントの設計と同時 — CRYPTO_SPEC 未決 #12)**設計・仕様起草済み(2026-08-18 — Wave 3 D。PR #80、docs/notes/session-27.md、CRYPTO_SPEC 0.6-draft §4.3 / §6.2 `checkpoint` / §6.6、AUTH_SPEC §16、AUDIT_SPEC 未決 #2 統合。仕様 PR #80 のマージ = 所有者承認)。実装は承認後の後続 PR(PR-M1 マニフェスト → M2 チェックポイント → M3 値スナップショット、M4 ゴシップは並走可 — session-27 §14)**
