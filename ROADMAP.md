...
- [ ] H1: サインアップ制御 — deployment 設定 `signupPolicy`(open / invite / closed。セルフホスト既定 open = 現行挙動)+ サインアップ招待コード(単回・期限つき bearer)+ コードの未認証事前検証エンドポイント + `/auth/config` の advisory 配布 + プロジェクト招待(AUTH_SPEC §15)との合成経路 + **CLI の login 事前 fail-fast(ログイン開始前 — 2026-08-31 の AUTH_SPEC §4 全面改訂〔session-48 裁定 DF〕により対象は `POST /auth/cli/start` 前へ再基底化。招待コードの事前検証は start payload への添付検証に統合可 — hosted-design.md §2-2 追記)**。AUTH_SPEC §3 / §4 の改訂は実装 PR 側(§4 の本体改訂は 2026-08-31 に着地済み — H1 分は signupPolicy まわりの追補のみ)
- [ ] H1b(2026-08-31 追加 — session-48 裁定 DF): CLI ログイン経路の置換(gap 9 の実装 — AUTH_SPEC §4 全面改訂 2026-08-31): サーバー `POST /auth/cli/start` / `GET /auth/cli/verify` / `POST /auth/cli/poll` + スクリプトなし承認ページ + 旧 `/auth/device/exchange`・check-token・形式事前検査の削除、CLI = login の置換(device flow 削除・client_id 解決の廃止・プロバイダ非依存)。`docs/SELF_HOSTING.md` の追随込み。H1 と独立・並走可。**H5 の前提に加える**(公開後の削除は破壊的変更 — 公開前が最安。hosted-design.md §9 追記)
