...
- [Deploy targets](https://maruhi.app/docs/deploy-targets) — copying values to Cloudflare Workers and Vercel with `maruhi sync` (a config file in the repository, `plan` / `apply`, and the vendor CLI you already use) or with `maruhi run` recipes (source: [`apps/site/docs/deploy-targets.mdx`](apps/site/docs/deploy-targets.mdx))
