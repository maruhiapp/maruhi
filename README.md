...
- [Deploy targets](https://maruhi.app/docs/deploy-targets) — copying values to Cloudflare Workers, Vercel, and Netlify with `maruhi sync` (a config file in the repository, `plan` / `apply`, the vendor CLI you already use or the vendor API with a token stored in maruhi, and `maruhi ci sync` from CI) or with `maruhi run` recipes (source: [`apps/site/docs/deploy-targets.mdx`](apps/site/docs/deploy-targets.mdx))
- [GitHub Actions](https://maruhi.app/docs/github-actions) — using maruhi from a workflow with no secrets stored on GitHub: `maruhi ci run` with an OIDC lease, the two workflow shapes for `maruhi ci sync` (re-apply on every deploy; dispatched by `maruhi push` plus a schedule), the lease policy, and required reviewers on production through GitHub Environments (source: [`apps/site/docs/github-actions.mdx`](apps/site/docs/github-actions.mdx))
