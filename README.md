...
- [Deploy targets](https://maruhi.app/docs/deploy-targets) — copying values to Cloudflare Workers, Vercel, Netlify, and GitHub Actions secrets with `maruhi sync` (a config file in the repository, `plan` / `apply`, the vendor CLI you already use or the vendor API with a token stored in maruhi, and `maruhi ci sync` from CI) or with `maruhi run` recipes (source: [`apps/site/docs/deploy-targets.mdx`](apps/site/docs/deploy-targets.mdx))
- [GitHub Actions](https://maruhi.app/docs/github-actions) — using maruhi from a workflow with no secrets stored on GitHub: `maruhi ci run` with an OIDC lease, the two workflow shapes for `maruhi ci sync` (re-apply on every deploy; dispatched by `maruhi push` plus a schedule, including GitHub secrets as a target with one bootstrap token), the lease policy, and required reviewers on production through GitHub Environments (source: [`apps/site/docs/github-actions.mdx`](apps/site/docs/github-actions.mdx))
- [Environment scopes](https://maruhi.app/docs/environment-scopes) — limiting a member to some environments by key distribution rather than a server rule: `maruhi invite create --env …`, `maruhi member change-role [--role] [--env … | --all-envs | --no-envs]`, what a member sees outside their scope, the rotation and backfill the CLI runs on narrowing and widening, and `maruhi member list` (source: [`apps/site/docs/environment-scopes.mdx`](apps/site/docs/environment-scopes.mdx))
- [Four-eyes approvals](https://maruhi.app/docs/four-eyes) — requiring the signatures of two or more owners on the project's chain before a member is removed, a server key is granted, a role is changed, or the policy itself changes: `maruhi project policy approvals`, the existing commands turning into proposals, `maruhi approval list / show / approve / withdraw`, how votes are recounted, and the operational prerequisites (source: [`apps/site/docs/four-eyes.mdx`](apps/site/docs/four-eyes.mdx))
...
| Brand assets in `apps/web/public/` (the ㊙ mark: `logo*.svg`, `favicon*`, `og.png`) | The `秘` outline derives from Noto Sans CJK JP (SIL OFL 1.1) — notice and license text in [`apps/web/public/fonts/OFL-NotoSansCJK.txt`](apps/web/public/fonts/OFL-NotoSansCJK.txt) (english-exempt: 秘 names the source kanji the mark derives from) |
