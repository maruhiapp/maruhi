...
- テレメトリ・外部送信を一切実装しない(「言わざる」)。利用者が明示した相手への、利用者の目的のための通信(同期先〔`maruhi sync`〕・身元の裏付け元〔`identityBacking = github-signing-keys` の公開 API 照合 — 送るのは名指しした login のみ、`none` で停止可〕)はテレメトリではない(2026-09-13 IV 裁定)
