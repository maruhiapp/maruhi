...
- 値を表示する系のコマンド(例: 値の cat / export)は **fail-closed** で守る(ADR-0016): 一次境界は「stdin と stdout の両方が端末か」(`Stdio` サービス)、二次層が既知の AI エージェント検出(std-env)。判定材料は Effect のサービス経由で取り、`process.*` を直に読まない
...
| CLI | Effect(`effect/unstable/cli` — ADR-0016。gunshi から移行中)+ HttpApi 導出型付きクライアント |
